	...
	return (
		<section className="relative">
			<div className="sticky top-0 left-0 right-0 z-40 flex items-center justify-between px-4 py-2.5 bg-background/95 backdrop-blur-sm border-b gap-2">
				...
				/>
				<CodeDialog onClick={fetchSourceCode} sourceCode={sourceCode} />
				<Copy text={sourceCode} onFetch={fetchSourceCode} />
			...
			>
				...
				>
					...
					<ResizablePanel
						id="center"
						onResize={(w) => {
							console.log({ get: w.inPixels });
							setContainerWidth(w.inPixels / 16);
						}}
					>
						{children}
					</ResizablePanel>
	...
	return (
		<div className="flex items-center gap-2 flex-wrap min-w-0 flex-1">
			{[
				[
					website,
					'bg-primary/10 text-primary border border-primary/30 ring-1 ring-primary/20 shadow-sm dark:bg-primary/15 dark:text-primary rounded',
				],
				...
				.filter(([item]) => item)
				.map(([item, className], index) => (
					<Badge
						key={index}
						className={`text-xs capitalize shrink-0 ${className}`}
					>
						{item}
					</Badge>
				))}
			...
			{href && (
				<Badge
					variant="outline"
					className="cursor-pointer border-2 border-violet-900 focus:ring-2 active:bg-violet-900 "
					onClick={() => navigator.clipboard.writeText(href + '.tsx')}
				>
					path
				</Badge>
			)}
