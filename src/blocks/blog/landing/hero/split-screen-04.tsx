...
const FormatCards = ({ items }: { items: FormatItem[] }) => (
	<div className="grid grid-cols-2 gap-4">
		{items.map(({ icon: Icon, title, count, color }) => (
			<Card
				key={title}
				className="group cursor-pointer transition-all hover:shadow-lg hover:border-primary/50 py-0"
			>
				<CardContent className="p-6 @md:p-8 text-center">
					<div
						className={`size-14 @md:size-16 rounded-2xl bg-card border flex items-center justify-center mx-auto mb-4 transition-transform group-hover:scale-110 ${color}`}
					>
