					...
					articles={[
						{
							...
							excerpt:
								'Learn the patterns and practices used by top engineering teams.',
							image:
		...
		{articles.map((article) => (
			<Link key={article.title} href="#" className="block break-inside-avoid">
				<Card className="group overflow-hidden py-0">
					<div
						className={`relative ${article.size === 'large' ? 'aspect-[4/5]' : article.size === 'medium' ? 'aspect-[4/3]' : 'aspect-video'}`}
					>
						<Image
							src={article.image}
							alt={article.title}
