        ...
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="relative mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="max-w-4xl mx-auto text-center">
                    <AuthorStack
                        authors={[
                            { name: 'Alex Rivera', image: 'https://i.pravatar.cc/100?img=1', initials: 'AR' },
                            { name: 'Jordan Lee', image: 'https://i.pravatar.cc/100?img=2', initials: 'JL' },
                            { name: 'Sam Chen', image: 'https://i.pravatar.cc/100?img=3', initials: 'SC' },
                            { name: 'Taylor Kim', image: 'https://i.pravatar.cc/100?img=4', initials: 'TK' },
                        ]}
                    ...
                    <CTA
                        items={[
                            { label: 'Explore Articles', href: '/articles', icon: ArrowRight },
                            { label: 'Meet the Authors', href: '/authors', variant: 'outline' },
                        ]}
                    />
                </div>
            </div>
...
const AuthorStack = ({ authors, label }: { authors: Author[]; label: string }) => (
    <div className="flex items-center justify-center gap-3 mb-6 @md:mb-8">
        <div className="flex -space-x-3">
            {authors.map((author) => (
                <Avatar key={author.name} className="size-10 @md:size-12 ring-2 ring-background">
                    <AvatarImage src={author.image} alt={author.name} />
                    <AvatarFallback className="text-xs bg-primary text-primary-foreground">{author.initials}</AvatarFallback>
                </Avatar>
            ))}
        </div>
        <Badge variant="secondary" className="text-xs @md:text-sm">{label}</Badge>
    </div>
)
