	...
	>
		<CardContent className="p-6 @md:p-8">
			...
			<p className="text-muted-foreground mb-6 max-w-md">
				Connect with fellow developers, share knowledge, and grow together. Your
				journey starts here.
			</p>
...
const LeaderboardCell = ({ leaders, className }: LeaderboardCellProps) => (
	<Card
		className={`bg-gradient-to-b from-amber-500/10 via-card to-card ${className}`}
	>
...
const DiscussionCell = ({ topic, replies, className }: DiscussionCellProps) => (
	<Card
		className={`group cursor-pointer transition-all hover:border-primary ${className}`}
	>
		<CardContent className="p-5 flex flex-col justify-center h-full">
			...
			<p className="text-sm text-muted-foreground">
				{replies} replies · Join the conversation
			</p>
		</CardContent>
	</Card>
);
