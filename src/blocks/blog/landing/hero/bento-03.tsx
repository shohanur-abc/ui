...
import { ArrowRight, Code, Database, Palette, Server, Sparkles, Terminal, Zap } from 'lucide-react'
        ...
        <section className="@container relative overflow-hidden" data-theme="emerald">
            <div className="relative mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                <div className="grid grid-cols-2 @md:grid-cols-4 @xl:grid-cols-6 gap-4 auto-rows-[minmax(140px,auto)]">
                    ...
                    />
                    <CategoryCell icon={Code} label="Frontend" count={342} color="text-blue-500" className="" />
                    <CategoryCell icon={Server} label="Backend" count={256} color="text-green-500" className="" />
                    <CategoryCell icon={Database} label="Database" count={128} color="text-purple-500" className="" />
                    <CategoryCell icon={Terminal} label="DevOps" count={187} color="text-orange-500" className="" />
                    <CategoryCell icon={Palette} label="Design" count={94} color="text-pink-500" className="" />
                    <CategoryCell icon={Zap} label="Performance" count={76} color="text-amber-500" className="" />
                </div>
            </div>
        </section>
    )
...
const CategoryCell = ({ icon: Icon, label, count, color, className }: CategoryCellProps) => (
    <Link href={`/category/${label.toLowerCase()}`}>
        <Card className={`h-full group cursor-pointer transition-all hover:border-primary hover:shadow-lg hover:-translate-y-1 ${className}`}>
            <CardContent className="p-5 flex flex-col items-center justify-center h-full text-center">
                <div className={`size-12 rounded-xl bg-muted flex items-center justify-center mb-3 transition-transform group-hover:scale-110 ${color}`}>
