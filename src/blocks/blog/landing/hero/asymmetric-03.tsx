	...
	<div className={`grid grid-cols-2 gap-4 ${className}`}>
		{categories.map((cat, i) => (
			<Link key={cat.label} href={`/category/${cat.label.toLowerCase()}`}>
				<Card
					className={`group overflow-hidden cursor-pointer py-0 h-full ${i === 0 ? 'col-span-2 @md:col-span-1 @md:row-span-2' : ''}`}
				>
					<div
						className={`relative ${i === 0 ? 'aspect-square' : 'aspect-video'}`}
					>
						<Image
							src={cat.image}
							alt={cat.label}
