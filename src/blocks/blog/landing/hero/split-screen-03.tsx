            ...
            <div className="relative mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                <div className="grid grid-cols-1 @3xl:grid-cols-2 gap-8 @xl:gap-12 items-center">
                    <ContentSection
                        ...
                        cta={[
                            { label: 'Read Article', href: '/articles/resilient-systems', icon: ArrowRight },
                            { label: 'Save for Later', href: '#', icon: Bookmark, variant: 'outline' },
                        ]}
                    />
                    <ImageSection
...
const ContentSection = ({ eyebrow, title, description, author, meta, cta }: ContentSectionProps) => (
...
const ImageSection = ({ src, alt, tags }: ImageSectionProps) => (
    <div className="relative group">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-2">
            {tags.map((tag) => (
                <Badge key={tag} className="bg-background/90 text-foreground backdrop-blur-sm">
                    {tag}
                </Badge>
            ))}
        </div>
