...
import { ArrowRight, Calendar, Clock, Play, Sparkles, Star } from 'lucide-react'
...
const MainCell = ({ title, cta, className }: MainCellProps) => (
    <Card className={`relative overflow-hidden bg-gradient-to-br from-primary/10 via-card to-transparent border-primary/10 flex flex-col justify-end ${className}`}>
...
const AuthorCell = ({ author, className }: AuthorCellProps) => (
    <Card className={`flex items-center transition-all hover:border-primary ${className}`}>
        <CardContent className="p-5 flex items-center gap-4 w-full">
            <Avatar className="size-14 ring-2 ring-primary/20">
                <AvatarImage src={author.avatar} alt={author.name} />
                <AvatarFallback className="bg-primary text-primary-foreground">{author.initials}</AvatarFallback>
            </Avatar>
...
const VideoCell = ({ title, duration, className }: VideoCellProps) => (
    <Card className={`relative overflow-hidden bg-gradient-to-br from-accent/20 to-card group cursor-pointer ${className}`}>
...
const RatingCell = ({ rating, reviews, className }: RatingCellProps) => (
    <Card className={`flex flex-col items-center justify-center text-center ${className}`}>
