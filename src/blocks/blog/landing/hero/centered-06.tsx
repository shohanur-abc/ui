						...
						items={[
							{
								...
								description: 'Get notified instantly when news breaks',
							},
							{
								icon: Clock,
								title: 'Daily Digests',
								description: 'Curated summaries delivered to your inbox',
							},
							{
								icon: Flame,
								title: 'Trending Analysis',
								description: 'Deep dives into what matters most',
							},
						]}
					/>
					<CTA
...
const FeatureCards = ({ items }: { items: FeatureItem[] }) => (
	<div className="grid grid-cols-1 @sm:grid-cols-3 gap-4 mb-8 @md:mb-10">
		{items.map(({ icon: Icon, title, description }) => (
			<Card
				key={title}
				className="bg-card/50 backdrop-blur border-primary/10 py-4"
			>
