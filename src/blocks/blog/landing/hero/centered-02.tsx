	...
	<div className="flex flex-wrap justify-center gap-2 @md:gap-3">
		{items.map(({ icon: Icon, text }) => (
			<Badge
				key={text}
				variant="secondary"
				className="flex items-center gap-2 px-2.5 @md:px-3 py-1.5"
			>
...
const Newsletter = ({
	...
	<div className="flex flex-col @sm:flex-row gap-2 @md:gap-3 max-w-md mx-auto">
		<Input
			...
			className="h-11 @md:h-12 px-3 @md:px-4"
		/>
	...
	<div className="flex flex-col items-center gap-3 @md:gap-4 pt-6 @md:pt-8">
		<div className="flex -space-x-3">
			{items.map(({ src, fallback }, i) => (
				<Avatar
					key={i}
					className="border-2 border-background size-9 @md:size-10"
				>
					<AvatarImage src={src} />
		...
		<p className="text-sm text-muted-foreground">
			5,000+ readers joined this month
		</p>
	</div>
);
