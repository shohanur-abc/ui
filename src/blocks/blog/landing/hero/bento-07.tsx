			...
			<div className="relative mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
				<div className="grid grid-cols-1 @md:grid-cols-2 @xl:grid-cols-4 gap-4 auto-rows-[minmax(160px,auto)]">
					...
					<FeatureCell
						icon={Star}
						title="Early Access"
						description="New content first"
						className=""
					/>
					<FeatureCell
						icon={Zap}
						title="AI Features"
						description="Smart summaries"
						className=""
					/>
					<TestimonialCell
...
const HeroCell = ({ title, subtitle, className }: HeroCellProps) => (
	<Card
		className={`relative overflow-hidden bg-gradient-to-br from-amber-500/10 via-card to-orange-500/5 border-amber-500/20 flex flex-col justify-center ${className}`}
	>
...
const PricingCell = ({
	...
	<Card
		className={`bg-gradient-to-b from-primary/5 to-card border-primary/20 ${className}`}
	>
...
const FeatureCell = ({
	...
	<Card
		className={`transition-all hover:border-amber-500/50 hover:shadow-lg ${className}`}
	>
