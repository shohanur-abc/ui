...
import { ArrowRight, Code2, ExternalLink, Github, Sparkles, Star, Terminal } from 'lucide-react'
...
const HeroCell = ({ title, className }: HeroCellProps) => (
    <Card className={`relative overflow-hidden bg-gradient-to-br from-primary/10 via-card to-transparent flex flex-col justify-center border-primary/20 ${className}`}>
        <CardContent className="p-6 @md:p-8">
            ...
            <p className="text-muted-foreground mb-6 max-w-md">
                Articles, tutorials, and resources. All open source. Fork, learn, contribute.
            </p>
...
const CodeSnippetCell = ({ className }: CodeSnippetCellProps) => (
    <Card className={`bg-[#0d1117] border-primary/20 overflow-hidden ${className}`}>
...
const RepoCell = ({ name, description, stars, className }: RepoCellProps) => (
    <Link href={`https://github.com/org/${name}`}>
        <Card className={`group h-full transition-all hover:border-primary ${className}`}>
