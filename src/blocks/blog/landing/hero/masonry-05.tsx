	...
	<div className="columns-1 @sm:columns-2 @xl:columns-3 gap-4 space-y-4">
		{quotes.map((quote) => (
			<Link key={quote.text} href="#" className="block break-inside-avoid">
				<Card
					className={`group transition-all hover:shadow-lg ${quote.featured ? 'bg-gradient-to-br from-primary/10 to-card border-primary/20' : ''}`}
				>
					<CardContent className={`${quote.featured ? 'p-6' : 'p-5'}`}>
						...
						<blockquote
							className={`${quote.featured ? 'text-xl' : 'text-base'} font-medium leading-relaxed mb-4`}
						>
