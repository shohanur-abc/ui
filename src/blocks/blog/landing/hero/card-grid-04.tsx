	...
	<div className="grid grid-cols-2 @md:grid-cols-3 @xl:grid-cols-6 gap-4">
		{categories.map((cat) => (
			<Link key={cat.label} href={`/category/${cat.label.toLowerCase()}`}>
				<Card
					className={`group cursor-pointer transition-all hover:shadow-lg hover:-translate-y-1 ${cat.color}`}
				>
					<CardContent className="p-5 flex flex-col items-center text-center">
						<div
							className={`size-14 rounded-xl flex items-center justify-center mb-3 transition-transform group-hover:scale-110`}
						>
