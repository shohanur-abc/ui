...
const HeroCell = ({ title, cta, className }: HeroCellProps) => (
	<Card
		className={`relative overflow-hidden bg-gradient-to-br from-primary/20 via-card to-accent/10 border-primary/20 flex flex-col justify-end ${className}`}
	>
		<CardContent className="p-6 @md:p-8">
			...
			<p className="text-muted-foreground mb-6 max-w-sm">
				Access thousands of tutorials, articles, and resources. No credit card
				required.
			</p>
...
const FeaturedVideoCell = ({
	title,
	duration,
	thumbnail,
	className,
}: FeaturedVideoCellProps) => (
	<Card
		className={`group relative overflow-hidden cursor-pointer py-0 ${className}`}
	>
		<Image
			src={thumbnail}
			alt={title}
...
const QuickStatCell = ({
	...
	<Card
		className={`flex flex-col items-center justify-center text-center transition-all hover:border-primary ${className}`}
	>
