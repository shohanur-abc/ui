...
import { ArrowRight, Bell, Calendar, Mail, MessageSquare, Sparkles } from 'lucide-react'
        ...
        <section className="@container relative overflow-hidden" data-theme="slate">
            <div className="relative mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                <div className="grid grid-cols-1 @md:grid-cols-2 @xl:grid-cols-4 gap-4 auto-rows-[minmax(160px,auto)]">
                    ...
                    <NotificationCell
                        items={[
                            { type: 'comment', text: 'New reply on your post', time: '2m ago' },
                            { type: 'mention', text: '@you in React discussion', time: '15m ago' },
                        ]}
                        className=""
                    />
                    <UpcomingCell
                        event={{
                            ...
                            date: 'Feb 5, 2026',
                            host: { name: 'Alex Kim', avatar: 'https://i.pravatar.cc/100?img=50' },
                        }}
                        className=""
                    />
                </div>
...
const NewsletterCell = ({ title, description, className }: NewsletterCellProps) => (
        ...
        <CardContent className="p-5 flex flex-col justify-between h-full">
            ...
            <div className="flex items-center gap-2 mt-3">
                <Avatar className="size-6">
                    ...
                    <AvatarFallback className="text-[10px]">{event.host.name[0]}</AvatarFallback>
                </Avatar>
                <span className="text-xs text-muted-foreground">with {event.host.name}</span>
            </div>
        </CardContent>
    </Card>
)
