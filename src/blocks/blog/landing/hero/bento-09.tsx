	...
	>
		<CardContent className="p-6 @md:p-8">
			...
			<p className="text-muted-foreground mb-6 max-w-md">
				Stay informed with the latest tech news, breaking stories, and industry
				updates.
			</p>
...
const FeaturedArticleCell = ({
	title,
	image,
	time,
	className,
}: FeaturedArticleCellProps) => (
	<Card
...
const TagCell = ({ tag, count, hot, className }: TagCellProps) => (
	<Card
		className={`flex items-center justify-center text-center transition-all hover:border-destructive cursor-pointer ${className}`}
	>
...
const QuickLinkCell = ({
	...
	<Link href={href}>
		<Card
			className={`h-full flex items-center justify-center text-center transition-all hover:border-primary hover:shadow-lg ${className}`}
		>
			<CardContent className="p-4">
