...
const SwipeableItemRow = ({ item }: { item: CartItem }) => (
	<div className="relative overflow-hidden group">
		<div className="absolute inset-y-0 right-0 w-20 bg-destructive flex items-center justify-center">
			<Trash2 className="size-6 text-destructive-foreground" />
		</div>
		<div className="relative flex gap-4 py-4 bg-background transition-transform active:translate-x-0 hover:-translate-x-20 touch-pan-y">
			...
			<div className="flex min-w-0 flex-1 flex-col justify-between">
				...
				<div className="flex items-center gap-2">
					<Button
						size="icon"
						variant="outline"
						className="size-10 rounded-full active:scale-95 transition-transform"
					>
						...
						<ScrollArea className="flex-1 -mx-6 px-6">
							<div className="divide-y divide-border">
								{cartData.items.map((item) => (
									<SwipeableItemRow key={item.id} item={item} />
								))}
							</div>
						</ScrollArea>
