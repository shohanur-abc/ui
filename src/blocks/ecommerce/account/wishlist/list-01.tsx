...
const ListItem = ({ item }: { item: WishlistItem }) => (
	<div
		className={`flex gap-4 p-4 rounded-xl border bg-card transition-all hover:shadow-md ${item.isSelected ? 'ring-2 ring-primary' : ''}`}
	>
		...
		<div className="flex-1 min-w-0">
			...
			<div className="flex items-center gap-2 mt-2">
				...
				<Badge
					...
					className={!item.inStock ? 'text-destructive' : ''}
				>
...
export default function Main() {
	const wishlistItems: WishlistItem[] = [
		...
		},
		{
			...
			description: '34-inch QHD display with 144Hz refresh',
			price: 599.0,
			originalPrice: 699.0,
			image:
		...
		},
		{
			...
			description: 'Precision sensor with programmable buttons',
			price: 79.0,
			image:
