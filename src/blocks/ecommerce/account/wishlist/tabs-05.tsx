...
const GridView = ({ items }: ViewProps) => (
	<div className="grid grid-cols-2 @sm:grid-cols-3 @md:grid-cols-4 gap-4">
		{items.map((item) => (
			<div
				key={item.id}
				className="group rounded-xl overflow-hidden bg-card border hover:border-primary transition-colors"
			>
		...
		<table className="w-full">
			<thead className="bg-muted">
				<tr>
					...
					<th className="text-center p-3 text-sm font-medium hidden @md:table-cell">
						Rating
					</th>
