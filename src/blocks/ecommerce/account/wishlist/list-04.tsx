	...
	return (
		<div className={`flex items-center gap-1 ${color}`}>
			...
			<span className="text-sm font-medium">
				{trend === 'stable'
					? 'Stable'
					: `${change}% ${trend === 'down' ? 'lower' : 'higher'}`}
			</span>
		</div>
	);
};
		...
		<div className="mt-3">
			...
			<div className="relative h-2 bg-gradient-to-r from-green-500 via-yellow-500 to-red-500 rounded-full">
				<div
					...
					style={{
						left: `calc(${Math.min(Math.max(position, 0), 100)}% - 8px)`,
					}}
				/>
			</div>
		</div>
			...
			<div className="flex-1 min-w-0">
				...
				<div className="flex flex-wrap items-center gap-4 mt-2">
					...
					<PriceTrend
						...
						change={item.priceHistory.change}
					/>
				</div>
				<PriceRange
		...
		<div className="flex items-center gap-3">
			...
			<div>
				...
				<p className="text-sm text-green-600">
					2 items in your wishlist are at their lowest price
				</p>
			</div>
		</div>
	</div>
