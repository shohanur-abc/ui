...
import { Heart, ShoppingCart, X, Target, TrendingUp, Award } from 'lucide-react';
...
const SavingsProgress = ({ price, saved }: { price: number; saved: number }) => {
	...
	return (
		<div className="mt-3 p-3 rounded-lg bg-muted/50">
			<div className="flex items-center justify-between text-sm mb-2">
				...
				<span className="font-medium">${saved.toFixed(0)} / ${price.toFixed(0)}</span>
			</div>
			<Progress value={percentage} className="h-2" />
			{remaining > 0 ? (
				<p className="text-xs text-muted-foreground mt-2">${remaining.toFixed(2)} more to go</p>
			) : (
		...
		<Card className={`p-4 ${goalReached ? 'ring-2 ring-green-500' : ''}`}>
			<div className="flex gap-4">
				...
				<div className="flex-1 min-w-0">
					...
					<span className="text-xl font-bold mt-1 block">${item.price.toFixed(2)}</span>
...
const SavingsSummary = ({ items }: { items: WishlistItem[] }) => {
	...
	const goalsReached = items.filter((item) => item.savedAmount >= item.price).length;
	...
	return (
		<div className="grid grid-cols-3 gap-4 mb-6">
			...
			<Card className="p-4 text-center">
				...
				<p className="text-2xl font-bold">${(totalNeeded - totalSaved).toFixed(0)}</p>
...
export default function Main() {
	const wishlistItems: WishlistItem[] = [
		{ id: '1', name: 'Sony A7 IV Camera', price: 2498.00, image: 'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=200&h=200&fit=crop', savedAmount: 2498.00, targetDate: 'Dec 25', href: '/product/1' },
		{ id: '2', name: 'DJI Mini 3 Pro Drone', price: 759.00, image: 'https://images.unsplash.com/photo-1473968512647-3e447244af8f?w=200&h=200&fit=crop', savedAmount: 450.00, targetDate: 'Jan 15', href: '/product/2' },
		{ id: '3', name: 'iPad Pro 12.9"', price: 1099.00, image: 'https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?w=200&h=200&fit=crop', savedAmount: 275.00, targetDate: 'Feb 28', href: '/product/3' },
	];
