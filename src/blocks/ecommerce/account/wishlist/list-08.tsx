...
const StoreFilter = ({ stores }: { stores: string[] }) => (
	<div className="flex flex-wrap gap-2 mb-6">
		...
		{stores.map((store) => (
			<Button key={store} variant="outline" size="sm">
				{store}
			</Button>
		))}
	</div>
...
export default function Main() {
	const wishlistItems: WishlistItem[] = [
		...
		},
		{
			...
			price: 129.0,
			image:
				'https://images.unsplash.com/photo-1580301762395-21ce84d00bc6?w=200&h=200&fit=crop',
			store: {
				name: 'Home & Heart',
		...
		},
		{
			...
			price: 65.0,
			image:
				'https://images.unsplash.com/photo-1586210579191-33b45e38fa2c?w=200&h=200&fit=crop',
			store: {
				name: 'Artisan Goods',
				logo: 'https://i.pravatar.cc/100?img=1',
				rating: 4.9,
	...
	const uniqueStores = [
		...new Set(wishlistItems.map((item) => item.store.name)),
	];
