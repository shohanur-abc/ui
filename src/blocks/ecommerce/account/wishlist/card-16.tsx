	...
	<div className="mt-3 p-2.5 rounded-lg bg-muted/50">
		<div className="flex items-center gap-2">
			...
			<span className="text-xs text-muted-foreground">
				{friends[0]?.name}
				{totalWants > 1 ? ` and ${totalWants - 1} others` : ''} also want this
			</span>
		</div>
	</div>
);
	...
	<Card className="group overflow-hidden transition-all duration-300 hover:shadow-lg">
		...
		<CardContent className="p-4">
			...
			{item.friendsWhoWant.length > 0 && (
				<FriendsWanting
					friends={item.friendsWhoWant}
					totalWants={item.totalWants}
				/>
			)}
	...
	const wishlistItems: WishlistItem[] = [
		{
			...
			friendsWhoWant: [
				...
				{ name: 'Jordan', avatar: 'https://i.pravatar.cc/100?img=2' },
		...
		},
		{
			...
			friendsWhoWant: [
				...
				{ name: 'Casey', avatar: 'https://i.pravatar.cc/100?img=6' },
			],
			totalWants: 8,
			href: '/product/3',
		},
