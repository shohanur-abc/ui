		...
		<CardContent className="p-4 space-y-3">
			...
			{expiringPoints > 0 && (
				<div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-500/10 p-2 rounded">
					...
					<span>
						{expiringPoints} pts expire {expiringDate}
					</span>
				</div>
			)}
	...
	<Link href="/vip/deals">
		<Card className="h-full hover:bg-muted/50 transition-colors group">
			<CardContent className="p-4 h-full flex flex-col items-center justify-center text-center">
				...
				<Badge className="mt-2 bg-purple-500/20 text-purple-600">
					Save up to {savings}
				</Badge>
			</CardContent>
		</Card>
			...
			<div className="space-y-2">
				<div className="flex justify-between text-sm">
					...
					<span className="text-muted-foreground">
						${requiredSpend.toLocaleString()}
					</span>
				</div>
				<Progress value={(spent / requiredSpend) * 100} className="h-3" />
				<p className="text-xs text-muted-foreground">
					Spend ${(requiredSpend - spent).toLocaleString()} more to reach{' '}
					{nextTier}
				</p>
			</div>
	...
	const profileData = {
		...
		benefits: [
			...
			},
			{
				icon: Zap,
				title: 'Early Access',
				description: 'Shop new arrivals 48h early',
