...
const MaintenanceAlerts = ({
	...
	alerts: { type: 'warning' | 'info' | 'success'; title: string; description: string; action?: string }[];
}) => (
...
const TripHistory = ({
	...
	trips: { from: string; to: string; date: string; distance: string; duration: string }[];
}) => (
	<Card>
		<CardHeader className="pb-3">
...
const Subscriptions = ({
	...
	subscriptions: { name: string; status: 'active' | 'expiring' | 'expired'; expiryDate: string; icon: React.ElementType }[];
}) => (
	<Card>
		<CardHeader className="pb-3">
			...
			{subscriptions.map((sub, i) => (
				<div key={i} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
					...
					<Badge className={
						...
						sub.status === 'expiring' ? 'bg-amber-500/20 text-amber-600' : 'bg-red-500/20 text-red-600'
					}>
						{sub.status === 'active' ? 'Active' : sub.status === 'expiring' ? 'Expiring Soon' : 'Expired'}
					</Badge>
				</div>
			))}
		</CardContent>
		...
		<CardContent>
			<div className="grid @sm:grid-cols-2 gap-3">
				{vehicles.map((vehicle, i) => (
					<div key={i} className={`flex items-center gap-3 p-3 rounded-lg ${vehicle.active ? 'bg-primary/10 border border-primary/30' : 'bg-muted/30'}`}>
						...
						<div className="flex-1">
							...
							{vehicle.active && <Badge variant="secondary" className="mt-1">Active</Badge>}
						</div>
					</div>
				))}
			</div>
...
export default function Main() {
	const profileData = {
		...
		vehicleStatus: { fuelLevel: 78, mileage: '24,567', nextService: '500 mi', batteryHealth: 95 },
		alerts: [
			{ type: 'warning' as const, title: 'Tire Pressure Low', description: 'Front left tire is 5 PSI below recommended.', action: 'View' },
			{ type: 'info' as const, title: 'Software Update', description: 'New update v2.4.1 available for your vehicle.' },
			{ type: 'success' as const, title: 'Insurance Renewed', description: 'Your policy is active until Dec 2024.' },
		],
		quickServices: [
		...
		trips: [
			{ from: 'Home', to: 'Office', date: 'Today', distance: '12.5 mi', duration: '25 min' },
			{ from: 'Office', to: 'Downtown Mall', date: 'Yesterday', distance: '8.2 mi', duration: '18 min' },
		],
		subscriptions: [
			{ name: 'Premium Nav', status: 'active' as const, expiryDate: 'Dec 2024', icon: Navigation },
			{ name: 'Roadside Assist', status: 'expiring' as const, expiryDate: 'Feb 2024', icon: Shield },
			{ name: 'CarPlay Plus', status: 'active' as const, expiryDate: 'Jun 2024', icon: Zap },
		],
		drivingScore: {
			score: 87,
			breakdown: [
		...
		vehicles: [
			{ name: 'Tesla Model 3', type: 'Electric', image: 'https://images.unsplash.com/photo-1560958089-b8a1929cea89?w=200', active: true },
			{ name: 'Honda CR-V', type: 'Hybrid', image: 'https://images.unsplash.com/photo-1519641471654-76ce0107ad1b?w=200', active: false },
		],
	};
