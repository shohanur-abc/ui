...
import { Mail, Lock, User, ArrowRight, Scissors, Sparkles, Star, CalendarDays } from 'lucide-react';
...
const SubscriptionPanel = ({
	...
	plans: Array<{ name: string; price: string; frequency: string; features: string[] }>;
}) => (
	<div className="relative hidden @lg:flex flex-col justify-center p-8 @xl:p-12 bg-gradient-to-br from-card via-background to-muted/50">
		...
		<p className="text-muted-foreground mb-8">Choose your beauty box frequency</p>
...
const TermsCheckbox = ({ label, termsHref, privacyHref }: { label: string; termsHref: string; privacyHref: string }) => (
	<div className="flex items-start gap-2">
		<Checkbox id="terms" className="mt-0.5" />
		<Label htmlFor="terms" className="text-sm font-normal cursor-pointer leading-relaxed">
			{label}{' '}
			<Link href={termsHref} className="text-primary hover:underline">Terms of Service</Link>
			{' '}and{' '}
...
export default function Main() {
	const plans = [
		...
		},
		{
			...
			price: '$79',
			frequency: '/3 months',
			features: ['6 full-size products', 'Free shipping', 'Bonus gift included'],
		},
		{
			name: 'Annual',
			price: '$299',
			...
			<div className="min-h-screen grid @lg:grid-cols-2">
				<div className="flex flex-col justify-center px-6 @sm:px-8 @xl:px-16 py-12">
					<div className="w-full max-w-md mx-auto @lg:mx-0 @lg:ml-auto">
						...
						<Title text="Start your subscription" subtitle="Get curated beauty products delivered monthly" />

						<form className="space-y-5">
							<FormField label="Full Name" type="text" placeholder="Jane Doe" icon={User} />
						...
						<div className="mt-8">
							<FooterLink text="Already subscribed?" linkText="Sign in" href="/login" />
						</div>
					</div>
				</div>
