...
const CategoryPanel = ({
	...
	categories: Array<{ icon: React.ElementType; name: string; count: string }>;
}) => (
	<div className="relative hidden @lg:flex flex-col justify-center p-8 @xl:p-12 bg-gradient-to-br from-primary/5 to-accent/5">
		<h2 className="text-2xl font-bold mb-2">Shop by Category</h2>
		<p className="text-muted-foreground mb-8">Explore our wide selection</p>
		<div className="grid grid-cols-2 gap-4">
			{categories.map((category, i) => (
				<div
					key={i}
					className="group p-4 rounded-xl bg-card/80 backdrop-blur-sm border border-border/50 hover:border-primary/50 transition-colors cursor-pointer"
				>
...
export default function Main() {
	const categories = [
		{ icon: Smartphone, name: 'Phones', count: '2,500+' },
		{ icon: Laptop, name: 'Laptops', count: '1,800+' },
		{ icon: Tv, name: 'TVs', count: '900+' },
		{ icon: Headphones, name: 'Audio', count: '3,200+' },
		{ icon: Watch, name: 'Wearables', count: '1,500+' },
		{ icon: Camera, name: 'Cameras', count: '800+' },
	];
