...
const BundleOption = ({
	...
	<div
		className={`relative rounded-2xl p-6 transition-all ${popular ? 'bg-primary text-primary-foreground ring-2 ring-primary' : 'bg-card border border-border/50 hover:border-primary/30'}`}
	>
		{popular && (
		...
		<div className="text-center mb-4">
			<div
				className={`text-5xl @md:text-6xl font-black ${popular ? '' : 'text-primary'}`}
			>
				{items}
			</div>
		...
		<div className="text-center mb-4">
			...
			<p
				className={`text-sm ${popular ? 'text-primary-foreground/80' : 'text-green-500'}`}
			>
				Save {savings}
			</p>
		</div>
