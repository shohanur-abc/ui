...
const StatCard = ({
    ...
    <div className={`p-6 @md:p-8 rounded-2xl text-center ${highlight ? "bg-primary text-primary-foreground" : "bg-card"}`}>
        <span className="text-4xl @md:text-5xl @lg:text-6xl font-black">{value}</span>
        <p className={`text-sm @md:text-base mt-2 ${highlight ? "opacity-80" : "text-muted-foreground"}`}>{label}</p>
    </div>
)
