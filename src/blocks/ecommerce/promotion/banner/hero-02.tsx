...
const ProductImage = ({
    ...
    <div className="relative aspect-square @lg:aspect-[4/5] rounded-3xl overflow-hidden bg-card">
        ...
        <Badge className="absolute top-4 right-4 shadow-lg text-lg px-4 py-1">{badge}</Badge>
    </div>
)
...
const PriceBlock = ({
    ...
    <div className="space-y-1">
        <div className="flex items-baseline gap-3">
            <span className="text-4xl @md:text-5xl font-black text-primary">{sale}</span>
            <span className="text-xl text-muted-foreground line-through">{original}</span>
        </div>
        <p className="text-sm text-green-500 font-medium">{savings}</p>
    </div>
)

const FeatureList = ({ items }: { items: { icon: React.ElementType; text: string }[] }) => (
...
const ProductDetails = ({
    ...
    <div className="space-y-6">
        ...
        <PriceBlock original={price.original} sale={price.sale} savings={price.savings} />
                ...
                <div className="relative max-w-7xl mx-auto">
                    <div className="grid @lg:grid-cols-2 gap-8 @lg:gap-12 items-center">
                        ...
                        <ProductDetails
                            ...
                            price={{ original: "$449", sale: "$299", savings: "You save $150 (33% off)" }}
                            features={[
