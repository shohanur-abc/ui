...
import { ArrowRight, Crown, Star, Zap, Gift, Percent, Truck, Award } from "lucide-react"
...
const MembershipTier = ({
    ...
    <Card className={`relative p-6 ${featured ? "bg-primary text-primary-foreground border-primary" : "bg-card"}`}>
        {featured && (
        ...
        <div className="flex items-center gap-3 mb-4">
            <div className={`size-12 rounded-xl flex items-center justify-center ${featured ? "bg-primary-foreground/20" : "bg-primary/10"}`}>
                <Icon className={`size-6 ${featured ? "text-primary-foreground" : "text-primary"}`} />
            </div>
        ...
        <Button variant={featured ? "secondary" : "default"} className="w-full gap-2" asChild>
            <Link href={cta.href}>
                {cta.label}
