...
const FeaturedCategory = ({
    ...
    <Link href={href} className="group relative overflow-hidden rounded-3xl col-span-2 row-span-2">
        <div className="aspect-square @md:aspect-[16/9] relative">
            <Image src={image} alt={title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-6 @md:p-8">
            <Badge className="bg-white/10 backdrop-blur-sm text-white border-0 mb-3">Featured</Badge>
            <h3 className="font-bold text-white text-2xl @md:text-3xl mb-2">{title}</h3>
