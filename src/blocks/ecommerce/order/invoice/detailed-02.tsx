			...
			<Table>
				...
				<TableBody>
					{entries.map((entry, index) => (
						<TableRow
							key={index}
							className={!entry.billable ? 'opacity-50' : ''}
						>
							...
							<TableCell className="text-right font-medium">
								{entry.billable
									? `${currency}${(entry.hours * entry.rate).toFixed(2)}`
									: '-'}
							</TableCell>
						</TableRow>
					))}
		...
		<CardContent className="space-y-4">
			<div className="space-y-2">
				<div className="flex justify-between text-sm">
					...
					<span>
						{currency}
						{laborSpent.toLocaleString()} / {currency}
						{laborBudget.toLocaleString()}
					</span>
				</div>
			...
			<div className="space-y-2">
				<div className="flex justify-between text-sm">
					...
					<span>
						{currency}
						{expenseSpent.toLocaleString()} / {currency}
						{expenseBudget.toLocaleString()}
					</span>
				</div>
				<Progress
					value={(expenseSpent / expenseBudget) * 100}
					className="h-2"
				/>
			</div>
			<Separator />
			...
			<div className="flex justify-between text-sm text-muted-foreground">
				...
				<span>
					{currency}
					{(laborSpent + expenseSpent).toLocaleString()}
				</span>
			</div>
			<div className="flex justify-between text-sm font-medium text-green-600">
				...
				<span>
					{currency}
					{(budgetTotal - laborSpent - expenseSpent).toLocaleString()}
				</span>
			</div>
		</CardContent>
	</Card>
