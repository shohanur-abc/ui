...
import { Bed, Calendar, Coffee, MapPin, Star, Utensils, Wifi } from "lucide-react"
...
const StayDates = ({ checkIn, checkInTime, checkOut, checkOutTime, nights }: StayDatesProps) => (
...
const ReservationSummary = ({ confirmationNumber, guestName, status }: ReservationSummaryProps) => (
  <div className="p-4 rounded-lg bg-muted/40 space-y-2">
    ...
    <p className="text-sm text-muted-foreground">Guest: <span className="text-foreground font-medium">{guestName}</span></p>
  </div>
)

const PricingDetails = ({ roomRate, nights, taxes, resortFee, total, currency, prepaid }: PricingDetailsProps) => (
  <div className="p-4 rounded-lg border space-y-3">
    <p className="font-semibold">Price Details</p>
    <div className="space-y-2 text-sm">
      <div className="flex justify-between">
        <span className="text-muted-foreground">{nights} nights × {currency}{roomRate}/night</span>
        <span>{currency}{(nights * roomRate).toFixed(2)}</span>
      </div>
      <div className="flex justify-between">
      ...
      <div className="flex justify-between">
        ...
        <span>{currency}{resortFee.toFixed(2)}</span>
      </div>
    </div>
    <Separator />
    ...
    {prepaid && (
      <Badge variant="secondary" className="w-full justify-center">Prepaid - No payment due at hotel</Badge>
    )}
  </div>
)
