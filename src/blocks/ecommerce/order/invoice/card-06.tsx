...
import { BookOpen, Calendar, Clock, DollarSign, GraduationCap, Library, User } from "lucide-react"
...
const StudentCard = ({ name, studentId, program, year, email }: StudentCardProps) => (
...
const SemesterCard = ({ semester, academicYear, enrollmentStatus, credits }: SemesterCardProps) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-base flex items-center gap-2">
...
const TuitionItemCard = ({ category, description, credits, amount, currency }: TuitionItemCardProps) => (
  <Card>
    <CardContent className="pt-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-lg bg-muted flex items-center justify-center">
            {category === "Tuition" ? <GraduationCap className="size-5" /> : 
             category === "Fees" ? <Library className="size-5" /> : 
             <BookOpen className="size-5" />}
          </div>
          <div>
            <p className="font-medium">{description}</p>
            {credits && <p className="text-sm text-muted-foreground">{credits} credits</p>}
...
const FinancialAidCard = ({ grants, loans, scholarships, totalAid, currency }: FinancialAidCardProps) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-base flex items-center gap-2">
    ...
    <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {scholarships.map((item, index) => (
            <div key={index} className="flex justify-between text-sm">
              <span>{item.name}</span>
              <span className="text-green-600">-{currency}{item.amount.toLocaleString()}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {loans.length > 0 && (
        <div className="space-y-2">
          ...
          {loans.map((item, index) => (
            <div key={index} className="flex justify-between text-sm">
              <span>{item.name}</span>
              <span className="text-blue-600">-{currency}{item.amount.toLocaleString()}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex justify-between font-semibold">
        <span>Total Aid</span>
        <span className="text-green-600">-{currency}{totalAid.toLocaleString()}</span>
      </div>
    </CardContent>
  </Card>
)

const BalanceSummaryCard = ({ totalCharges, totalAid, previousBalance, amountDue, dueDate, currency }: BalanceSummaryCardProps) => (
    ...
    <CardContent className="pt-6 space-y-4">
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span>-{currency}{totalAid.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
      ...
      <div className="space-y-1">
        ...
        <p className="text-3xl font-bold">{currency}{amountDue.toLocaleString()}</p>
      </div>
...
export default function Main() {
  ...
  const tuitionItems: TuitionItemCardProps[] = [
    { category: "Tuition", description: "Undergraduate Tuition", credits: 15, amount: 12500, currency: "$" },
    { category: "Fees", description: "Student Activity Fee", amount: 350, currency: "$" },
    { category: "Fees", description: "Technology Fee", amount: 275, currency: "$" },
    { category: "Fees", description: "Health Services Fee", amount: 450, currency: "$" },
    { category: "Housing", description: "Residence Hall - Double", amount: 4500, currency: "$" },
    { category: "Meal Plan", description: "Meal Plan - Standard", amount: 2200, currency: "$" },
  ]
  ...
  const financialAid: FinancialAidCardProps = {
    ...
    grants: [
      { name: "Federal Pell Grant", amount: 3500 },
    ],
    loans: [
      { name: "Direct Subsidized Loan", amount: 2750 },
    ],
    totalAid: 12750,
    currency: "$",
  }
      ...
      <div className="mx-auto max-w-4xl px-4 @sm:px-6 @2xl:px-8 py-8 @md:py-12">
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-sm text-muted-foreground">Charges</h3>
