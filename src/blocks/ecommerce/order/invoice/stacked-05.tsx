...
interface RefundTimelineProps {
  steps: { label: string; date: string; status: "done" | "current" | "pending" }[]
}
...
const RefundHeader = ({ title, refundNumber, originalOrder, status }: RefundHeaderProps) => {
  const statusConfig = {
    processing: { variant: "secondary" as const, icon: Clock, label: "Processing" },
    completed: { variant: "default" as const, icon: ArrowUpRight, label: "Refunded" },
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground font-mono">{refundNumber}</p>
        </div>
    ...
    {steps.map((step, index) => (
      <div key={index} className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className={step.status === "pending" ? "text-muted-foreground" : "font-medium"}>{step.label}</p>
        </div>
...
const RefundItemCard = ({ item, currency }: RefundItemCardProps) => (
  <div className="p-4 rounded-lg border space-y-2">
    ...
    <div className="flex items-center justify-between text-sm">
      <span className="text-muted-foreground">Original: {currency}{item.originalPrice.toFixed(2)}</span>
      <span className="font-semibold text-green-600">Refund: {currency}{item.refundAmount.toFixed(2)}</span>
    </div>
  </div>
)

const RefundSummary = ({ subtotal, restockingFee, refundTotal, currency, method }: RefundSummaryProps) => (
    ...
    <div className="space-y-2 text-sm">
      ...
      {restockingFee > 0 && (
        <div className="flex justify-between text-amber-600">
          ...
          <span>-{currency}{restockingFee.toFixed(2)}</span>
        </div>
      )}
    </div>
    <Separator />
...
export default function Main() {
  ...
  const items: RefundItem[] = [
    { name: "Premium Backpack", reason: "Wrong color received", originalPrice: 129.99, refundAmount: 129.99 },
    { name: "Travel Organizer Set", reason: "Damaged in shipping", originalPrice: 49.99, refundAmount: 49.99 },
  ]
