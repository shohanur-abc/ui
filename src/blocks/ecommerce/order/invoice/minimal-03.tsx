...
const SubscriptionHeader = ({ productName, planName, status }: SubscriptionHeaderProps) => (
...
const BillingCycle = ({ currentPeriodStart, currentPeriodEnd, nextBillingDate }: BillingCycleProps) => (
...
const PaymentSummary = ({ subtotal, discount, discountLabel, total, currency }: PaymentSummaryProps) => (
...
export default function Main() {
  ...
  const charges: ChargeItemProps[] = [
    { description: "Business Plan (Monthly)", amount: 49.00, currency: "$" },
    { description: "Additional Storage (50GB)", amount: 10.00, currency: "$" },
    { description: "Priority Support Add-on", amount: 15.00, currency: "$" },
  ]
      ...
      <div className="mx-auto max-w-md px-4 @sm:px-6 py-8 @md:py-12">
        <div className="bg-card border rounded-lg p-6 space-y-6">
          ...
          <div className="flex gap-3">
            <Button variant="outline" className="flex-1" size="sm">Manage Plan</Button>
            <Button className="flex-1" size="sm">Pay Now</Button>
          </div>
        </div>
      </div>
    </section>
