...
interface PaymentMethodsProps {
  title: string
  methods: { icon: React.ComponentType<{ className?: string }>; name: string; details: string }[]
}
...
const DepositHeader = ({ title, invoiceNumber, projectName, stage }: DepositHeaderProps) => (
      ...
      <div key={index} className="p-4 rounded-lg border space-y-2">
        ...
        <div className="space-y-0.5">
          {item.lines.map((line, lIndex) => (
            <p key={lIndex} className={lIndex === 0 ? "font-semibold" : "text-sm text-muted-foreground"}>
              {line}
            </p>
          ))}
        </div>
      ...
      <TableBody>
        {items.map((item, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="font-mono text-sm text-muted-foreground">{item.reference}</TableCell>
            <TableCell className="text-right">{currency}{item.amount.toLocaleString()}</TableCell>
            <TableCell className="text-right text-green-600">{currency}{item.received.toLocaleString()}</TableCell>
            <TableCell className="text-right font-medium">
              {currency}{(item.amount - item.received).toLocaleString()}
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
...
const PaymentProgress = ({ totalContract, depositsReceived, remainingBalance, currency }: PaymentProgressProps) => (
  <div className="p-4 rounded-lg bg-gradient-to-r from-primary/10 to-primary/5 space-y-4">
    ...
    <div className="grid grid-cols-3 gap-4">
      <div>
        ...
        <p className="text-xl font-bold">{currency}{totalContract.toLocaleString()}</p>
      </div>
      <div>
        <p className="text-xs text-muted-foreground">Deposits Received</p>
        <p className="text-xl font-bold text-green-600">{currency}{depositsReceived.toLocaleString()}</p>
      </div>
      <div>
        <p className="text-xs text-muted-foreground">Remaining</p>
        <p className="text-xl font-bold text-primary">{currency}{remainingBalance.toLocaleString()}</p>
      </div>
    </div>
    ...
    <p className="text-sm text-muted-foreground text-center">
      {((depositsReceived / totalContract) * 100).toFixed(0)}% of contract value received
    </p>
  </div>
)
...
export default function Main() {
  ...
  const parties = [
    {
      ...
      lines: ["CodeCraft Solutions", "500 Developer Lane", "San Jose, CA 95110", "Tax ID: 94-1234567"],
    },
    {
      label: "Client",
      lines: ["Retail Dynamics Inc.", "800 Commerce Blvd", "Chicago, IL 60601", "PO: RD-2024-156"],
    },
    {
      label: "Project Details",
      lines: ["E-commerce Platform", "Contract: SC-2024-089", "Start: Jan 2024", "Est. Completion: Jun 2024"],
    },
  ]

  const depositItems: DepositItem[] = [
    { description: "Initial Deposit (20%)", type: "Milestone", reference: "DEP-001", amount: 15000, received: 15000 },
    { description: "Phase 1 Completion (25%)", type: "Milestone", reference: "DEP-002", amount: 18750, received: 18750 },
    { description: "Phase 2 Start (25%)", type: "Milestone", reference: "DEP-003", amount: 18750, received: 0 },
  ]
  ...
  const paymentMethods = [
    { icon: CreditCard, name: "Credit Card", details: "Visa, Mastercard, Amex accepted" },
        ...
        <Card>
          ...
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="grid @lg:grid-cols-2 gap-6">
              <PaymentMethods title="Accepted Payment Methods" methods={paymentMethods} />
