...
const BreakdownSection = ({ items, currency }: BreakdownSectionProps) => (
	<div className="space-y-2">
		{items.map((item, index) => (
			...
			>
				<span className={item.type === 'total' ? '' : 'text-muted-foreground'}>
					{item.label}
				</span>
				<span>
					{item.type === 'discount' ? '-' : ''}
...
export default function Main() {
	...
	const services: ServiceItem[] = [
		{
			...
			description: 'Complete system architecture analysis and recommendations',
			quantity: 1,
			rate: 3500.0,
		},
		{
	...
	const breakdown = [
		{ label: 'Subtotal', value: 10800.0 },
		{ label: 'Volume Discount (5%)', value: 540.0, type: 'discount' as const },
	...
	const notes: NotesSectionProps = {
		title: 'Payment Notes',
		content:
			'Please include invoice number in payment reference. Wire transfers preferred for amounts over $5,000. Contact billing@techconsult.pro for questions.',
	};
