...
import { Calendar, Car, Clock, Fuel, Key, MapPin, Shield, Users } from "lucide-react"
...
const RentalDetails = ({ 
  confirmationNumber, pickupDate, pickupTime, pickupLocation, 
...
const PricingBreakdown = ({ dailyRate, days, insurance, extras, taxes, total, currency }: PricingBreakdownProps) => (
  <div className="p-4 rounded-lg border space-y-3">
    <p className="font-semibold">Pricing Breakdown</p>
    <div className="space-y-2 text-sm">
      <div className="flex justify-between">
        <span className="text-muted-foreground">{days} days × {currency}{dailyRate}/day</span>
        <span>{currency}{(days * dailyRate).toFixed(2)}</span>
      </div>
      <div className="flex justify-between">
        <span className="text-muted-foreground">Insurance</span>
        <span>{currency}{insurance.toFixed(2)}</span>
      </div>
      {extras.map((extra, index) => (
        <div key={index} className="flex justify-between">
          <span className="text-muted-foreground">{extra.name}</span>
          <span>{currency}{extra.price.toFixed(2)}</span>
        </div>
      ))}
      <div className="flex justify-between">
...
export default function Main() {
  ...
  const pricing: PricingBreakdownProps = {
    ...
    insurance: 75.00,
    extras: [
      { name: "GPS Navigation", price: 25.00 },
      { name: "Child Seat", price: 35.00 },
    ],
    taxes: 42.50,
    total: 502.50,
    currency: "$",
          ...
          <CardContent className="pt-6">
            <div className="grid @lg:grid-cols-5 gap-6">
              <div className="@lg:col-span-2 space-y-4">
                <VehicleInfo {...vehicle} />
                <RenterInfo {...renter} />
                <Button variant="outline" className="w-full">Modify Reservation</Button>
              </div>
