...
const LicenseHeader = ({ productName, version, licenseType, status }: LicenseHeaderProps) => (
  <div className="flex items-start justify-between">
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <Badge variant="secondary" className="mt-1">{licenseType}</Badge>
      </div>
    </div>
...
const LicenseKey = ({ licenseKey, activatedOn, expiresOn }: LicenseKeyProps) => (
  <div className="p-4 rounded-lg bg-muted/40 space-y-3">
    <p className="text-sm font-semibold">License Key</p>
    <p className="font-mono text-sm bg-background p-2 rounded border select-all">{licenseKey}</p>
...
const InvoiceInfo = ({ invoiceNumber, purchaseDate, amount, currency, paymentMethod }: InvoiceInfoProps) => (
  <div className="p-4 rounded-lg border space-y-3">
    ...
    <div className="flex justify-between items-center">
      ...
      <span className="text-xl font-bold text-primary">{currency}{amount.toFixed(2)}</span>
    </div>
  </div>
)
...
const SupportInfo = ({ level, expires, email }: SupportInfoProps) => (
  <div className="p-4 rounded-lg border-2 border-primary/20 bg-primary/5 space-y-2">
    ...
    <div className="text-sm space-y-1">
      <p className="text-muted-foreground">Coverage until: <span className="text-foreground font-medium">{expires}</span></p>
      <p className="text-muted-foreground">Contact: <span className="text-foreground">{email}</span></p>
    </div>
  </div>
)
