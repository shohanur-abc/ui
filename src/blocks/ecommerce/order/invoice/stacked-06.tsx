...
const QuickReceiptHeader = ({ storeName, storeLocation, receiptNumber, date, time }: QuickReceiptHeaderProps) => (
...
const PaymentBreakdown = ({ 
  subtotal, tax, taxRate, total, amountPaid, change, currency, paymentMethod 
    ...
    <div className="p-3 rounded-lg bg-muted/40 space-y-2 text-sm">
      <div className="flex justify-between">
        <span className="flex items-center gap-2">
          {paymentMethod === "Cash" ? <Banknote className="size-4" /> : <Wallet className="size-4" />}
          {paymentMethod}
        </span>
...
const LoyaltyInfo = ({ pointsEarned, totalPoints, memberName, memberId }: LoyaltyInfoProps) => (
