...
import { Building, Calendar, FileText, Home, Percent, Receipt, Wallet } from "lucide-react"
...
const TenantCard = ({ name, leaseStart, leaseEnd, tenantId }: TenantCardProps) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-base flex items-center gap-2">
...
const PaymentStatusCard = ({ statementPeriod, dueDate, status, lateFee, currency }: PaymentStatusCardProps) => (
  <Card className={status === "Past Due" ? "border-destructive" : ""}>
    <CardContent className="pt-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Badge variant={status === "Past Due" ? "destructive" : "default"}>{status}</Badge>
      </div>
      <div>
      ...
      {status === "Past Due" && lateFee > 0 && (
        <div className="p-3 rounded-lg bg-destructive/10 text-destructive text-sm">
          Late fee of {currency}{lateFee.toFixed(2)} has been applied
        </div>
      )}
    </CardContent>
  </Card>
...
const BalanceCard = ({ previousBalance, payments, currentCharges, totalDue, currency }: BalanceCardProps) => (
    ...
    <CardContent className="space-y-3">
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span>{currency}{currentCharges.toFixed(2)}</span>
        </div>
      </div>
      ...
      <Button variant="secondary" className="w-full">Pay Rent Online</Button>
    </CardContent>
  </Card>
)
...
export default function Main() {
  ...
  const rentItems = [
    { label: "Base Rent", amount: 2450.00 },
    { label: "Parking (1 Space)", amount: 150.00 },
    { label: "Pet Fee", amount: 50.00 },
    { label: "Water/Sewer", amount: 45.00 },
    { label: "Trash", amount: 25.00 },
  ]
