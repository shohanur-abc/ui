...
import { ArrowRight, Building2, DollarSign, FileText, PieChart, TrendingUp } from "lucide-react"
...
const PortfolioSummary = ({ totalValue, totalGain, gainPercentage, currency }: PortfolioSummaryProps) => (
  <div className="p-6 rounded-xl bg-gradient-to-br from-green-500/10 to-emerald-500/10 space-y-4">
    ...
    <div>
      <p className="text-3xl font-bold">{currency}{totalValue.toLocaleString()}</p>
      <div className="flex items-center gap-2 mt-1 text-sm text-green-600">
        <TrendingUp className="size-4" />
        <span>+{currency}{totalGain.toLocaleString()} ({gainPercentage}%)</span>
      </div>
    </div>
  </div>
)

const AccountInfo = ({ accountNumber, accountType, custodian, openedDate }: AccountInfoProps) => (
  ...
  <div className="p-4 rounded-lg border hover:border-primary/50 transition-colors">
    <div className="flex items-start justify-between gap-4">
      ...
      <div className="text-right">
        <p className="font-bold">{currency}{(item.shares * item.pricePerShare).toLocaleString()}</p>
        <p className="text-sm text-muted-foreground">{item.shares} shares @ {currency}{item.pricePerShare}</p>
      </div>
    </div>
  </div>
)

const TransactionDetails = ({ transactionId, date, type, status }: TransactionDetailsProps) => (
...
export default function Main() {
  ...
  const investments: InvestmentItem[] = [
    { name: "S&P 500 Index Fund", type: "ETF", shares: 50, pricePerShare: 485.00 },
    { name: "Total Bond Market", type: "Mutual Fund", shares: 200, pricePerShare: 102.50 },
    { name: "International Equity", type: "ETF", shares: 75, pricePerShare: 68.00 },
    { name: "Real Estate Investment", type: "REIT", shares: 100, pricePerShare: 45.25 },
  ]

  const fees = [
    { label: "Trading Commission", value: 0.00 },
