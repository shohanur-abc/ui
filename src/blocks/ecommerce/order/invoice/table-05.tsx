...
const BrandHeader = ({ logoSrc, logoFallback, companyName, invoiceLabel, invoiceNumber }: BrandHeaderProps) => (
...
const BillingPeriod = ({ label, period, status, statusVariant }: BillingPeriodProps) => (
...
const AccountInfo = ({ label, accountName, accountId, email }: AccountInfoProps) => (
      ...
      <TableBody>
        {items.map((item, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="text-right font-medium">{currency}{(item.unitCost * item.quantity).toFixed(2)}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
  ...
  <div className="w-full @md:w-80 ml-auto space-y-2">
    {items.map((item, index) => (
      <div key={index} className={`flex justify-between text-sm ${item.isCredit ? "text-green-600" : ""}`}>
        ...
        <span>{item.isCredit ? "-" : ""}{currency}{Math.abs(item.amount).toFixed(2)}</span>
      </div>
    ))}
    <Separator />
    <div className="flex justify-between text-lg font-bold">
      <span>{total.label}</span>
      <span>{currency}{total.amount.toFixed(2)}</span>
    </div>
  </div>
)
...
export default function Main() {
  ...
  const subscriptions: Subscription[] = [
    { name: "Enterprise Plan", description: "Unlimited users, 10TB storage", period: "Monthly", unitCost: 499.00, quantity: 1 },
    { name: "Premium Support", description: "24/7 priority support", period: "Monthly", unitCost: 149.00, quantity: 1 },
    { name: "API Requests", description: "Additional 1M requests", period: "Usage", unitCost: 50.00, quantity: 3 },
    { name: "Data Backup", description: "Daily encrypted backups", period: "Monthly", unitCost: 29.00, quantity: 1 },
  ]

  const costItems = [
    { label: "Subtotal", amount: 827.00 },
    { label: "Credits Applied", amount: 50.00, isCredit: true },
    { label: "Tax (0%)", amount: 0.00 },
  ]

  const totalData = { label: "Amount Due", amount: 777.00 }

  const noticeData: InvoiceNoticeProps = {
    title: "Payment will be processed automatically",
    message: "This invoice will be charged to your Visa ending in 4242 on February 1, 2024.",
  }
