...
const InvoiceMeta = ({ invoiceId, issueDate, dueDate, status }: InvoiceMetaProps) => {
  const statusVariant = status === "paid" ? "default" : status === "overdue" ? "destructive" : "secondary"
  return (
    <div className="text-right space-y-2">
      ...
      <div className="text-sm space-y-1">
        <p><span className="text-muted-foreground">Invoice: </span>{invoiceId}</p>
        <p><span className="text-muted-foreground">Issue Date: </span>{issueDate}</p>
        <p><span className="text-muted-foreground">Due Date: </span>{dueDate}</p>
      </div>
    </div>
  )
}

const PartyInfo = ({ title, company, addressLine1, addressLine2, taxId }: PartyInfoProps) => (
    ...
    <TableBody>
      {items.map((item) => (
        <TableRow key={item.id}>
          ...
          <TableCell className="text-right">{currency}{item.rate.toFixed(2)}</TableCell>
...
const InvoiceSummary = ({ subtotal, discount, discountLabel, shipping, tax, taxLabel, total, currency }: SummaryProps) => (
...
export default function Main() {
  ...
  const items: LineItem[] = [
    { id: "1", product: "Enterprise License", sku: "ENT-001", quantity: 5, rate: 499.00, amount: 2495.00 },
    { id: "2", product: "Premium Support", sku: "SUP-PRO", quantity: 1, rate: 1200.00, amount: 1200.00 },
    { id: "3", product: "API Access", sku: "API-UNL", quantity: 1, rate: 350.00, amount: 350.00 },
    { id: "4", product: "Training Session", sku: "TRN-4HR", quantity: 2, rate: 200.00, amount: 400.00 },
  ]
