...
import { Building2, Calendar, CreditCard, Download, FileText, Mail, MapPin, Phone, Printer, User } from "lucide-react"
...
const CompanyDetails = ({ name, address, city, phone, email, website, taxId }: CompanyDetailsProps) => (
  <div className="space-y-3">
    ...
    <p className="text-xs text-muted-foreground">Tax ID: {taxId} • {website}</p>
  </div>
)

const CustomerDetails = ({ name, company, address, city, phone, email, customerNumber }: CustomerDetailsProps) => (
  <Card>
...
const InvoiceMetadata = ({ invoiceNumber, invoiceDate, dueDate, poNumber, terms, status }: InvoiceMetadataProps) => (
  <Card>
    <CardHeader className="pb-3">
...
const LineItemsTable = ({ items, currency }: { items: LineItemProps[]; currency: string }) => (
  <Card>
...
const InvoiceTotals = ({ subtotal, totalDiscount, taxableAmount, taxAmount, shipping, total, amountPaid, balanceDue, currency }: InvoiceTotalsProps) => (
  <Card>
    <CardContent className="pt-6 space-y-3">
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span>{currency}{taxableAmount.toFixed(2)}</span>
        </div>
        <div className="flex justify-between">
...
const PaymentInfo = ({ bankName, accountName, accountNumber, routingNumber, swiftCode }: PaymentInfoProps) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
...
export default function Main() {
  ...
  const items: LineItemProps[] = [
    { sku: "SVC-001", description: "Enterprise Software License - Annual", quantity: 25, unitPrice: 1200.00, discount: 10, taxRate: 8.5, total: 27000.00 },
    { sku: "SVC-002", description: "Implementation Services", quantity: 80, unitPrice: 175.00, discount: 0, taxRate: 8.5, total: 14000.00 },
    { sku: "SVC-003", description: "Training Sessions (Per Day)", quantity: 5, unitPrice: 2500.00, discount: 5, taxRate: 8.5, total: 11875.00 },
    { sku: "HW-101", description: "Server Hardware - Enterprise Grade", quantity: 2, unitPrice: 8500.00, discount: 0, taxRate: 8.5, total: 17000.00 },
  ]

  const totals: InvoiceTotalsProps = {
    subtotal: 69875.00,
    totalDiscount: 5837.50,
    taxableAmount: 64037.50,
    taxAmount: 5443.19,
    shipping: 450.00,
    total: 69930.69,
    amountPaid: 20000.00,
