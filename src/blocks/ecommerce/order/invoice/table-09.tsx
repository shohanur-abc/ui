...
const RecurringHeader = ({ title, subtitle, cycleLabel, cycleNumber }: RecurringHeaderProps) => (
...
const SubscriberInfo = ({ label, name, email, customerId, since }: SubscriberInfoProps) => (
  <div className="p-4 rounded-lg border space-y-3">
    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-4 pt-2 text-sm">
        <span className="text-muted-foreground">ID: <span className="font-mono">{customerId}</span></span>
      ...
      <TableBody>
        {items.map((item, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="text-muted-foreground">{item.nextBilling}</TableCell>
            <TableCell className="text-right font-medium">{currency}{item.amount.toFixed(2)}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
const AutoPayInfo = ({ enabled, method, last4, nextCharge }: AutoPayInfoProps) => (
  <div className={`p-4 rounded-lg ${enabled ? "bg-green-500/10 border border-green-500/20" : "bg-muted/40 border"}`}>
...
const RecurringSummary = ({ items, total, currency }: RecurringSummaryProps) => (
  <div className="p-4 rounded-lg bg-muted/30 space-y-2">
    {items.map((item, index) => (
      <div key={index} className={`flex justify-between text-sm ${item.isCredit ? "text-green-600" : ""}`}>
        <span className="text-muted-foreground">{item.label}</span>
        <span>{item.isCredit ? "-" : ""}{currency}{Math.abs(item.value).toFixed(2)}</span>
      </div>
    ))}
    <Separator />
    <div className="flex justify-between font-bold text-lg">
      <span>{total.label}</span>
      <span className="text-primary">{currency}{total.value.toFixed(2)}</span>
    </div>
  </div>
)
...
export default function Main() {
  ...
  const recurringItems: RecurringItem[] = [
    { service: "Cloud Hosting Pro", cycle: "Monthly", startDate: "Feb 1, 2022", nextBilling: "Feb 1, 2024", amount: 299.00 },
    { service: "CDN Bandwidth (1TB)", cycle: "Monthly", startDate: "Jun 15, 2022", nextBilling: "Feb 1, 2024", amount: 79.00 },
    { service: "SSL Certificate", cycle: "Yearly", startDate: "Feb 1, 2023", nextBilling: "Feb 1, 2025", amount: 149.00 },
    { service: "Email Service (100 users)", cycle: "Monthly", startDate: "Sep 1, 2022", nextBilling: "Feb 1, 2024", amount: 50.00 },
  ]

  const billingHistory = [
    { date: "Jan 1, 2024", amount: 428.00, status: "Paid" },
    { date: "Dec 1, 2023", amount: 428.00, status: "Paid" },
    { date: "Nov 1, 2023", amount: 428.00, status: "Paid" },
  ]
  ...
  const summaryItems = [
    { label: "Recurring Services", value: 577.00 },
          ...
          <CardContent className="space-y-6 pt-6">
            ...
            <div className="grid @lg:grid-cols-2 gap-6">
              <div className="space-y-4">
                <BillingHistory title="Recent Payments" items={billingHistory} currency="$" />
                <AutoPayInfo {...autoPayData} />
              </div>
              <div className="space-y-4">
                <RecurringSummary items={summaryItems} total={totalData} currency="$" />
                <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 text-amber-700 dark:text-amber-400">
                  <AlertCircle className="size-4 mt-0.5 shrink-0" />
                  <p className="text-sm">Your subscription will auto-renew. Cancel anytime from your account settings.</p>
                </div>
              </div>
            </div>
          </CardContent>
