...
import { Activity, Calendar, Check, Clock, Dumbbell, Heart, Sparkles, Trophy, User, Zap } from "lucide-react"
...
const TrainerCard = ({ name, certifications, specialties, rating, totalClients }: TrainerCardProps) => (
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">• {totalClients} clients</span>
          </div>
        </div>
      </div>
...
const ClientCard = ({ name, email, memberSince, fitnessGoal }: ClientCardProps) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-base flex items-center gap-2">
...
const SessionPackageCard = ({ name, sessionsTotal, sessionsUsed, sessionDuration, pricePerSession, totalPrice, validUntil, currency }: SessionPackageCardProps) => (
  <Card className="border-primary">
    ...
    <CardContent className="space-y-4">
      <div className="flex items-center justify-center py-4">
        <div className="relative size-32">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-muted-foreground">of {sessionsTotal} used</span>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4 text-sm">
      ...
      <div className="flex justify-between font-semibold">
        <span>{sessionsTotal} × {currency}{pricePerSession}</span>
...
const PaymentCard = ({ invoiceNumber, invoiceDate, packagePrice, discount, discountReason, total, paymentStatus, paymentMethod, currency }: PaymentCardProps) => (
    ...
    <CardContent className="pt-6 space-y-4">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <span>{currency}{packagePrice.toLocaleString()}</span>
        </div>
        {discount > 0 && (
