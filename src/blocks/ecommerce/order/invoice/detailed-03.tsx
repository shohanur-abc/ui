...
import { Anchor, Box, Calendar, FileText, Globe, MapPin, Package, Ship, Truck, Weight } from "lucide-react"
...
const ShipmentHeader = ({ billOfLadingNumber, bookingNumber, vesselName, voyageNumber, shipmentDate, status }: ShipmentHeaderProps) => (
    ...
    <CardContent className="pt-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Commercial Invoice & Shipping Document</p>
          </div>
        </div>
        <Badge variant="default" className="text-sm">{status}</Badge>
      </div>
      <div className="grid grid-cols-2 @md:grid-cols-4 gap-4 mt-6 text-sm">
        ...
        <div>
          ...
          <p className="font-medium">{vesselName} / {voyageNumber}</p>
        </div>
        <div>
...
const PartyInfo = ({ role, name, address, country, contactPerson, phone, email }: PartyInfoProps) => (
  <Card>
...
const RouteInfo = ({ portOfLoading, countryOfOrigin, portOfDischarge, countryOfDestination, finalDestination, estimatedDeparture, estimatedArrival }: RouteInfoProps) => (
  <Card>
    <CardHeader className="pb-3">
    ...
    <CardContent className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground">{countryOfDestination}</p>
        ...
        <TableBody>
          {containers.map((container, index) => (
            <TableRow key={index}>
              ...
              <TableCell className="font-mono">{container.sealNumber}</TableCell>
              <TableCell><Badge variant="outline">{container.type}</Badge></TableCell>
              <TableCell>{container.size}</TableCell>
              <TableCell className="text-right">{container.weight.toLocaleString()}</TableCell>
...
const CargoTable = ({ items, currency }: { items: CargoItemProps[]; currency: string }) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
        ...
        <TableBody>
          {items.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell className="text-right">{item.grossWeight.toLocaleString()}</TableCell>
              <TableCell className="text-right">{item.netWeight.toLocaleString()}</TableCell>
              <TableCell className="text-right font-medium">{currency}{item.value.toLocaleString()}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
...
const ChargesBreakdown = ({ oceanFreight, documentationFee, terminalHandling, customsClearance, insurance, totalCharges, currency }: ChargesBreakdownProps) => (
  <Card>
    <CardHeader className="pb-3">
      <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
    ...
    <CardContent className="space-y-3">
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <span>{currency}{oceanFreight.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span>Documentation Fee</span>
          <span>{currency}{documentationFee.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span>Terminal Handling (Origin)</span>
          <span>{currency}{terminalHandling.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span>Customs Clearance</span>
          <span>{currency}{customsClearance.toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span>Cargo Insurance</span>
          <span>{currency}{insurance.toLocaleString()}</span>
        </div>
      </div>
      <Separator />
      <div className="flex justify-between font-bold text-lg">
        <span>Total Charges</span>
        <span className="text-primary">{currency}{totalCharges.toLocaleString()}</span>
      </div>
    </CardContent>
  </Card>
)
...
export default function Main() {
  ...
  const containers: ContainerProps[] = [
    { containerNumber: "MSCU1234567", sealNumber: "SL789456", type: "Dry", size: "40' HC", weight: 24500, volume: 76 },
    { containerNumber: "MSCU7654321", sealNumber: "SL789457", type: "Dry", size: "40' HC", weight: 22800, volume: 76 },
  ]

  const cargo: CargoItemProps[] = [
    { hsCode: "8471.30", description: "Laptop Computers - Model XL500", quantity: 500, unit: "Units", grossWeight: 3500, netWeight: 3250, value: 375000 },
    { hsCode: "8471.60", description: "Computer Monitors - 27 inch LED", quantity: 1000, unit: "Units", grossWeight: 12000, netWeight: 11500, value: 225000 },
    { hsCode: "8471.70", description: "External Storage Devices - 2TB", quantity: 2500, unit: "Units", grossWeight: 875, netWeight: 750, value: 125000 },
  ]
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold flex items-center gap-1">
                      47,300 <span className="text-sm font-normal text-muted-foreground">kg</span>
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Badge variant="secondary" className="text-lg">CIF</Badge>
                  </div>
                </div>
