				...
				<TableBody>
					{items.map((item, index) => (
						<TableRow key={index}>
							...
							<TableCell className="text-center">
								{item.warranty && (
									<Shield className="size-4 text-green-500 mx-auto" />
								)}
							</TableCell>
							<TableCell className="text-right font-medium">
								{currency}
...
export default function Main() {
	...
	const inspection: InspectionItemProps[] = [
		...
		},
		{
			...
			status: 'Fair',
			notes: '5mm remaining',
			recommendation: 'Next Service',
		},
		{
			system: 'Tires',
			component: 'Front Tires',
			status: 'Good',
			notes: '6/32 tread depth',
			recommendation: '',
		},
		{
			system: 'Tires',
	...
	const labor: LaborItemProps[] = [
		...
		{
			code: 'FL-205',
			description: 'Brake Fluid Flush Service',
			technicianId: 'T-042',
			hours: 0.5,
			rate: 145,
			total: 72.5,
	...
	const parts: PartItemProps[] = [
		...
		{
			partNumber: '45251-TBA-A01',
			description: 'Front Brake Rotor - Right',
			quantity: 1,
			unitPrice: 124.99,
			warranty: false,
			total: 124.99,
		...
		{
			partNumber: '45251-TBA-A02',
			description: 'Front Brake Rotor - Left',
			quantity: 1,
			unitPrice: 124.99,
			warranty: false,
			total: 124.99,
		...
		{
			partNumber: '08203-999-A25',
			description: 'DOT 3 Brake Fluid - 1L',
			quantity: 2,
			unitPrice: 18.99,
			warranty: false,
			total: 37.98,
				...
				<div className="space-y-6">
					...
					<div className="grid @md:grid-cols-3 gap-4">
						<div className="@md:col-span-2 p-4 rounded-lg border bg-muted/30 text-sm text-muted-foreground">
							...
							<p>
								Front brake pads were worn below minimum specification. Rotors
								showed signs of scoring and were replaced. Brake fluid was dark
								and contaminated - performed full system flush. All work
								completed per manufacturer specifications. Test drove vehicle -
