...
const AssetAllocation = ({
	...
	allocations: AssetAllocationProps[];
}) => (
	<Card>
		<CardHeader className="pb-3">
			<CardTitle className="text-base flex items-center gap-2">
				...
				<TableBody>
					{holdings.map((holding, index) => (
						<TableRow key={index}>
							...
							<TableCell
								className={`text-right font-medium ${holding.unrealizedGain >= 0 ? 'text-green-600' : 'text-red-500'}`}
							>
							...
							<TableCell className="text-right">
								<Badge
									variant={holding.percentGain >= 0 ? 'default' : 'destructive'}
									className="gap-1"
								>
				...
				<TableBody>
					{transactions.map((tx, index) => (
						<TableRow key={index}>
							...
							<TableCell className="text-right">
								{tx.price ? `${currency}${tx.price.toFixed(2)}` : '-'}
							</TableCell>
							<TableCell
								className={`text-right font-medium ${tx.amount >= 0 ? '' : 'text-red-500'}`}
							>
								{tx.amount >= 0 ? '' : '-'}
								{currency}
					...
					<div className="grid @md:grid-cols-3 gap-4">
						<div className="@md:col-span-2 p-4 rounded-lg border bg-muted/30 text-sm">
							...
							<p className="text-muted-foreground text-xs">
								...
								values fluctuate so that when redeemed, they may be worth more
								or less than their original cost. This statement is provided for
								informational purposes only and should not be considered tax or
								legal advice. Please consult your tax advisor regarding your
								specific situation. All fees are charged in accordance with the
								advisory agreement.
							</p>
						</div>
						<FeeSummary {...fees} />
