...
const ProfileHeader = ({ avatarSrc, avatarFallback, name, role, email, phone }: ProfileHeaderProps) => (
...
const LineItemRow = ({ item, currency }: LineItemRowProps) => (
  <div className="p-4 rounded-lg border space-y-1">
    <div className="flex items-start justify-between gap-4">
      ...
      <p className="font-semibold whitespace-nowrap">{currency}{item.amount.toFixed(2)}</p>
    </div>
  </div>
)

const TotalBlock = ({ subtotal, discount, tax, total, currency }: TotalBlockProps) => (
...
const PaymentConfirmation = ({ message, date, method }: PaymentConfirmationProps) => (
...
export default function Main() {
  ...
  const items: LineItem[] = [
    { description: "Brand Identity Package", details: "Logo, color palette, typography", amount: 2500.00 },
    { description: "Business Card Design", details: "Front and back, print-ready", amount: 350.00 },
    { description: "Social Media Kit", details: "Templates for 5 platforms", amount: 800.00 },
  ]
