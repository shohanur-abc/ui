...
const CompanyInfo = ({ name, tagline, address, phone, email }: CompanyInfoProps) => (
...
const InvoiceMeta = ({ invoiceNumber, date, dueDate, terms, status }: InvoiceMetaProps) => (
...
const ItemsSection = ({ items, currency }: ItemsSectionProps) => (
  <div className="space-y-3">
    ...
    {items.map((item, index) => (
      <div key={index} className="grid grid-cols-12 gap-4 text-sm py-2 border-b border-dashed last:border-0">
        ...
        <div className="col-span-2 text-right text-muted-foreground">{currency}{item.rate.toFixed(2)}</div>
        <div className="col-span-2 text-right font-medium">{currency}{(item.quantity * item.rate).toFixed(2)}</div>
      </div>
    ))}
  </div>
)

const TotalsSection = ({ subtotal, tax, taxLabel, total, currency }: TotalsSectionProps) => (
...
export default function Main() {
  ...
  const items: LineItem[] = [
    { description: "Brand Strategy & Research", quantity: 1, rate: 2500.00 },
    { description: "Logo Design (3 concepts)", quantity: 1, rate: 1800.00 },
    { description: "Website Design", quantity: 1, rate: 4500.00 },
    { description: "Social Media Templates", quantity: 10, rate: 150.00 },
  ]
