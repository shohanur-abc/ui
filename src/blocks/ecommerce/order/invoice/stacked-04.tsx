...
interface TotalCardProps {
  rows: { label: string; value: number; isDiscount?: boolean; isBold?: boolean }[]
  currency: string
}
  ...
  <div className="flex flex-col @sm:flex-row gap-2">
    {items.map((item, index) => (
      <div key={index} className="flex items-center gap-2 flex-1">
        <div className={`size-3 rounded-full ${item.completed ? "bg-green-500" : "bg-muted"}`} />
        <div className="text-sm">
          <p className={item.completed ? "font-medium" : "text-muted-foreground"}>{item.label}</p>
...
const OrderSummaryHeader = ({ orderNumber, date, status }: OrderSummaryHeaderProps) => (
...
const TotalCard = ({ rows, currency }: TotalCardProps) => (
  <div className="p-4 rounded-lg border space-y-2">
    {rows.map((row, index) => (
      ...
      >
        <span className={row.isBold ? "" : "text-muted-foreground"}>{row.label}</span>
...
const ShippingInfo = ({ method, address, estimatedDelivery }: ShippingInfoProps) => (
  <div className="p-4 rounded-lg border space-y-3">
    <div className="flex items-center gap-2">
...
export default function Main() {
  ...
  const items: OrderItem[] = [
    { name: "Wireless Earbuds Pro", variant: "Black", quantity: 1, price: 179.99 },
