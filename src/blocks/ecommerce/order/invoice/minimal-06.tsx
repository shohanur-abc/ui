...
const RefundHeader = ({ refundId, originalOrderId, refundDate, status }: RefundHeaderProps) => (
  <div className="space-y-4">
    ...
    <div className="flex items-center justify-between text-sm">
      <span className="text-muted-foreground">Original Order: {originalOrderId}</span>
...
const RefundItem = ({ name, reason, quantity, refundAmount, currency }: RefundItemProps) => (
  <div className="flex justify-between py-3">
    <div>
      <p className="font-medium">{name}</p>
      <p className="text-sm text-muted-foreground">{reason} × {quantity}</p>
    </div>
    <p className="font-medium text-green-600">+{currency}{refundAmount.toFixed(2)}</p>
  </div>
)

const RefundSummary = ({ itemsRefund, shippingRefund, taxRefund, totalRefund, currency, refundMethod }: RefundSummaryProps) => (
  <div className="space-y-3">
    <div className="space-y-2 text-sm">
      <div className="flex justify-between">
        <span className="text-muted-foreground">Items Refund</span>
        <span className="text-green-600">+{currency}{itemsRefund.toFixed(2)}</span>
      </div>
      {shippingRefund > 0 && (
        <div className="flex justify-between">
          <span className="text-muted-foreground">Shipping Refund</span>
          <span className="text-green-600">+{currency}{shippingRefund.toFixed(2)}</span>
        </div>
      )}
      <div className="flex justify-between">
        <span className="text-muted-foreground">Tax Refund</span>
        <span className="text-green-600">+{currency}{taxRefund.toFixed(2)}</span>
      </div>
    </div>
    <Separator />
    <div className="flex justify-between font-bold text-lg">
      <span>Total Refund</span>
      <span className="text-green-600">+{currency}{totalRefund.toFixed(2)}</span>
    </div>
...
export default function Main() {
  ...
  const items: RefundItemProps[] = [
    { name: "Wireless Headphones", reason: "Defective item", quantity: 1, refundAmount: 149.00, currency: "$" },
  ]
    ...
    <section className="@container">
      <div className="mx-auto max-w-md px-4 @sm:px-6 py-8 @md:py-12">
        <div className="bg-card border rounded-lg p-6 space-y-6">
          ...
          <Separator />
          <div>
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Refunded Items</p>
