    ...
    <TableBody>
      {items.map((item, index) => (
        <TableRow key={index}>
          ...
          <TableCell className="text-right font-medium">{currency}{(item.quantity * item.price).toFixed(2)}</TableCell>
        </TableRow>
      ))}
    </TableBody>
  </Table>
...
const BreakdownRow = ({ label, value, isBold, isLarge, isHighlight }: BreakdownRowProps) => (
  <div className={`flex justify-between ${isLarge ? "text-lg" : "text-sm"} ${isBold ? "font-semibold" : ""} ${isHighlight ? "text-green-600" : ""}`}>
    <span className={isBold ? "" : "text-muted-foreground"}>{label}</span>
    <span>{value}</span>
  </div>
)

const PaymentMethod = ({ method, last4, transactionId, paidAt }: PaymentMethodProps) => (
...
export default function Main() {
  ...
  const seller = {
    heading: "From",
    lines: ["ModernShop Inc.", "500 Commerce Way", "New York, NY 10001", "billing@modernshop.com"],
  }

  const buyer = {
    heading: "Bill To",
    lines: ["Jennifer Adams", "Creative Agency LLC", "123 Design Street", "Chicago, IL 60601"],
  }

  const items: OrderItem[] = [
    { name: "Premium Wireless Headphones", variant: "Color: Midnight Black", sku: "WH-PRO-001", quantity: 2, price: 299.99 },
    { name: "Leather Laptop Sleeve", variant: "Size: 15 inch", sku: "LS-LTH-015", quantity: 1, price: 89.99 },
    { name: "USB-C Hub Pro", variant: "7-in-1 Adapter", sku: "USB-HUB-7", quantity: 3, price: 59.99 },
  ]
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid @md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 p-4 rounded-lg bg-muted/30">
                ...
                <BreakdownRow label="Total Paid" value="$866.52" isBold isLarge />
              </div>
            </div>
