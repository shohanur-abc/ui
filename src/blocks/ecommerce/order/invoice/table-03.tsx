...
const PaymentSummary = ({ rows, currency }: PaymentSummaryProps) => (
	<div className="rounded-lg bg-muted/30 p-4 space-y-2">
		{rows.map((row, index) => (
			...
			>
				<span className={row.isTotal ? '' : 'text-muted-foreground'}>
					{row.label}
				</span>
				<span>
					{row.isDiscount ? '-' : ''}
...
export default function Main() {
	...
	const paymentRows = [
		{ label: 'Subtotal', value: 8080.0 },
		{ label: 'Early Payment Discount', value: 404.0, isDiscount: true },
