...
const DonationHeader = ({ organizationName, campaignName, donationId, date }: DonationHeaderProps) => (
...
const DonorInfo = ({ name, email, isAnonymous, isRecurring }: DonorInfoProps) => (
...
const DonationAmount = ({ amount, processingFee, netAmount, currency, isRecurring, frequency }: DonationAmountProps) => (
  <div className="p-4 rounded-lg bg-gradient-to-br from-pink-500/10 to-red-500/10 border border-pink-500/20 space-y-3">
    <div className="text-center">
      <p className="text-3xl font-bold text-pink-600">{currency}{amount.toFixed(2)}</p>
      {isRecurring && <p className="text-sm text-muted-foreground">{frequency}</p>}
    </div>
    <Separator />
    <div className="space-y-1 text-sm">
...
export default function Main() {
  ...
  const thankYou: ThankYouMessageProps = {
    message: "Your generous contribution helps us provide education to underprivileged children. Every dollar makes a difference in shaping young minds.",
