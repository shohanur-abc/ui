...
const CounterStat = ({ value, label, description }: CounterStatProps) => (
	<div className="group relative border-l border-border pl-6 first:border-l-0 first:pl-0">
		...
		<p className="text-4xl font-bold tracking-tighter @sm:text-5xl @lg:text-6xl">{value}</p>
		<p className="mt-2 text-sm font-semibold uppercase tracking-wider text-primary">{label}</p>
...
export default function Main() {
	const stats: CounterStatProps[] = [
		{ value: '$847M', label: 'GMV Processed', description: 'Gross merchandise value this year' },
		{ value: '12M+', label: 'Orders Fulfilled', description: 'Successfully delivered worldwide' },
		{ value: '4.9/5', label: 'Customer Rating', description: 'Based on 500K+ reviews' },
