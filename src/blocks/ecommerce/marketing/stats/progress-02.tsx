...
const ProgressStat = ({
	...
	return (
		<div className="group flex gap-4 p-4 transition-colors hover:bg-secondary/30">
			<div
				className={`mt-0.5 rounded-full p-1 ${status === 'complete' ? 'text-accent' : 'text-muted-foreground'}`}
			>
	...
	const stats: ProgressStatProps[] = [
		{
			...
			description: 'Target: $2.5M',
			value: 100,
			status: 'complete',
		},
		{
			label: 'Q2 Revenue Target',
			description: 'Target: $3.2M',
			value: 78,
			status: 'in-progress',
		},
		{
			label: 'Customer Acquisition',
			description: 'Target: 50K new customers',
			value: 64,
			status: 'in-progress',
		},
		{
