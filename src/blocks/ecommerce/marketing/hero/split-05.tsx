...
const TrendingBadge = ({ icon: Icon, text }: { icon: React.ElementType; text: string }) => (
...
const Title = ({ text }: { text: string }) => (
    <h1 className="text-3xl @sm:text-4xl @lg:text-5xl font-bold tracking-tight leading-tight">{text}</h1>
)
...
const Reviews = ({ rating, count, avatars }: { rating: number; count: string; avatars: { src: string; fallback: string }[] }) => (
...
const ProductShowcase = ({ images, badge }: { images: { src: string; alt: string }[]; badge?: string }) => (
            ...
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24">
                <div className="grid @lg:grid-cols-2 gap-10 @xl:gap-16 items-center">
                    <div className="space-y-6 @md:space-y-8">
                        ...
                        />
                        <CTA items={[
                            { label: "Shop Collection", href: "/collection", icon: ShoppingBag },
                            { label: "View Lookbook", href: "/lookbook", variant: "outline", icon: ArrowRight }
                        ]} />
                    </div>
                    <ProductShowcase 
                        images={[
                            { src: "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=600&h=800&fit=crop", alt: "Featured look" },
                            { src: "https://images.unsplash.com/photo-1496747611176-843222e1e57c?w=400&h=300&fit=crop", alt: "Accessory" },
                            { src: "https://images.unsplash.com/photo-1509631179647-0177331693ae?w=400&h=300&fit=crop", alt: "Detail shot" }
                        ]}
                        badge="New Season"
                    />
                </div>
