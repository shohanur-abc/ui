...
const ComparisonTable = ({
	...
	<div className="grid @md:grid-cols-2 gap-6 max-w-4xl mx-auto">
		{products.map((product, i) => (
			<div
				key={i}
				className={`rounded-2xl border ${product.highlighted ? 'border-primary bg-primary/5' : 'bg-card'} overflow-hidden`}
			>
				{product.highlighted && (
					...
					<div className="text-center">
						...
						<div className="flex justify-center mt-2">
							{Array.from({ length: 5 }).map((_, j) => (
								<Star
									key={j}
									className={`size-4 ${j < product.rating ? 'fill-primary text-primary' : 'text-muted'}`}
								/>
							))}
						</div>
					</div>
