...
const ReferralStats = ({ stats }: { stats: { value: string; label: string }[] }) => (
...
const HowItWorks = ({ steps }: { steps: { icon: React.ElementType; title: string; description: string }[] }) => (
...
const RecentReferrals = ({ users }: { users: { avatar: string; name: string; earned: string }[] }) => (
    <div className="max-w-md mx-auto mt-8">
        <p className="text-sm text-muted-foreground text-center mb-4">Recent referrals:</p>
        <div className="space-y-2">
            {users.map((user, i) => (
                <div key={i} className="flex items-center gap-3 p-2 rounded-lg border bg-card">
                    ...
                    <Badge variant="secondary" className="text-xs">{user.earned}</Badge>
                </div>
            ))}
        </div>
    </div>
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 space-y-12">
                ...
                <HowItWorks steps={[
                    { icon: Users, title: "Share Your Link", description: "Send your unique referral link to friends" },
                    { icon: Sparkles, title: "Friends Shop", description: "They get $20 off their first order" },
                    { icon: Zap, title: "You Earn", description: "Get $20 credit when they purchase" }
                ]} />
                <CTA label="Get My Referral Link" href="/referral" />
                <RecentReferrals users={[
                    { avatar: "https://i.pravatar.cc/150?img=1", name: "Sarah just earned", earned: "+$20" },
                    { avatar: "https://i.pravatar.cc/150?img=2", name: "Mike just earned", earned: "+$20" },
                    { avatar: "https://i.pravatar.cc/150?img=3", name: "Emma just earned", earned: "+$20" }
                ]} />
            </div>
        </section>
    )
