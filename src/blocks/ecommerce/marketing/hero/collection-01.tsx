...
const CollectionItems = ({ items }: { items: { image: string; count: string }[] }) => (
    <div className="flex -space-x-4">
        {items.map((item, i) => (
            <div key={i} className="relative size-16 rounded-full overflow-hidden border-4 border-background">
...
const CollectionGrid = ({ collections }: { collections: { image: string; name: string; items: string }[] }) => (
    <div className="grid grid-cols-2 gap-4">
        {collections.map((c, i) => (
            <Link key={i} href={`/collection/${c.name.toLowerCase().replace(' ', '-')}`} className={`group relative rounded-2xl overflow-hidden ${i === 0 ? "col-span-2 aspect-[2/1]" : "aspect-square"}`}>
            ...
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24">
                <div className="grid @lg:grid-cols-2 gap-12 @xl:gap-20 items-center">
                    <div className="space-y-8">
                        ...
                        <CTA items={[
                            { label: "Browse Collections", href: "/collections", icon: Layers },
                            { label: "Shop All", href: "/shop", variant: "outline", icon: ShoppingBag }
                        ]} />
                    </div>
                    <CollectionGrid collections={[
                        { image: "https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800&h=400&fit=crop", name: "Summer Essentials", items: "42 items" },
                        { image: "https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=400&h=400&fit=crop", name: "Office Wear", items: "38 items" },
                        { image: "https://images.unsplash.com/photo-1445205170230-053b83016050?w=400&h=400&fit=crop", name: "Weekend Casual", items: "56 items" }
                    ]} />
                </div>
            </div>
        </section>
