...
import { ArrowRight, GraduationCap, Users, Sparkles, Calendar } from "lucide-react"
...
const Benefits = ({ items }: { items: { icon: React.ElementType; title: string; description: string }[] }) => (
...
const StudentCard = ({ discount, code }: { discount: string; code: string }) => (
    <div className="relative">
        <div className="relative rounded-3xl overflow-hidden border bg-gradient-to-br from-primary/10 via-accent/10 to-primary/10 p-8">
            ...
            <div className="relative z-10 text-center space-y-6">
                ...
                <div>
                    <p className="text-6xl @md:text-7xl font-bold text-primary">{discount}</p>
                ...
                <div className="p-4 rounded-xl border bg-background/80 backdrop-blur">
                    <p className="text-sm text-muted-foreground mb-1">Use code at checkout:</p>
                    <p className="text-2xl font-bold font-mono">{code}</p>
                </div>
                <p className="text-xs text-muted-foreground">Valid student ID required. Terms apply.</p>
            </div>
        </div>
    </div>
)
            ...
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24">
                <div className="grid @lg:grid-cols-2 gap-12 @xl:gap-20 items-center">
                    <div className="space-y-8">
                        ...
                        <Benefits items={[
                            { icon: Sparkles, title: "Exclusive Deals", description: "Access student-only promotions" },
                            { icon: Users, title: "Easy Verification", description: "Quick one-time verification" },
                            { icon: Calendar, title: "Year-Round Savings", description: "Discount valid all year" }
                        ]} />
                        <CTA items={[
                            { label: "Verify Student Status", href: "/student-verify", icon: GraduationCap },
                            { label: "Learn More", href: "/student-program", variant: "outline", icon: ArrowRight }
                        ]} />
                    </div>
