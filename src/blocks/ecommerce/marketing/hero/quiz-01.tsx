...
const StyleSuggestions = ({
	...
	items: { image: string; style: string }[];
}) => (
	<div className="flex justify-center gap-4 mt-12 overflow-hidden">
		{items.map((item, i) => (
			<div
				key={i}
				className="relative size-24 @md:size-32 rounded-full overflow-hidden border-4 border-background shadow-lg"
			>
				<Image
					src={item.image}
					alt={item.style}
					...
					questions={[
						{
							...
							options: ['Casual', 'Formal', 'Sporty', 'Bohemian', 'Minimalist'],
						},
						{
