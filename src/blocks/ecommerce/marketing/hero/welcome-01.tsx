...
const Description = ({ text }: { text: string }) => (
    <p className="text-lg @md:text-xl text-muted-foreground text-center max-w-3xl mx-auto">{text}</p>
)

const TrustIndicators = ({ items }: { items: { icon: React.ElementType; text: string }[] }) => (
...
const FeaturedImages = ({ images }: { images: { src: string; alt: string }[] }) => (
    <div className="flex justify-center items-center gap-4 @md:gap-6">
        {images.map((img, i) => (
            <div 
                key={i} 
                className={`relative rounded-2xl overflow-hidden shadow-2xl ${
                    i === 1 ? "size-48 @md:size-64 z-10" : "size-32 @md:size-48 opacity-80"
                }`}
            >
        ...
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-32 space-y-12">
                ...
                <FeaturedImages images={[
                    { src: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&h=400&fit=crop", alt: "Featured 1" },
                    { src: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=500&h=500&fit=crop", alt: "Featured 2" },
                    { src: "https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=400&h=400&fit=crop", alt: "Featured 3" }
                ]} />
                <CTA items={[
                    { label: "Shop Now", href: "/shop", icon: ArrowRight },
                    { label: "Explore Collections", href: "/collections", variant: "outline" }
                ]} />
