...
const RatingStats = ({
	...
	stats: { rating: string; reviews: string; recommended: string };
}) => (
			...
			<div key={i} className="p-6 rounded-2xl border bg-card">
				<div className="flex gap-0.5 mb-4">
					{Array.from({ length: 5 }).map((_, j) => (
						<Star
							key={j}
							className={`size-4 ${j < t.rating ? 'fill-primary text-primary' : 'text-muted'}`}
						/>
					))}
				</div>
	...
	return (
		<section className="@container relative overflow-hidden" data-theme="neon">
			<div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 space-y-12">
				...
				<RatingStats
					stats={{ rating: '4.9', reviews: '25,847', recommended: '98%' }}
				/>
				<TestimonialCards
					testimonials={[
						{
							quote:
								"Best online shopping experience I've ever had! Quality products and fast shipping.",
							author: 'Sarah M.',
						...
						{
							quote:
								'The customer service is outstanding. They helped me find the perfect gift!',
							author: 'Michael R.',
						...
						{
							quote:
								'Love the quality and the easy returns policy. Will definitely shop here again.',
							author: 'Emma L.',
