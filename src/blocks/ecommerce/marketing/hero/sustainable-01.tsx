	...
	<div className="flex flex-wrap justify-center gap-4">
		{items.map(({ label, href, icon: Icon, variant = 'default' }, i) => (
			<Button
				...
				size="lg"
				variant={variant}
				className={`gap-2 ${variant === 'default' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}`}
				asChild
			>
				<Link href={href}>
					{label}
...
const ProductGrid = ({
	...
	products: { image: string; name: string; badge: string }[];
}) => (
	<div className="grid grid-cols-2 @lg:grid-cols-4 gap-4">
		{products.map((product, i) => (
			...
			>
				<Badge className="absolute top-3 left-3 z-10 bg-emerald-500">
					{product.badge}
				</Badge>
				<Image
					src={product.image}
