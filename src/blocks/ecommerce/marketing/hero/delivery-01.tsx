...
const Timeline = ({
	...
	<div className="flex flex-col @md:flex-row justify-between max-w-4xl mx-auto">
		{steps.map(({ icon: Icon, title, description, completed }, i) => (
			<div
				key={i}
				className="flex @md:flex-col items-center @md:items-center gap-4 @md:text-center relative pb-8 @md:pb-0"
			>
				<div
					className={`size-12 rounded-full flex items-center justify-center shrink-0 ${completed ? 'bg-primary text-primary-foreground' : 'border-2 border-muted bg-background'}`}
				>
					{completed ? (
					...
					steps={[
						{
							icon: Package,
							title: 'Order Placed',
							description: 'Instantly confirmed',
							completed: true,
						},
						{
							icon: Package,
				...
				<Features
					items={[
						...
						},
						{
							icon: Truck,
							title: 'Free Express',
							description: 'Free express shipping on orders $75+',
						},
						{
							icon: Package,
							title: 'Easy Returns',
