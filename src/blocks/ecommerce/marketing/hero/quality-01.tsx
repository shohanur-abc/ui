...
const Certifications = ({ items }: { items: { icon: React.ElementType; title: string; issuer: string }[] }) => (
...
const QualityImage = ({ src, alt, badges }: { src: string; alt: string; badges: string[] }) => (
    <div className="relative">
        ...
        <div className="absolute -left-4 top-1/4 space-y-3">
            {badges.map((badge, i) => (
                <Badge key={i} className="backdrop-blur bg-background/80 shadow-lg">{badge}</Badge>
            ))}
        </div>
    </div>
)
                ...
                <div className="grid @lg:grid-cols-2 gap-12 @xl:gap-20 items-center">
                    <div className="space-y-8">
                        ...
                        <Certifications items={[
                            { icon: BadgeCheck, title: "100% Authentic", issuer: "Verified Supplier Network" },
                            { icon: Shield, title: "Quality Tested", issuer: "Independent Lab Certified" },
                            { icon: Award, title: "Eco-Certified", issuer: "Global Sustainability Board" },
                            { icon: Sparkles, title: "Ethically Made", issuer: "Fair Trade Association" }
                        ]} />
                        <CTA items={[
                            { label: "Shop Certified Products", href: "/certified", icon: BadgeCheck },
                            { label: "Our Standards", href: "/quality", variant: "outline", icon: ArrowRight }
                        ]} />
                    </div>
                    <QualityImage 
