...
const ProductBadge = ({ text, variant }: { text: string; variant?: "default" | "secondary" | "destructive" }) => (
    <Badge variant={variant} className="absolute top-4 left-4 z-10">
        {text}
    </Badge>
)

const ProductImage = ({ src, alt, badge }: { src: string; alt: string; badge?: { text: string; variant?: "default" | "secondary" | "destructive" } }) => (
...
const Price = ({ current, original }: { current: string; original?: string }) => (
    <div className="flex items-baseline gap-3">
        <span className="text-3xl @md:text-4xl font-bold text-primary">{current}</span>
...
const CTA = ({ items }: { items: { label: string; href: string; icon?: React.ElementType; variant?: "default" | "outline" }[] }) => (
    <div className="flex flex-wrap gap-3">
        {items.map(({ label, href, icon: Icon, variant = "default" }, i) => (
            <Button key={i} size="lg" variant={variant} className="gap-2 flex-1 @sm:flex-none" asChild>
                <Link href={href}>
                ...
                <div className="grid @lg:grid-cols-2 gap-8 @xl:gap-16 items-center">
                    ...
                    <div className="space-y-6">
                        ...
                        <CTA items={[
                            ...
                            { label: "Watch Review", href: "#video", variant: "outline", icon: Play }
                        ]} />
