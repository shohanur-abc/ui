...
const Title = ({ text, highlight }: { text: string; highlight?: string }) => (
    <h1 className="text-4xl @sm:text-5xl @lg:text-6xl @xl:text-7xl font-serif font-bold tracking-tight">
        {text}{" "}
        {highlight && <span className="bg-gradient-to-r from-amber-500 to-yellow-400 bg-clip-text text-transparent">{highlight}</span>}
    </h1>
)
...
const CTA = ({ items }: { items: { label: string; href: string; icon?: React.ElementType; variant?: "default" | "outline" }[] }) => (
    <div className="flex flex-wrap gap-4">
        {items.map(({ label, href, icon: Icon, variant = "default" }, i) => (
            <Button key={i} size="lg" variant={variant} className={`gap-2 ${variant === "default" ? "bg-gradient-to-r from-amber-600 to-yellow-500 text-black hover:from-amber-700 hover:to-yellow-600" : "border-amber-500/50 hover:bg-amber-500/10"}`} asChild>
                <Link href={href}>
                    {label}
    ...
    <div className="relative">
        ...
        <div className="relative aspect-[4/5] rounded-3xl overflow-hidden border-2 border-amber-500/20">
            ...
            <div className="absolute bottom-6 inset-x-6 text-white">
                <p className="text-sm text-amber-300 uppercase tracking-wider">Limited Edition</p>
