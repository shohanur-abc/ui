...
const ProductSlide = ({ 
    ...
    <div className="grid @lg:grid-cols-2 gap-8 @lg:gap-16 items-center">
        ...
        <div className="space-y-6">
            <span className="text-sm text-primary font-medium uppercase tracking-wider">{category}</span>
            <h2 className="text-3xl @sm:text-4xl @lg:text-5xl font-bold tracking-tight">{title}</h2>
            <div className="flex items-center gap-3">
                <div className="flex">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`size-5 ${i < rating ? "fill-primary text-primary" : "text-muted"}`} />
                    ))}
                </div>
...
const CarouselNav = ({ current, total }: { current: number; total: number }) => (
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24">
                <ProductSlide 
                    image={{ src: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=800&h=1000&fit=crop", alt: "Featured sneaker" }}
                    badge="Featured"
                    category="Footwear"
