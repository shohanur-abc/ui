...
const PromoCard = ({
	...
	<div
		className={`rounded-2xl border bg-card p-6 @md:p-8 flex flex-col ${size === 'large' ? 'justify-between min-h-[280px]' : ''}`}
	>
		<div
			className={`size-12 @md:size-14 rounded-2xl ${color} flex items-center justify-center mb-4`}
		>
			<Icon className="size-6 @md:size-7 text-primary-foreground" />
		</div>
		<div className="space-y-2 mb-6">
			<h3
				className={`font-bold ${size === 'large' ? 'text-2xl @md:text-3xl' : 'text-xl'}`}
			>
				{title}
			</h3>
		...
		<div className="p-6 space-y-3">
			<div className="flex">
				{Array.from({ length: 5 }).map((_, i) => (
					<Star
						key={i}
						className={`size-4 ${i < rating ? 'fill-primary text-primary' : 'text-muted'}`}
					/>
				))}
			</div>
