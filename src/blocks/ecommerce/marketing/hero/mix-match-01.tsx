...
const MixMatchGrid = ({
	...
	items: {
		category: string;
		options: { image: string; name: string; selected?: boolean }[];
	}[];
}) => (
			...
			<div key={i} className="space-y-3">
				...
				<div className="flex gap-3 overflow-x-auto pb-2">
					{item.options.map((option, j) => (
						<div
							key={j}
							className={`relative size-20 @md:size-24 rounded-xl overflow-hidden shrink-0 cursor-pointer ring-2 ${option.selected ? 'ring-primary' : 'ring-transparent'} hover:ring-primary/50 transition-all`}
						>
							<Image
								src={option.image}
								alt={option.name}
		...
		<div className="relative aspect-[3/4] rounded-3xl overflow-hidden bg-gradient-to-br from-muted to-background border">
			<div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-8">
				{items.map((item, i) => (
					<div
						key={i}
						className="relative size-24 @md:size-32 rounded-2xl overflow-hidden shadow-lg"
					>
						<Image
							src={item.image}
