...
const Title = ({ text }: { text: string }) => (
    <h1 className="text-3xl @sm:text-4xl @lg:text-5xl font-bold tracking-tight text-center">{text}</h1>
)
...
const SocialProof = ({ stats }: { stats: { value: string; label: string }[] }) => (
...
const Testimonials = ({ items }: { items: { avatar: string; name: string; rating: number; text: string }[] }) => (
                    ...
                    <div>
                        ...
                        <div className="flex">
                            {Array.from({ length: 5 }).map((_, j) => (
                                <Star key={j} className={`size-3 ${j < item.rating ? "fill-primary text-primary" : "text-muted"}`} />
                            ))}
                        </div>
                    </div>
                </div>
...
const CustomerAvatars = ({ avatars }: { avatars: { src: string; fallback: string }[] }) => (
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 space-y-12">
                ...
                <Testimonials items={[
                    { avatar: "https://i.pravatar.cc/150?img=10", name: "Sarah K.", rating: 5, text: "Best online shopping experience! The quality is amazing and shipping was super fast." },
                    { avatar: "https://i.pravatar.cc/150?img=11", name: "Michael R.", rating: 5, text: "I've been a customer for years. The styles are always on-trend and the customer service is top-notch." },
                    { avatar: "https://i.pravatar.cc/150?img=12", name: "Emma L.", rating: 5, text: "Love the sustainable options! Finally a brand that cares about the planet and still looks great." }
                ]} />
                <CTA items={[
                    { label: "Shop Now", href: "/shop", icon: ArrowRight },
                    { label: "Read More Reviews", href: "/reviews", variant: "outline" }
                ]} />
            </div>
        </section>
    )
