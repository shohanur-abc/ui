...
import { ArrowRight, ShoppingCart, Package, Truck, CheckCircle2 } from "lucide-react"
...
const CartPreview = ({ items, total }: { items: { image: string; name: string; price: string; qty: number }[]; total: string }) => (
    <div className="max-w-md mx-auto rounded-2xl border bg-card overflow-hidden">
        <div className="p-4 border-b">
            <div className="flex items-center gap-2">
                <ShoppingCart className="size-5 text-primary" />
                <span className="font-semibold">Your Cart ({items.reduce((acc, i) => acc + i.qty, 0)} items)</span>
            </div>
        </div>
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 space-y-12">
                ...
                <CartPreview 
                    items={[
                        { image: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=100&h=100&fit=crop", name: "Urban Runner Pro", price: "$149", qty: 1 },
                        { image: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=100&h=100&fit=crop", name: "Summer Dress", price: "$89", qty: 2 }
                    ]}
                    total="$327"
                />
                <Features items={[
