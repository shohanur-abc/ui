...
const Title = ({ text, highlight }: { text: string; highlight?: string }) => (
    <h1 className="text-3xl @sm:text-4xl @lg:text-5xl @xl:text-6xl font-bold tracking-tight">
        {text}{" "}
        {highlight && <span className="bg-gradient-to-r from-orange-500 to-amber-500 bg-clip-text text-transparent">{highlight}</span>}
    </h1>
)
...
const SeasonalTabs = ({ seasons }: { seasons: { icon: React.ElementType; label: string; active?: boolean }[] }) => (
    <div className="flex gap-2">
        {seasons.map(({ icon: Icon, label, active }, i) => (
            <Button key={i} variant={active ? "default" : "outline"} size="sm" className="gap-2">
                <Icon className="size-4" />
                {label}
            </Button>
        ))}
...
const CTA = ({ items }: { items: { label: string; href: string; icon?: React.ElementType; variant?: "default" | "outline" }[] }) => (
    <div className="flex flex-wrap gap-4">
        {items.map(({ label, href, icon: Icon, variant = "default" }, i) => (
            <Button key={i} size="lg" variant={variant} className={`gap-2 ${variant === "default" ? "bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600" : ""}`} asChild>
                <Link href={href}>
                    {label}
...
const SeasonalProducts = ({ products }: { products: { image: string; name: string; price: string }[] }) => (
    <div className="grid grid-cols-2 gap-4">
        {products.map((product, i) => (
            <div key={i} className={`group relative rounded-2xl overflow-hidden ${i === 0 ? "col-span-2 aspect-[2/1]" : "aspect-square"}`}>
...
const CollectionCards = ({ items }: { items: { title: string; count: string }[] }) => (
                ...
                <div className="grid @lg:grid-cols-2 gap-12 @xl:gap-20 items-center">
                    <div className="space-y-8">
                        ...
                        <CTA items={[
                            ...
                            { label: "View Lookbook", href: "/lookbook/fall", variant: "outline", icon: ArrowRight }
                        ]} />
                    </div>
                    <SeasonalProducts products={[
                        { image: "https://images.unsplash.com/photo-1551028719-00167b16eac5?w=800&h=400&fit=crop", name: "Wool Blend Coat", price: "$289" },
                        { image: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&h=400&fit=crop", name: "Leather Boots", price: "$179" },
                        { image: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=400&h=400&fit=crop", name: "Cashmere Sweater", price: "$199" }
                    ]} />
                </div>
            </div>
        </section>
