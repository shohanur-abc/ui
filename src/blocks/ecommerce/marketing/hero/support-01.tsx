...
import { ArrowRight, MessageSquare, HeadphonesIcon, Mail, Phone } from "lucide-react"
...
const ContactOptions = ({ options }: { options: { icon: React.ElementType; title: string; description: string; action: string; href: string }[] }) => (
    <div className="grid @md:grid-cols-3 gap-6">
        {options.map(({ icon: Icon, title, description, action, href }, i) => (
            <div key={i} className="rounded-2xl border bg-card p-6 text-center space-y-4 hover:border-primary transition-colors">
        ...
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 space-y-12">
                ...
                <ContactOptions options={[
                    { icon: MessageSquare, title: "Live Chat", description: "Chat with us instantly for quick answers", action: "Start Chat", href: "/chat" },
                    { icon: Mail, title: "Email Us", description: "Send us an email, we'll reply within 24hrs", action: "Send Email", href: "/contact" },
                    { icon: Phone, title: "Call Us", description: "Speak directly with our support team", action: "Call Now", href: "tel:+1234567890" }
                ]} />
            </div>
        </section>
    )
