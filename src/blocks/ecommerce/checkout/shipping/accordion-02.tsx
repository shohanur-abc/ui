import { Truck, Package, Zap, Clock, Check, Info, ChevronDown } from 'lucide-react';
...
const ShippingTierAccordion = ({
	...
	<AccordionItem value={value} className={`border rounded-xl px-4 ${popular ? 'ring-2 ring-primary/30' : ''}`}>
		{popular && <Badge className="absolute -top-2.5 left-4 z-10">Most Popular</Badge>}
...
export default function Main() {
	const tiers = [
		...
		},
		{
			...
			price: '$7.99',
			description: 'Reliable delivery at a great value. Best for most customers.',
			features: ['Real-time tracking', 'SMS & email updates', 'Insurance up to $100', 'Priority handling'],
			popular: true,
		},
		{
			value: 'express',
			...
			features: ['Priority handling', 'Real-time tracking', 'Full insurance coverage', 'Dedicated support'],
		},
		{
			value: 'overnight',
			icon: Zap,
			...
			features: ['Guaranteed delivery', 'Premium tracking', 'Full insurance', 'Signature on delivery', '24/7 support'],
		},
	];
		...
		<section className="@container relative overflow-hidden">
			<div className="mx-auto max-w-2xl px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
				<div className="text-center mb-10">
					...
					<p className="text-muted-foreground">Expand to see details about each tier</p>
				</div>
