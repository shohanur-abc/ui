import { Check, CreditCard, Lock, Minus, Plus, ShoppingBag, Trash2, Truck } from 'lucide-react';
...
const CartProduct = ({ name, variant, price, quantity, inStock }: CartProductProps) => (
...
const DeliveryOption = ({ label, estimate, price, selected }: DeliveryOptionProps) => (
	<div
...
const SummaryLine = ({ label, value, isTotal, isDiscount }: { label: string; value: string; isTotal?: boolean; isDiscount?: boolean }) => (
...
export default function Main() {
	const products: CartProductProps[] = [
		{ id: '1', name: 'Leather Backpack', variant: 'Brown / Large', price: '$149.00', quantity: 1, inStock: true },
		{ id: '2', name: 'Classic Watch', variant: 'Silver', price: '$299.00', quantity: 1, inStock: false },
	];

	const deliveryOptions: DeliveryOptionProps[] = [
		{ id: 'standard', label: 'Standard', estimate: '5-7 business days', price: 'Free', selected: true },
		{ id: 'express', label: 'Express', estimate: '2-3 business days', price: '$12.00' },
	];
					...
					<div>
						<Card className="border-border/50 bg-card/50 backdrop-blur-sm @lg:sticky @lg:top-8">
							...
							<CardContent className="space-y-6">
								...
								<Separator />
								<div className="space-y-4">
									<CardField id="card" label="Card Number" placeholder="1234 5678 9012 3456" icon={CreditCard} />
									<CardField id="name" label="Name on Card" placeholder="John Doe" />
									<div className="grid grid-cols-2 gap-4">
										<CardField id="exp" label="Expiry" placeholder="MM/YY" />
										<CardField id="cvc" label="CVC" placeholder="123" type="password" />
									</div>
								</div>
							</CardContent>
							<CardFooter>
