...
import { ArrowLeft, ArrowRight, Building2, Check, CreditCard, Lock, Shield, Smartphone, Wallet, Zap } from 'lucide-react';
		...
		{Array.from({ length: total }, (_, i) => (
			<div
				...
				className={`rounded-full transition-all ${
					i < current ? 'size-2 bg-primary' : i === current ? 'size-3 bg-primary' : 'size-2 bg-muted'
				}`}
			/>
		))}
	</div>
...
const PaymentProviderCard = ({ id, name, icon: Icon, description, badge, color, selected, onSelect }: PaymentProvider & { selected: boolean; onSelect: (id: string) => void }) => (
	<button
		type="button"
		onClick={() => onSelect(id)}
	...
	>
		...
		<div className={`size-5 rounded-full border-2 flex items-center justify-center ${selected ? 'border-primary bg-primary' : 'border-muted-foreground'}`}>
...
const CardPaymentForm = () => (
	<div className="space-y-4">
		...
		<div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
			...
			<Label htmlFor="save-payment" className="text-sm cursor-pointer">Save for faster checkout</Label>
		</div>
	</div>
);
...
const BankTransferInfo = () => (
	<div className="space-y-4">
		<div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
			...
			<p className="text-sm text-muted-foreground">Secure bank-to-bank transfer. Usually completes in 1-3 business days.</p>
		</div>
		<div className="space-y-2">
...
export default function Main() {
	...
	const providers: PaymentProvider[] = [
		{ id: 'card', name: 'Credit/Debit Card', icon: CreditCard, description: 'Visa, Mastercard, Amex', badge: 'Popular', color: 'bg-blue-500/10 text-blue-500' },
		{ id: 'wallet', name: 'PayPal', icon: Wallet, description: 'Fast and secure', badge: 'Instant', color: 'bg-purple-500/10 text-purple-500' },
		{ id: 'apple', name: 'Apple Pay', icon: Smartphone, description: 'Pay with Face ID', color: 'bg-gray-500/10 text-gray-500' },
		{ id: 'bank', name: 'Bank Transfer', icon: Building2, description: '1-3 business days', color: 'bg-emerald-500/10 text-emerald-500' },
	];
