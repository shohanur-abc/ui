...
import { Bitcoin, Building2, CreditCard, DollarSign, Lock, Sparkles, Wallet } from 'lucide-react';
...
const PaymentTabButton = ({ value, icon: Icon, label, fee, recommended }: PaymentTabProps) => (
	<TabsTrigger
		value={value}
...
const PaymentTabs = ({ tabs, children }: { tabs: PaymentTabProps[]; children: React.ReactNode }) => (
...
const CardContent_Tab = () => (
	<div className="space-y-4">
		<FormInput id="card" label="Card Number" placeholder="4242 4242 4242 4242" icon={CreditCard} />
...
const BankContent = () => (
	<div className="space-y-4">
		<FormInput id="bank" label="Bank Name" placeholder="Select your bank" icon={Building2} />
		<FormInput id="account" label="Account Number" placeholder="Enter account number" />
		<FormInput id="routing" label="Routing Number" placeholder="Enter routing number" />
...
const CryptoContent = () => (
	<div className="space-y-4">
		<div className="grid grid-cols-2 gap-3">
			<Button variant="outline" className="h-16 flex-col gap-1 bg-orange-500/5 border-orange-500/20 hover:bg-orange-500/10">
				<Bitcoin className="size-6 text-orange-500" />
				<span className="text-xs">Bitcoin</span>
			</Button>
			<Button variant="outline" className="h-16 flex-col gap-1 bg-blue-500/5 border-blue-500/20 hover:bg-blue-500/10">
		...
		<div className="p-3 rounded-lg bg-primary/5 border border-primary/20 text-sm">
			<span className="font-medium text-primary">5% discount</span> when paying with crypto
		</div>
	</div>
);

const PricingBreakdown = ({ lines }: { lines: { label: string; value: string; isTotal?: boolean; isDiscount?: boolean }[] }) => (
...
export default function Main() {
	const tabs: PaymentTabProps[] = [
		...
		{ value: 'wallet', icon: Wallet, label: 'Wallet', fee: 'No fee', recommended: true },
