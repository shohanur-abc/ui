	...
	<div className="group relative overflow-hidden rounded-xl border border-cyan-500/30 bg-slate-900 p-4 transition-all hover:border-cyan-500/60 hover:shadow-[0_0_30px_rgba(0,255,255,0.2)]">
		...
		<div className="relative flex items-center gap-4">
			...
			<div className="text-right">
				...
				<Badge className="border-cyan-500/50 bg-cyan-500/10 text-cyan-300">×{item.qty}</Badge>
			</div>
		</div>
	</div>
);
...
const SummaryLine = ({
	...
	<div className={`flex justify-between ${bold ? 'text-xl font-bold' : 'text-sm'}`}>
		...
		<span className={green ? 'text-green-400' : bold ? 'text-cyan-400' : 'text-cyan-50'}>{value}</span>
	</div>
);
...
export default function Main() {
	const items: CartItem[] = [
		...
		},
		{
			...
			price: 39.99,
			qty: 1,
			image: 'https://images.unsplash.com/photo-1629429407756-446d66f5b24f?w=200&h=200&fit=crop',
		},
	];

	return (
		<section className="@container relative overflow-hidden bg-slate-950" data-theme="neon">
				...
				<div className="grid gap-6 @lg:grid-cols-[1fr_340px]">
					...
					<Card className="@lg:sticky @lg:top-8 @lg:self-start border-cyan-500/30 bg-slate-900 shadow-[0_0_50px_rgba(0,255,255,0.1)]">
						<CardContent className="space-y-4 pt-6">
							...
							<Button size="lg" className="w-full gap-2 bg-gradient-to-r from-cyan-500 to-purple-500 text-white hover:from-cyan-600 hover:to-purple-600">
