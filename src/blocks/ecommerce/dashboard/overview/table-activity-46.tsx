...
export default function Main() {
	...
	const activities: ActivityRow[] = [
		...
		},
		{
			...
			description: 'Joined from referral program',
			user: 'Sarah Wilson',
			userInitials: 'SW',
			time: '45 min ago',
			read: true,
		...
		},
		{
			...
			type: 'order',
			title: 'Order shipped',
			description: 'Order #4519 shipped via FedEx',
			user: 'System',
			userInitials: 'SY',
			time: '3 hours ago',
			read: true,
		},
		{
			...
			type: 'review',
			title: '4-star review',
			description: 'USB-C Hub - "Good but could be faster"',
			user: 'Emma Davis',
			userInitials: 'ED',
			time: '4 hours ago',
			read: true,
							...
							<Table>
								...
								<TableBody>
									{activities.map((activity) => (
										<TableRow
											key={activity.id}
											className={activity.read ? '' : 'bg-primary/5'}
										>
											<TableCell>
												<div className="flex items-center gap-3">
													...
													<div>
														<p
															className={`font-medium ${activity.read ? '' : 'text-primary'}`}
														>
															{activity.title}
														</p>
