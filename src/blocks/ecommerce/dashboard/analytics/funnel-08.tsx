			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-8 @md:py-12 @xl:py-16">
				<Card className="border-border/50 bg-card/80 backdrop-blur-sm">
					<CardHeader className="pb-2">
						...
						<p className="text-xs text-muted-foreground">Why users leave at each step</p>
					</CardHeader>
					<CardContent className="pt-4">
						<div className="space-y-4">
							{steps.map((step, i) => {
								const exitRate = ((step.exited / step.entered) * 100).toFixed(1);
									...
									<div key={i} className="border border-border/50 rounded-lg p-4">
										...
										<div className="flex gap-2 mb-2">
											<div
												...
												style={{ width: `${(continued / step.entered) * 100}%` }}
											/>
											<div
												className="h-2 bg-rose-500/50 rounded-sm"
												style={{ width: `${(step.exited / step.entered) * 100}%` }}
											/>
										</div>
						...
						<div className="mt-6 pt-4 border-t border-border/50 flex items-center justify-between text-sm">
							...
							<span className="text-lg font-semibold text-emerald-500">
								{(((steps[4].entered - steps[4].exited) / steps[0].entered) * 100).toFixed(2)}%
							</span>
						</div>
					</CardContent>
				</Card>
