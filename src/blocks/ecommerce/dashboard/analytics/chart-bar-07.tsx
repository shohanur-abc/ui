...
type WaterfallData = { label: string; value: number; type: 'positive' | 'negative' | 'total' };
...
const WaterfallChart = ({ data }: { data: WaterfallData[] }) => {
	...
	const min = Math.min(...processedData.map((d) => Math.min(d.start, d.end)), 0);
	const range = max - min;
		...
		<div className="relative h-64 w-full">
			<svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full">
				{processedData.map((d, i) => {
					...
					const color = d.type === 'total' ? '#3b82f6' : d.type === 'positive' ? '#22c55e' : '#ef4444';
					return (
						<g key={i}>
							<rect x={x} y={top} width={width} height={height || 0.5} fill={color} rx="0.5" />
							{i > 0 && d.type !== 'total' && (
								<line
									x1={x - 2}
									y1={getY(d.start)}
				...
				<line x1="0" y1={getY(0)} x2="100" y2={getY(0)} stroke="hsl(var(--border))" strokeWidth="0.2" />
			</svg>
			<div className="absolute bottom-0 left-0 right-0 flex justify-between text-xs text-muted-foreground px-2">
				{data.map((d, i) => (
					<span key={i} className="flex-1 text-center truncate text-[10px]">{d.label}</span>
				))}
			</div>
		</div>
	);
			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-8 @md:py-12 @xl:py-16">
				<Card className="border-border/50 bg-card/80 backdrop-blur-sm">
					<CardHeader className="pb-2">
						<CardTitle className="text-sm font-medium">Revenue Waterfall</CardTitle>
						<p className="text-xs text-muted-foreground">Breakdown of revenue changes</p>
					</CardHeader>
					<CardContent>
