...
type DistributionBarData = { label: string; value: number; previous: number; color: string };
		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-2 @sm:grid-cols-3 @lg:grid-cols-5 gap-4">
				{data.map((item, i) => {
					...
					const change = ((item.value - item.previous) / item.previous * 100).toFixed(1);
					const changeNum = parseFloat(change);
					const TrendIcon: LucideIcon = changeNum > 0 ? TrendingUp : changeNum < 0 ? TrendingDown : Minus;
					return (
						<div key={i} className="p-3 rounded-lg bg-muted/20">
							...
							<div className={`flex items-center gap-1 text-xs ${changeNum >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
								<TrendIcon className="size-3" />
								<span>{changeNum >= 0 ? '+' : ''}{change}%</span>
							</div>
						</div>
					);
				})}
			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-8 @md:py-12 @xl:py-16">
				<Card className="border-border/50 bg-card/80 backdrop-blur-sm">
					<CardHeader className="pb-2">
						...
						<p className="text-xs text-muted-foreground">Visitors by acquisition channel</p>
					</CardHeader>
					<CardContent>
