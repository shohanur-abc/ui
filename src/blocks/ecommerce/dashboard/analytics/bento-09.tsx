					...
					<div key={i}>
						...
						<div className="flex h-1.5 rounded-full overflow-hidden">
							<div
								...
								style={{ width: `${(level.inStock / level.total) * 100}%` }}
							/>
							<div
								className="bg-amber-500"
								style={{ width: `${(level.lowStock / level.total) * 100}%` }}
							/>
							<div
								className="bg-rose-500"
								style={{ width: `${(level.outOfStock / level.total) * 100}%` }}
							/>
						</div>
					</div>
				))}
	...
	<Card className="border-border/50 bg-card/80">
		...
		<CardContent className="space-y-3">
			{products.map((product, i) => (
				...
				>
					<span className="flex items-center justify-center size-5 rounded bg-muted text-xs">
						{i + 1}
					</span>
					...
					<div className="text-right">
						<p
							className={`text-sm font-semibold ${type === 'fast' ? 'text-emerald-500' : 'text-amber-500'}`}
						>
							{product.velocity}
						</p>
