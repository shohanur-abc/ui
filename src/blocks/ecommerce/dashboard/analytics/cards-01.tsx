...
const MetricCard = ({ icon: Icon, label, value, change, trend }: MetricCardProps) => (
		...
		<CardContent className="relative p-4 @sm:p-5 @lg:p-6">
			<div className="flex items-start justify-between gap-4">
				<div className="flex-1 space-y-2">
					...
					<p className="text-2xl @sm:text-3xl @lg:text-4xl font-bold tracking-tight">{value}</p>
					<div className={`inline-flex items-center gap-1 text-xs @sm:text-sm font-medium ${trend === 'up' ? 'text-emerald-500' : 'text-rose-500'}`}>
						{trend === 'up' ? <ArrowUpRight className="size-3 @sm:size-4" /> : <ArrowDownRight className="size-3 @sm:size-4" />}
...
const metrics: MetricCardProps[] = [
	...
	{ icon: Users, label: 'Active Customers', value: '1,234', change: 3.1, trend: 'down' },
	{ icon: Package, label: 'Products Sold', value: '12,543', change: 15.3, trend: 'up' },
];
