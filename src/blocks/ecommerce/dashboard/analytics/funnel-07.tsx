						...
						<div className="grid grid-cols-1 @lg:grid-cols-2 gap-8">
							...
							<div>
								<div className="flex items-center gap-2 mb-4">
									...
									<Badge variant="outline" className="text-xs">
										Conv:{' '}
										{((steps[4].desktop / steps[0].desktop) * 100).toFixed(2)}%
									</Badge>
								</div>
							...
							<div>
								<div className="flex items-center gap-2 mb-4">
									...
									<Badge variant="outline" className="text-xs">
										Conv:{' '}
										{((steps[4].mobile / steps[0].mobile) * 100).toFixed(2)}%
									</Badge>
								</div>
								<div className="space-y-2">
									{steps.map((step, i) => {
						...
						<div className="mt-6 pt-4 border-t border-border/50">
							<div className="grid grid-cols-5 gap-2">
								{steps.map((step, i) => {
									const mobileRate =
										i > 0
											? ((step.mobile / steps[i - 1].mobile) * 100).toFixed(0)
											: '100';
									const desktopRate =
										i > 0
											? ((step.desktop / steps[i - 1].desktop) * 100).toFixed(0)
											: '100';
									...
									return (
										<div key={i} className="text-center">
											...
											<p
												className={`text-xs font-medium ${diff > 0 ? 'text-cyan-500' : diff < 0 ? 'text-purple-500' : ''}`}
											>
												{diff > 0 ? '+' : ''}
												{diff.toFixed(0)}%
											</p>
