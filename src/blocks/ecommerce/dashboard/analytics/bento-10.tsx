...
const SystemMetric = ({ icon: Icon, label, value, status, uptime }: SystemMetricProps) => {
	const statusConfig = {
		healthy: { color: 'text-emerald-500', bg: 'bg-emerald-500/10', icon: CheckCircle2 },
		warning: { color: 'text-amber-500', bg: 'bg-amber-500/10', icon: AlertCircle },
		critical: { color: 'text-rose-500', bg: 'bg-rose-500/10', icon: AlertCircle },
	};
			...
			<CardContent className="p-4">
				<div className="flex items-center justify-between mb-3">
					...
					<div className={`flex items-center gap-1 px-2 py-0.5 rounded-full ${statusConfig[status].bg}`}>
						...
						<span className={`text-xs font-medium ${statusConfig[status].color}`}>{status}</span>
					</div>
				</div>
...
const ResponseTimesCard = ({ endpoints }: { endpoints: ResponseTimeProps[] }) => (
	<Card className="border-border/50 bg-card/80 col-span-2">
		<CardHeader className="flex flex-row items-center justify-between pb-2">
			<div className="flex items-center gap-2">
				<Zap className="size-4 text-primary" />
				<CardTitle className="text-sm font-medium">API Response Times</CardTitle>
			</div>
				...
				{endpoints.map((ep, i) => (
					<div key={i} className="grid grid-cols-5 gap-2 text-sm">
						<span className="col-span-2 font-medium truncate flex items-center gap-2">
							<div className={`size-2 rounded-full ${ep.status === 'fast' ? 'bg-emerald-500' : ep.status === 'normal' ? 'bg-amber-500' : 'bg-rose-500'}`} />
							{ep.endpoint}
						</span>
			...
			{errors.map((error, i) => (
				<div key={i} className="space-y-1.5">
					<div className="flex items-center justify-between text-sm">
						...
						<span className={`font-semibold ${error.rate > 1 ? 'text-rose-500' : 'text-emerald-500'}`}>
							{error.rate}%
						</span>
					</div>
					<Progress value={error.rate * 10} className={`h-1 ${error.rate > 1 ? '[&>div]:bg-rose-500' : ''}`} />
			...
			{events.map((event, i) => (
				<div key={i} className="flex items-center justify-between py-1.5 border-b border-border/20 last:border-0">
					...
					<div className="text-right">
						...
						<p className="text-xs text-muted-foreground mt-1">{event.count} events</p>
					</div>
				</div>
			))}
		</CardContent>
...
const UptimeCard = ({ data }: { data: UptimeData[] }) => (
	<Card className="border-border/50 bg-card/80 col-span-2">
		<CardHeader className="flex flex-row items-center justify-between pb-2">
			<CardTitle className="text-sm font-medium">System Uptime (Last 24 Hours)</CardTitle>
			<Badge variant="default" className="bg-emerald-500">99.9%</Badge>
		</CardHeader>
		<CardContent>
...
const systemMetrics: SystemMetricProps[] = [
	{ icon: Server, label: 'API Server', value: '45ms', status: 'healthy', uptime: '99.99%' },
	{ icon: Server, label: 'Database', value: '12ms', status: 'healthy', uptime: '99.95%' },
	{ icon: Server, label: 'CDN', value: '8ms', status: 'healthy', uptime: '100%' },
	{ icon: Server, label: 'Cache', value: '2ms', status: 'warning', uptime: '99.8%' },
];

const endpoints: ResponseTimeProps[] = [
	{ endpoint: '/api/products', avg: '45ms', p95: '120ms', p99: '250ms', status: 'fast' },
	{ endpoint: '/api/checkout', avg: '180ms', p95: '450ms', p99: '800ms', status: 'normal' },
	{ endpoint: '/api/search', avg: '95ms', p95: '200ms', p99: '350ms', status: 'fast' },
	{ endpoint: '/api/users', avg: '320ms', p95: '850ms', p99: '1.2s', status: 'slow' },
];
...
const securityEvents: SecurityEventProps[] = [
	{ type: 'Failed Login Attempts', count: 156, severity: 'medium', lastSeen: '5 min ago' },
	{ type: 'Rate Limit Exceeded', count: 45, severity: 'low', lastSeen: '1 hour ago' },
	{ type: 'Suspicious Activity', count: 3, severity: 'high', lastSeen: '2 hours ago' },
];
