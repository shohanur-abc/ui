...
import { BarChart3, DollarSign, ShoppingCart, Users, TrendingUp, TrendingDown, ArrowUpRight } from 'lucide-react';
	...
	<Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/30 @xl:col-span-2 @xl:row-span-2">
		...
		<CardContent>
			<div className="mb-4">
				...
				<div className={`flex items-center gap-1 mt-2 ${metric.change >= 0 ? 'text-primary' : 'text-destructive'}`}>
				...
				<ResponsiveContainer width="100%" height="100%">
					{metric.chartType === 'area' ? (
						<AreaChart data={metric.chartData}>
							<defs>
								<linearGradient id="heroGradient" x1="0" y1="0" x2="0" y2="1">
									<stop offset="5%" stopColor="oklch(0.70 0.18 155)" stopOpacity={0.4} />
						...
						<BarChart data={metric.chartData}>
							<Bar dataKey="value" fill="oklch(0.70 0.18 155)" radius={[4, 4, 0, 0]} />
						</BarChart>
					)}
				</ResponsiveContainer>
			</div>
...
const BentoLayout11 = ({ heroMetric, secondaryMetrics, quickStats }: BentoLayout11Props) => (
...
export default function Main() {
	const heroMetric: HeroMetric = {
		...
		chartData: [
			...
			{ value: 58 }, { value: 72 }, { value: 68 }, { value: 85 },
		],
		chartType: 'area',
	};

	const secondaryMetrics: HeroMetric[] = [
		{ title: 'Orders', value: '1,284', change: 8.2, period: 'This Month', chartData: [{ value: 30 }, { value: 45 }, { value: 35 }, { value: 55 }, { value: 48 }, { value: 62 }], chartType: 'area' },
		{ title: 'Customers', value: '324', change: 18.7, period: 'New', chartData: [{ value: 20 }, { value: 35 }, { value: 42 }, { value: 38 }, { value: 55 }, { value: 68 }], chartType: 'area' },
	];

	const quickStats: QuickStat[] = [
		{ label: 'Avg Order', value: '$111', icon: <DollarSign className="size-4" /> },
		{ label: 'Products', value: '4,821', icon: <ShoppingCart className="size-4" /> },
		{ label: 'Active Users', value: '2.4K', icon: <Users className="size-4" /> },
		{ label: 'Conversion', value: '3.8%', icon: <BarChart3 className="size-4" /> },
	];

	return (
		<section className="@container" data-theme="sales">
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-8 @md:py-12">
				<BentoLayout11 heroMetric={heroMetric} secondaryMetrics={secondaryMetrics} quickStats={quickStats} />
			</div>
		</section>
	);
}
