...
const IntegrationRow = ({ integration, onToggle }: IntegrationRowProps) => {
	const statusConfig = {
		...
		disconnected: {
			icon: <Unlink className="size-4 text-muted-foreground" />,
			label: 'Disconnected',
	...
	return (
		<div className="flex items-center gap-4 border-b py-4 last:border-0">
			<div
				className={`flex size-10 items-center justify-center rounded-lg ${integration.status === 'connected' ? 'bg-primary/10' : 'bg-muted'}`}
			>
				<Link2
					className={`size-5 ${integration.status === 'connected' ? 'text-primary' : 'text-muted-foreground'}`}
				/>
			</div>
			...
			{integration.itemsSynced !== undefined && (
				<div className="hidden text-center @sm:block">
					...
					<p className="font-medium">
						{integration.itemsSynced.toLocaleString()}
					</p>
				</div>
			)}
			<div className="flex items-center gap-2">
...
export default function Main() {
	...
	const totalSynced = integrations.reduce(
		(sum, i) => sum + (i.itemsSynced || 0),
		0,
	);
