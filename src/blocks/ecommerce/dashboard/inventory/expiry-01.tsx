...
const ExpiryBadge = ({ status, daysRemaining }: ExpiryBadgeProps) => {
	const config: Record<ExpiryStatus, { label: string; variant: 'destructive' | 'secondary' | 'outline' }> = {
		expired: { label: 'Expired', variant: 'destructive' },
		'expiring-soon': { label: `${daysRemaining} days left`, variant: 'secondary' },
...
const BatchRow = ({ item }: BatchRowProps) => {
	...
	const maxDays = 90;
	const progressValue = Math.max(0, Math.min((item.daysRemaining / maxDays) * 100, 100));

	return (
		<div className="flex items-center gap-4 rounded-lg border p-4 transition-colors hover:bg-muted/50">
			<div className={`flex size-10 items-center justify-center rounded-lg ${
				item.status === 'expired' ? 'bg-destructive/10' : item.status === 'expiring-soon' ? 'bg-amber-500/10' : 'bg-muted'
			}`}>
				{item.status === 'expired' || item.status === 'expiring-soon' ? (
					<AlertTriangle className={`size-5 ${item.status === 'expired' ? 'text-destructive' : 'text-amber-500'}`} />
				) : (
...
export default function Main() {
	const items: BatchItem[] = [
		{ id: '1', productName: 'Organic Protein Bars', sku: 'OPB-001', batchNumber: 'B2024-001', quantity: 500, expiryDate: '2024-01-10', daysRemaining: -8, status: 'expired' },
		{ id: '2', productName: 'Vitamin C Supplements', sku: 'VCS-002', batchNumber: 'B2024-015', quantity: 250, expiryDate: '2024-02-05', daysRemaining: 18, status: 'expiring-soon' },
		{ id: '3', productName: 'Energy Drinks Pack', sku: 'EDP-003', batchNumber: 'B2024-022', quantity: 1200, expiryDate: '2024-02-15', daysRemaining: 28, status: 'expiring-soon' },
		{ id: '4', productName: 'Protein Powder', sku: 'PP-004', batchNumber: 'B2024-030', quantity: 180, expiryDate: '2024-04-20', daysRemaining: 93, status: 'good' },
		{ id: '5', productName: 'Herbal Tea Collection', sku: 'HTC-005', batchNumber: 'B2024-045', quantity: 340, expiryDate: '2024-06-01', daysRemaining: 135, status: 'good' },
	];
	...
	return (
		<section className="@container" data-theme="dashboard">
			<div className="mx-auto max-w-7xl px-4 py-8 @sm:px-6 @md:py-10 @2xl:px-8">
				<Card>
					<CardHeader>
						<CardTitle className="text-xl @lg:text-2xl">Expiry Management</CardTitle>
						<CardDescription>Track product batches and expiration dates</CardDescription>
					</CardHeader>
					<CardContent className="space-y-6">
						<SummaryCards expired={expiredCount} expiringSoon={expiringSoonCount} good={goodCount} />
