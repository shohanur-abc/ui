	...
	return (
		<div className="flex items-center gap-4 border-b py-4 last:border-0">
			...
			<div className="hidden text-center @sm:block">
				...
				<p
					className={`font-semibold ${item.stock <= item.reorderPoint ? 'text-destructive' : ''}`}
				>
					{item.stock}
				</p>
			</div>
			...
			<div className="text-center">
				...
				<div className="flex items-center justify-center gap-1">
					...
					<span
						className={`font-semibold ${item.daysOfStock <= 7 ? 'text-destructive' : item.daysOfStock <= 14 ? 'text-amber-500' : ''}`}
					>
						{item.daysOfStock}d
					</span>
				</div>
