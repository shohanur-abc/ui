...
const WeightDisplay = ({ weight, label }: WeightDisplayProps) => (
	<div className="flex items-center gap-2 text-sm">
		...
		<span className="text-muted-foreground">{label}:</span>
		<span className="font-medium">{weight.value} {weight.unit}</span>
	</div>
);
...
const ShippingClassBadge = ({ shippingClass, labels }: ShippingClassBadgeProps) => {
	const config = {
	...
	return (
		<Badge className={config[shippingClass]}>
			{labels[shippingClass]}
		</Badge>
	);
};
...
const ShippingZones = ({ zones, freeThreshold, handlingFee, labels }: ShippingZonesProps) => (
	<div className="space-y-2">
		...
		<div className="grid gap-2 @sm:grid-cols-2 @lg:grid-cols-4">
			{zones.map((zone) => (
				<div key={zone.name} className="rounded-lg border bg-muted/30 p-2 text-center">
					<div className="flex items-center justify-center gap-1 text-xs text-muted-foreground">
						<MapPin className="size-3" />
						{zone.name}
					</div>
					<div className="text-lg font-bold">${zone.rate.toFixed(2)}</div>
					<div className="text-xs text-muted-foreground">{zone.estimatedDays}</div>
				</div>
			))}
		</div>
		{handlingFee > 0 && (
...
interface ProductRowProps {
	...
	labels: {
		...
		shippingClasses: Record<'standard' | 'fragile' | 'oversized' | 'hazmat', string>;
		zones: string;
		free: string;
		handling: string;
	};
...
const ProductRow = ({ product, actions, labels }: ProductRowProps) => (
	<div className="rounded-lg border bg-card p-4">
			...
			<div className="min-w-0 flex-1">
				<div className="flex items-start justify-between">
					...
					<div className="flex items-center gap-2">
						<PackageTypeBadge type={product.packageType} labels={labels.packageTypes} />
						<ShippingClassBadge shippingClass={product.shippingClass} labels={labels.shippingClasses} />
						<DropdownMenu>
				...
				<div className="mt-2 flex flex-wrap gap-4">
					<DimensionDisplay dimensions={product.dimensions} label={labels.dimensions} />
		...
		<ShippingZones
			...
			labels={{ zones: labels.zones, free: labels.free, handling: labels.handling }}
		/>
	</div>
);
...
export default function Main() {
	const products: ProductShipping[] = [
		{
			...
			sku: 'HOM-VAS-001',
			image: 'https://images.unsplash.com/photo-1612196808214-b8e1d6145a8c?w=100&h=100&fit=crop',
		...
		},
		{
			...
			sku: 'FUR-SHF-003',
			image: 'https://images.unsplash.com/photo-1532372576444-dda954194ad0?w=100&h=100&fit=crop',
	...
	const actions = [
		{ label: 'Edit Shipping', icon: Pencil, onClick: (id: string) => console.log('Edit', id) },
		{ label: 'Duplicate', icon: Copy, onClick: (id: string) => console.log('Duplicate', id) },
		{ label: 'Calculate Rate', icon: Truck, onClick: (id: string) => console.log('Calculate', id) },
	];

	const labels = {
		dimensions: 'Dimensions',
		weight: 'Weight',
		packageTypes: { box: 'Box', envelope: 'Envelope', tube: 'Tube', custom: 'Custom' },
		shippingClasses: { standard: 'Standard', fragile: 'Fragile', oversized: 'Oversized', hazmat: 'Hazmat' },
