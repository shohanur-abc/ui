...
const TrendIndicator = ({ value }: TrendIndicatorProps) => {
	...
	return (
		<div
			className={`flex items-center gap-1 text-xs ${isPositive ? 'text-emerald-500' : 'text-red-500'}`}
		>
			{isPositive ? (
...
const ProductRow = ({
	...
	>
		...
		<div className="hidden @sm:block">
			<span
				className={`text-sm ${product.stock === 0 ? 'text-red-500' : product.stock < 10 ? 'text-amber-500' : ''}`}
			>
				{product.stock}
			</span>
		</div>
