...
const RevenueSummary = ({
	...
	const trendColor =
		revenue.trend === 'up' ? 'text-accent' : 'text-destructive';
		...
		<Card className="border-border/50 bg-card/50 backdrop-blur-sm">
			...
			<CardContent className="space-y-6">
				<div className="p-6 rounded-xl bg-primary/5 border border-primary/20 text-center">
					...
					<div
						className={`flex items-center justify-center gap-1 mt-2 ${trendColor}`}
					>
				...
				<div>
					...
					<div className="space-y-3">
						{breakdown.map((item, i) => {
							...
							const isNegative =
								item.icon === 'returns' || item.icon === 'discounts';
							return (
								<div
									key={i}
								...
								>
									<div className="flex items-center gap-3">
										<Icon
											className={`size-5 ${isNegative ? 'text-destructive' : 'text-muted-foreground'}`}
										/>
										<span>{item.label}</span>
									</div>
									<div className="text-right">
										<p
											className={`font-semibold ${isNegative ? 'text-destructive' : ''}`}
										>
											{isNegative ? '-' : ''}
											{item.value}
										</p>
