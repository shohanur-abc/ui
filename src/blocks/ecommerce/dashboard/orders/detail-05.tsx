...
const StatusBadge = ({
	...
	status: PaymentDetailProps['payment']['status'];
}) => {
	const config = {
		completed: {
			icon: CheckCircle,
		...
		<CardContent className="space-y-6">
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Processed {payment.timestamps.processed}
							</p>
						</div>
					</div>
				</div>
			...
			<div className="p-4 rounded-xl bg-muted/20 border border-border/50">
				...
				<div className="flex items-center gap-6">
					<SecurityCheck
						label="AVS Match"
						passed={payment.securityChecks.avs}
					/>
					<SecurityCheck
						label="CVV Check"
						passed={payment.securityChecks.cvv}
					/>
					<SecurityCheck
						label="3D Secure"
						passed={payment.securityChecks.threeDSecure}
					/>
				</div>
			</div>
...
export default function Main() {
	...
	const payment = {
		...
		timestamps: {
			created: 'Jan 26, 2024 9:00 PM',
			processed: 'Jan 26, 2024 9:02 PM',
		},
