...
const CustomsStatusBadge = ({
	...
	status: InternationalOrder['customsStatus'];
}) => {
	const config: Record<
		InternationalOrder['customsStatus'],
...
const CountryFlag = ({ flag, name }: { flag: string; name: string }) => (
	<div
		className="size-8 rounded-lg bg-muted/50 border border-border/50 flex items-center justify-center text-lg"
		title={name}
	>
		{flag}
	</div>
...
const InternationalOrderRow = ({
	order,
	labels,
}: InternationalOrderRowProps) => (
