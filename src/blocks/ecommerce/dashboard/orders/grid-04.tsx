		...
		<Card className="border-border/50 bg-card/50 backdrop-blur-sm hover:border-primary/30 hover:shadow-md transition-all">
			<CardContent className="p-4">
				<div className="flex items-start justify-between mb-3">
					<div
						className={`size-10 rounded-lg flex items-center justify-center ${stat.iconBgColor}`}
					>
						<stat.icon className={`size-5 ${stat.iconColor}`} />
					</div>
					{TrendIcon && (
						<div
							className={`flex items-center gap-1 text-sm ${changeColors[stat.changeType]}`}
						>
