	...
	return (
		<div className={`flex gap-4 ${isCustomer ? '' : 'flex-row-reverse'}`}>
			...
			<div className={`flex-1 max-w-[80%] ${isLast ? '' : 'pb-6'}`}>
				<div
					className={`p-4 rounded-xl ${isCustomer ? 'bg-muted/30 border border-border/50' : 'bg-primary/10 border border-primary/20'}`}
				>
	...
	const interactions: SupportInteraction[] = [
		{
			...
			type: 'message',
			direction: 'outbound',
			agent: { name: 'Sarah Support', avatar: '', initials: 'SS' },
			content:
				'Hi! Your refund has been processed and should appear in your account within 3-5 business days. Is there anything else I can help you with?',
			timestamp: '2:45 PM',
		},
		{
			id: '2',
			...
			direction: 'inbound',
			customer: { name: 'John' },
			content: 'Thanks! When will I receive my refund?',
			timestamp: '2:40 PM',
		},
		{
			id: '3',
			...
			agent: { name: 'Sarah Support', avatar: '', initials: 'SS' },
			content:
				'Internal note: Processed refund for $89.99. Customer satisfied.',
			timestamp: '2:38 PM',
		},
		{
			id: '4',
			type: 'call',
			direction: 'outbound',
			agent: { name: 'Mike Agent', avatar: '', initials: 'MA' },
			content:
				'Called customer to clarify return process. Customer confirmed they will ship the item back today.',
			timestamp: '11:30 AM',
		},
		{
			id: '5',
			...
			customer: { name: 'John' },
			content:
				'I received the wrong item in my order #ORD-2024-156. I ordered black headphones but received blue ones. Please help.',
			timestamp: 'Yesterday',
		},
	];
