...
const StatusIndicator = ({ status }: { status: Subscription['status'] }) => (
	<span
		className={`inline-flex items-center gap-1.5 text-sm ${status === 'active' ? 'text-accent' : 'text-muted-foreground'}`}
	>
		<span
			className={`size-2 rounded-full ${status === 'active' ? 'bg-accent' : 'bg-muted-foreground'}`}
		/>
		...
		<CardContent className="p-4">
			...
			<div className="flex items-center gap-4 mt-4 pt-4 border-t border-border/50 text-sm">
				<div className="flex items-center gap-1.5 text-muted-foreground">
					...
					<span>
						{labels.nextDelivery}: {subscription.nextDelivery}
					</span>
				</div>
				<div className="flex items-center gap-1.5 text-muted-foreground">
					<Package className="size-4" />
					<span>
						{subscription.deliveriesCompleted} {labels.deliveries}
					</span>
				</div>
			</div>
	...
	const subscriptions: Subscription[] = [
		{
			...
			product: 'Coffee Bean Subscription - Dark Roast',
			frequency: 'biweekly',
			amount: '$29.99',
			nextDelivery: 'Feb 3',
			status: 'active',
		...
		},
		{
			...
			product: 'Vitamin Pack - Complete Daily',
			frequency: 'monthly',
			amount: '$49.99',
			nextDelivery: 'Feb 1',
			status: 'active',
		...
		},
		{
			...
			product: 'Snack Box - Healthy Mix',
			frequency: 'weekly',
			amount: '$14.99',
			nextDelivery: 'On Hold',
			status: 'paused',
