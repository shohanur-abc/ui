...
const ProductHeader = ({ brand, name }: ProductHeaderProps) => (
	<div className="space-y-1">
		<Link
			href="#brand"
			className="text-sm text-primary font-medium hover:underline uppercase tracking-wider"
		>
			{brand}
		</Link>
...
const RatingWithPhotos = ({
	rating,
	reviews,
	photos,
}: RatingWithPhotosProps) => (
...
const QuickSpecs = ({ specs }: QuickSpecsProps) => (
	<div className="grid grid-cols-2 gap-x-6 gap-y-2">
		{specs.map((spec, i) => (
			<div
				key={i}
				className="flex justify-between py-1.5 border-b border-muted text-sm"
			>
				...
				<div className="grid @lg:grid-cols-2 gap-8 @xl:gap-12">
					...
					<div className="flex flex-col gap-5">
						...
						<PriceBlock
							price="$189"
							originalPrice="$229"
							memberPrice="$169.99 with membership"
						/>
