	...
	<div className="flex flex-col gap-4">
		...
		<div className="grid grid-cols-4 gap-2">
			{images.slice(0, 4).map((img, i) => (
				<div
					key={i}
					className="relative aspect-square overflow-hidden rounded-lg bg-muted/30 cursor-pointer ring-2 ring-transparent hover:ring-primary transition-all"
				>
	...
	<div className="flex items-center gap-3">
		<div className="flex items-center gap-0.5">
			{Array.from({ length: 5 }).map((_, i) => (
				<Star
					key={i}
					className={`size-4 ${i < value ? 'fill-yellow-400 text-yellow-400' : 'fill-muted text-muted'}`}
				/>
			))}
		</div>
...
const SizeOptions = ({ sizes }: SizeOptionProps) => (
	<div className="space-y-3">
		...
		<RadioGroup
			...
			defaultValue={sizes.find((s) => s.available)?.label}
		>
			{sizes.map((size, i) => (
				<label
					key={i}
				...
				<div className="grid @lg:grid-cols-2 gap-8 @xl:gap-16">
					...
					<ProductGallery
						images={[
							...
							{
								src: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=800',
								alt: 'Red sneaker top view',
							},
							{
								src: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=800',
								alt: 'Red sneaker back view',
							},
							{
								src: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=800',
