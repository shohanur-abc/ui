...
import { Star, ShoppingCart, Heart, Flame, Users, Clock, ChefHat, type LucideIcon } from "lucide-react"
...
const HeroImage = ({ src, alt, badges }: HeroImageProps) => (
  <div className="relative aspect-square overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50 to-orange-100 dark:from-amber-950/30 dark:to-orange-950/30">
    ...
    <div className="absolute top-4 left-4 flex flex-wrap gap-2">
      {badges.map((badge, i) => (
        <Badge key={i} className="bg-primary/90 backdrop-blur-sm">{badge}</Badge>
      ))}
    </div>
  </div>
)
...
const ProductInfo = ({ category, name, description }: ProductInfoProps) => (
  <div className="space-y-2">
    <p className="text-sm text-primary font-medium uppercase tracking-wider">{category}</p>
...
const NutritionCard = ({ icon: Icon, label, value, percentage }: NutritionCardProps) => (
        ...
        <div className="grid @lg:grid-cols-12 gap-4">
          ...
          <div className="@lg:col-span-7 flex flex-col gap-4 p-6 rounded-xl bg-card border">
            ...
            <div className="grid grid-cols-2 gap-3">
              <QuantityPicker label="Servings" options={[1, 2, 4, 6]} selected={2} />
              <SubscriptionOption
                options={[
          ...
          <div className="@lg:col-span-7 grid @sm:grid-cols-2 @lg:grid-cols-4 gap-3">
            <NutritionCard icon={Flame} label="Calories" value="450" percentage={23} />
            <NutritionCard icon={Users} label="Protein" value="35g" percentage={70} />
            <NutritionCard icon={Clock} label="Prep Time" value="15min" percentage={15} />
            <NutritionCard icon={ChefHat} label="Skill Level" value="Easy" percentage={30} />
          </div>
        </div>
      </div>
    </section>
