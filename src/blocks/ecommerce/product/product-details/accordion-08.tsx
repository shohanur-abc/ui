...
import { Star, ShoppingCart, Heart, Leaf, Award, Globe, type LucideIcon } from "lucide-react"
...
interface SustainabilityAccordionProps {
  sections: { id: string; icon: LucideIcon; title: string; content: string; stats?: { label: string; value: string }[] }[]
}
...
const ProductImage = ({ src, alt, badges }: ProductImageProps) => (
  <div className="relative aspect-square overflow-hidden rounded-2xl bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-950/30 dark:to-emerald-950/30">
    ...
    <div className="absolute top-4 left-4 flex flex-wrap gap-2">
      {badges.map((badge, i) => (
        <Badge key={i} variant={badge.variant || "default"}>{badge.text}</Badge>
      ))}
    </div>
  </div>
)
...
const SustainabilityAccordion = ({ sections }: SustainabilityAccordionProps) => (
              ...
              <div className="grid grid-cols-2 gap-2">
                {section.stats.map((stat, i) => (
                  <div key={i} className="p-2 rounded-lg bg-green-50 dark:bg-green-900/20">
                    ...
                    <p className="text-sm font-medium text-green-700 dark:text-green-300">{stat.value}</p>
                  </div>
                ))}
              </div>
            )}
              ...
              sections={[
                {
                  ...
                  content: "This jacket is made with 100% recycled polyester ripstop shell and lining. The PrimaLoft Gold Insulation Eco is made with 55% post-consumer recycled content.",
                  stats: [
                ...
                },
                {
                  ...
                  content: "bluesign® approved fabric means it was made using only safe chemicals and processes that ensure clean air, clean water, and worker safety throughout the supply chain.",
                  stats: [
                ...
                },
                {
                  ...
                  content: "We track this jacket from raw materials to finished product. It was sewn in a Fair Trade Certified™ factory, which means the workers who made it earned a premium for their labor.",
                },
              ]}
            />
