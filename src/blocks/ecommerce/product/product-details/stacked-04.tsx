...
import { Star, ShoppingCart, Heart, Clock, Flame, Leaf, type LucideIcon } from "lucide-react"
...
const NutritionFacts = ({ facts }: NutritionFactsProps) => (
  <div className="space-y-3">
    <p className="font-medium text-center text-sm uppercase tracking-wide text-muted-foreground">Nutrition per serving</p>
    <div className="grid grid-cols-2 @sm:grid-cols-4 gap-3">
      {facts.map((fact, i) => (
        <Card key={i} className="bg-muted/30 border-muted text-center">
          <CardContent className="p-4">
            {fact.icon && <fact.icon className="size-5 mx-auto mb-2 text-primary" />}
            <p className="text-2xl font-bold">{fact.value}<span className="text-sm font-normal text-muted-foreground ml-1">{fact.unit}</span></p>
...
const Ingredients = ({ ingredients }: IngredientsProps) => (
  <div className="space-y-3">
    <p className="font-medium text-center text-sm uppercase tracking-wide text-muted-foreground">Ingredients</p>
...
const DietaryBadges = ({ badges }: DietaryBadgesProps) => (
  <div className="flex flex-wrap justify-center gap-3">
    {badges.map((badge, i) => (
      <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-full bg-green-100 dark:bg-green-900/30">
        {badge.icon && <badge.icon className="size-4 text-green-600 dark:text-green-400" />}
        <span className="text-sm font-medium text-green-700 dark:text-green-300">{badge.text}</span>
      </div>
    ))}
  </div>
)
        ...
        <div className="flex flex-col gap-10">
          ...
          <Actions
            buttons={[
              ...
              { label: "Save Recipe", href: "#wishlist", icon: Heart, variant: "outline" },
            ]}
          />
        </div>
      </div>
