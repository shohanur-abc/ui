...
import { Star, ShoppingCart, Heart, Check, Truck, Shield, RotateCcw, type LucideIcon } from "lucide-react"
...
const ProductPrice = ({ current, original, discount }: ProductPriceProps) => (
  <div className="flex items-center gap-3 flex-wrap">
    <span className="text-2xl @sm:text-3xl font-bold text-primary">{current}</span>
    {original && (
...
const ProductCTA = ({ items }: ProductCTAProps) => (
  <div className="flex flex-wrap gap-3">
    {items.map((item, i) => (
      <Button key={i} variant={item.variant || "default"} size="lg" className="gap-2" asChild>
        <Link href={item.href}>
