...
import { Star, ShoppingCart, Heart, Dumbbell, Timer, Flame, Trophy, type LucideIcon } from "lucide-react"
...
const Price = ({ current, original, servings }: PriceProps) => (
  <div className="space-y-1">
    ...
    <p className="text-sm text-muted-foreground">{servings} servings • ${(parseFloat(current.replace('$', '')) / servings).toFixed(2)}/serving</p>
  </div>
)
...
const NutritionFacts = ({ facts }: NutritionFactsProps) => (
  <div className="space-y-3">
    ...
    {facts.map((fact, i) => (
      <div key={i} className="flex items-center justify-between py-1 border-b border-muted">
        <span className="text-sm">{fact.label}</span>
        <div className="flex items-center gap-2">
          <span className="font-medium text-sm">{fact.value}</span>
          {fact.daily && <span className="text-xs text-muted-foreground">{fact.daily}</span>}
        </div>
      </div>
    ))}
  </div>
      ...
      <Card key={i} className="bg-muted/30 border-muted">
        <CardContent className="p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">{item.instruction}</p>
          </div>
        </CardContent>
      </Card>
    ))}
          ...
          <div className="flex flex-col gap-5">
            ...
            <Tabs defaultValue="nutrition" className="w-full">
              ...
              <TabsContent value="ingredients" className="mt-4">
                <IngredientBreakdown
                  ingredients={[
                    { name: "Whey Protein Isolate", amount: "25g", benefit: "Fast-absorbing complete protein with all essential amino acids." },
                    { name: "BCAAs (Leucine, Isoleucine, Valine)", amount: "5.5g", benefit: "Supports muscle recovery and reduces post-workout soreness." },
                    { name: "L-Glutamine", amount: "4g", benefit: "Aids immune function and gut health during intense training." },
                  ]}
                />
              </TabsContent>
              ...
              <TabsContent value="usage" className="mt-4">
                <UsageGuide
                  timing={[
                    { icon: Timer, time: "Pre-Workout (30 min before)", instruction: "Mix 1 scoop with 8oz water. Provides sustained energy for your workout." },
                    { icon: Dumbbell, time: "Post-Workout (within 30 min)", instruction: "Mix 1-2 scoops with water or milk for optimal muscle recovery." },
                    { icon: Flame, time: "Morning Shake", instruction: "Blend with fruits and oats for a high-protein breakfast." },
                    { icon: Trophy, time: "Competition Prep", instruction: "Increase to 2 scoops during cutting phases to preserve lean mass." },
                  ]}
                />
              </TabsContent>
            </Tabs>
