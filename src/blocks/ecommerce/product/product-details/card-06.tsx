...
interface SpecsCardProps {
  title: string
  specs: { label: string; value: number; max: number }[]
}
...
const SpecsCard = ({ title, specs }: SpecsCardProps) => (
      ...
      {specs.map((spec, i) => (
        <div key={i} className="space-y-1">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-muted-foreground">{spec.value}/{spec.max}</span>
          </div>
    ...
    <section className="@container relative overflow-hidden" data-theme="neon">
      <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
        <div className="grid @lg:grid-cols-2 gap-8">
          ...
          {/* Details with specs card */}
          <div className="flex flex-col gap-5">
            <Header brand="Adidas" name="Ultraboost Light" />

            <Rating rating={5} reviews={6543} />

            <Price current="$190" original="$210" discount="-10%" />

            <Separator />

            <p className="text-muted-foreground leading-relaxed">
              Our lightest Ultraboost ever, designed for runners who want to go the distance. Featuring Light BOOST midsole technology for incredible energy return with 30% less weight.
            </p>

            <SpecsCard
              title="Performance Ratings"
              specs={[
                { label: "Cushioning", value: 9, max: 10 },
                { label: "Energy Return", value: 10, max: 10 },
                { label: "Breathability", value: 8, max: 10 },
                { label: "Durability", value: 8, max: 10 },
                { label: "Weight", value: 9, max: 10 },
              ]}
            />
