...
import { Star, ShoppingCart, Heart, Gamepad2, Cpu, Monitor, Zap, type LucideIcon } from "lucide-react"
...
interface EditionSelectorProps {
  editions: { name: string; price: string; features: string; selected?: boolean }[]
}
...
const PriceDisplay = ({ price, originalPrice, discount }: PriceDisplayProps) => (
      ...
      {editions.map((edition, i) => (
        <button
          ...
          className={`p-4 text-left border rounded-xl transition-all ${
            edition.selected ? "border-primary bg-primary/10" : "hover:border-primary"
          }`}
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">{edition.features}</p>
        </button>
      ))}
    </div>
  </div>
      ...
      <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
        <div className="grid @lg:grid-cols-12 gap-6">
          ...
          <div className="@lg:col-span-5 @lg:row-span-2 flex flex-col gap-5 p-6 rounded-xl bg-card border">
            ...
            <PriceDisplay price="$59.99" originalPrice="$69.99" discount="-15%" />

            <Separator />

            <EditionSelector
              editions={[
                { name: "Standard Edition", price: "$59.99", features: "Base game + Day One patch", selected: true },
                { name: "Deluxe Edition", price: "$79.99", features: "Base game + Season Pass + Bonus content" },
                { name: "Ultimate Edition", price: "$99.99", features: "Everything + Collector's items + Early access" },
              ]}
            />

            <PurchaseButtons
              primary={{ label: "Buy Now", href: "#checkout", icon: ShoppingCart }}
              secondary={{ label: "Wishlist", href: "#wishlist", icon: Heart }}
            />

            <FeatureTags
              tags={["Open World", "RPG", "Multiplayer", "Co-op", "Ray Tracing"]}
            />
          </div>

          {/* System Requirements */}
          <div className="@lg:col-span-7 grid grid-cols-2 @sm:grid-cols-4 gap-3">
            <SpecBadge icon={Cpu} label="Processor" value="Intel i7 / Ryzen 7" />
            <SpecBadge icon={Monitor} label="Graphics" value="RTX 3070 / RX 6800" />
