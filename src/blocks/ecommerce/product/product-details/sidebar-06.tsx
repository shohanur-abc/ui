...
const RatingBreakdownSidebar = ({ title, overall, total, breakdown }: RatingBreakdownSidebarProps) => (
      ...
      <div className="text-center pb-4 border-b border-border">
        ...
        <div className="flex justify-center mt-1">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star key={i} className={`size-4 ${i < overall ? "fill-yellow-400 text-yellow-400" : "fill-muted text-muted"}`} />
          ))}
        </div>
      ...
      <div className="space-y-2">
        {breakdown.map((item, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground w-10">{item.count}</span>
          </div>
        ))}
      </div>
    </CardContent>
        ...
        <div className="grid @lg:grid-cols-[1fr_300px] gap-8">
          ...
          <div className="grid @md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-5">
              ...
              <Description
                text="Precision pour-over kettle with variable temperature control and built-in stopwatch. The iconic Stagg design combined with studio-grade technology for the perfect cup."
              />
