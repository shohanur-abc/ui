...
const ProgressCarousel = ({ images, currentIndex, total }: ProgressCarouselProps) => (
  <div className="space-y-4">
    <div className="relative aspect-square overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50 to-orange-100 dark:from-amber-950/30 dark:to-orange-950/30">
      ...
      <div className="absolute bottom-4 left-4 right-4 flex items-center gap-4">
        <Button variant="outline" size="icon" className="bg-background/80 backdrop-blur-sm shrink-0">
          <ChevronLeft className="size-4" />
        </Button>
        <Progress value={((currentIndex + 1) / total) * 100} className="flex-1 h-1" />
        <Button variant="outline" size="icon" className="bg-background/80 backdrop-blur-sm shrink-0">
  ...
  <div className="space-y-3">
    ...
    <div className="space-y-2">
      {options.map((option, i) => (
        <Card key={i} className={`cursor-pointer transition-colors ${option.selected ? "border-primary bg-primary/5" : "border-muted hover:border-primary/50"}`}>
          <CardContent className="p-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`size-4 rounded-full border-2 ${option.selected ? "border-primary bg-primary" : "border-muted"}`}>
                {option.selected && <div className="size-2 m-0.5 rounded-full bg-primary-foreground" />}
              </div>
    ...
    <section className="@container relative overflow-hidden" data-theme="neon">
      <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
        <div className="grid @lg:grid-cols-2 gap-8 @xl:gap-12">
          ...
          <ProgressCarousel
            images={[
              { src: "https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=800", alt: "Vitamin bottle front" },
              { src: "https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=800", alt: "Vitamin bottle back" },
              { src: "https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=800", alt: "Vitamin capsules" },
            ]}
            currentIndex={0}
            total={3}
          />
          ...
          <div className="flex flex-col gap-5">
            ...
            <Subscription
              options={[
                { label: "Subscribe & Save", discount: "Save 20%", selected: true },
            ...
            <Actions
              buttons={[
                { label: "Start Subscription", href: "#cart", icon: ShoppingCart },
