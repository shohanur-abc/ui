...
import { Star, ShoppingCart, Heart, BookOpen, Clock, User, Award, type LucideIcon } from "lucide-react"
  ...
  <Card className="bg-muted/30 border-muted">
    <CardContent className="p-6 flex gap-4">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground leading-relaxed mt-1">{bio}</p>
      </div>
    </CardContent>
  </Card>
)
...
const Formats = ({ formats }: FormatsProps) => (
  <div className="space-y-3">
    <p className="font-medium text-center text-sm uppercase tracking-wide text-muted-foreground">Available Formats</p>
    <div className="flex justify-center gap-3">
      {formats.map((format, i) => (
        <Button key={i} variant={format.selected ? "default" : "outline"} className="flex-col h-auto py-3 px-6">
