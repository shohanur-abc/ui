...
import { Star, ShoppingCart, Heart, Share2, ChevronLeft, ChevronRight, Check, type LucideIcon } from "lucide-react"
...
const TitleBlock = ({ title, subtitle }: TitleBlockProps) => (
  <div className="space-y-1">
    <h1 className="text-2xl @sm:text-3xl @xl:text-4xl font-bold tracking-tight leading-tight">{title}</h1>
    ...
    <div className="text-center @sm:text-left">
      ...
      <div className="flex items-center justify-center @sm:justify-start gap-0.5 my-1">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star key={i} className={`size-4 ${i < Math.floor(average) ? "fill-yellow-400 text-yellow-400" : "fill-muted text-muted"}`} />
        ))}
      </div>
      <p className="text-sm text-muted-foreground">{total.toLocaleString()} reviews</p>
    </div>
...
const OptionSelector = ({ title, options, selected }: OptionSelectorProps) => (
  <div className="space-y-3">
    <span className="text-sm font-medium">{title}: <span className="text-muted-foreground">{selected}</span></span>
...
const ActionBar = ({ actions }: ActionBarProps) => (
  <div className="flex gap-2">
    {actions.map((action, i) => (
      <Button key={i} variant={action.variant || "default"} size="lg" className="flex-1 gap-2" asChild>
      ...
      <div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
        <div className="grid @lg:grid-cols-2 gap-8 @xl:gap-12">
          ...
          <div className="space-y-4">
            <CarouselImage
              images={[
                { src: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800", alt: "Headphones front view" },
                { src: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800", alt: "Headphones side view" },
                { src: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800", alt: "Headphones back view" },
                { src: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800", alt: "Headphones detail" },
              ]}
              currentIndex={0}
            />
