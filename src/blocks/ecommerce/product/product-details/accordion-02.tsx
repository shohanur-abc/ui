	...
	<div className="space-y-3">
		...
		<div className="grid grid-cols-4 gap-2">
			{thumbnails.map((thumb, i) => (
				<div
					key={i}
					className={`relative aspect-square overflow-hidden rounded-lg bg-muted ring-2 transition-all cursor-pointer ${i === 0 ? 'ring-primary' : 'ring-transparent hover:ring-primary/50'}`}
				>
					<Image
						src={thumb.src}
						alt={thumb.alt}
				...
				<div className="text-center">
					...
					<div className="flex gap-0.5 my-1">
						{Array.from({ length: 5 }).map((_, i) => (
							<Star
								key={i}
								className={`size-3 ${i < Math.floor(average) ? 'fill-yellow-400 text-yellow-400' : 'fill-muted text-muted'}`}
							/>
						))}
					</div>
		...
		<section className="@container relative overflow-hidden" data-theme="neon">
			<div className="max-w-7xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
				<div className="grid @lg:grid-cols-2 gap-8 @xl:gap-12">
					...
					<Gallery
						main={{
							src: 'https://images.unsplash.com/photo-1610725664285-7c57e6eeac3f?w=800',
							alt: 'Protein powder',
						}}
						thumbnails={[
							{
								src: 'https://images.unsplash.com/photo-1610725664285-7c57e6eeac3f?w=200',
								alt: 'Front',
							},
							{
								src: 'https://images.unsplash.com/photo-1610725664285-7c57e6eeac3f?w=200',
