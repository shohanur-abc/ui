...
import { Star, ShoppingCart, Heart, MapPin, Calendar, Users, type LucideIcon } from "lucide-react"
...
const HeroImage = ({ src, alt, badges }: HeroImageProps) => (
  <div className="relative aspect-video overflow-hidden rounded-2xl bg-muted">
    ...
    <div className="absolute top-4 left-4 flex gap-2">
      {badges.map((badge, i) => (
        <Badge key={i} variant={i === 0 ? "destructive" : "default"}>{badge}</Badge>
      ))}
    </div>
  </div>
)
...
const IncludedItems = ({ items }: IncludedItemsProps) => (
  <div className="space-y-3">
    <p className="font-medium text-center text-sm uppercase tracking-wide text-muted-foreground">What&apos;s Included</p>
...
const Description = ({ text }: DescriptionProps) => (
  <p className="text-muted-foreground leading-relaxed text-center max-w-3xl mx-auto text-lg">{text}</p>
)
    ...
    <section className="@container relative overflow-hidden" data-theme="neon">
      <div className="max-w-5xl mx-auto px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20">
        <div className="flex flex-col gap-10">
          ...
          {/* Description */}
          <Description
            text="Discover the magic of Bali on this unforgettable 7-day adventure. From ancient temples to pristine beaches, lush rice terraces to vibrant nightlife, experience the best of this tropical paradise. Our expert local guides will take you off the beaten path to hidden gems most tourists never see."
          />
          ...
          <Actions
            buttons={[
              ...
              { label: "Save Trip", href: "#wishlist", icon: Heart, variant: "outline" },
            ]}
          />
        </div>
      </div>
