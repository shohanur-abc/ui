...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
	<div className="relative mx-auto size-24 @sm:size-28">
		...
		<Image
			...
			alt={alt}
			fill
			className="relative rounded-full object-cover ring-4 ring-primary/20"
		/>
	</div>
);
...
export default function Main() {
	const subscription: SubscriptionProps = {
		image:
			'https://images.unsplash.com/photo-1614850715649-1d0106293bd1?w=200&h=200&fit=crop',
		name: 'Premium Plan',
	...
	return (
		<section className="@container">
			<div className="mx-auto max-w-sm px-4 py-8">
				<Card
					className={`relative space-y-6 p-6 ${subscription.popular ? 'border-primary ring-2 ring-primary/20' : ''}`}
				>
