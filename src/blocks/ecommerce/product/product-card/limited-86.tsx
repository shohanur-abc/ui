...
import { Clock, Crown, Heart, Lock, ShoppingCart, Star, Sparkles, Users } from "lucide-react"
...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
    <div className="relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-slate-900 to-slate-800">
        <Image src={src} alt={alt} fill className="object-cover opacity-90 transition-transform duration-500 group-hover:scale-105" />
...
const AvailabilityProgress = ({ remaining, total }: { remaining: number; total: number }) => {
    ...
    return (
        <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
                ...
                <span className={`font-medium ${percent < 20 ? "text-destructive" : "text-foreground"}`}>
                ...
                <Card className="group overflow-hidden border-amber-500/30 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
                    ...
                    <div className="space-y-3 p-4">
                        <div className="flex items-center justify-between">
                            ...
                            <ProductRating rating={limited.rating} reviews={limited.reviews} />
                        </div>
                        <div className="space-y-0.5">
                            <ProductName text={limited.name} />
                            <EditionInfo edition={limited.edition} />
                        </div>
                        <AvailabilityProgress remaining={limited.remainingUnits} total={limited.totalUnits} />
