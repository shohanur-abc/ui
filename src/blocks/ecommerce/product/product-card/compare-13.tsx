...
const ProductPrice = ({ amount }: { amount: number }) => (
    <p className="text-center text-2xl font-bold text-primary">${amount.toFixed(0)}</p>
)

const FeatureList = ({ items }: { items: { label: string; available: boolean }[] }) => (
...
const SelectButton = ({ label, highlighted }: { label: string; highlighted?: boolean }) => (
    <Button variant={highlighted ? "default" : "outline"} className="w-full gap-2">
        {label}
...
const HighlightBadge = ({ text }: { text: string }) => (
    <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground">{text}</Badge>
)
