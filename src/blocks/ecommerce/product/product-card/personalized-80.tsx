...
import { Heart, PenTool, ShoppingCart, Star, Clock, Type, Sparkles } from "lucide-react"
...
const TypeSelector = ({ types, selected }: { types: { name: string; price: number }[]; selected: number }) => (
...
const TextPreview = ({ text, maxChars }: { text: string; maxChars: number }) => (
    <div className="space-y-2">
...
const PriceDisplay = ({ base, extra }: { base: number; extra: number }) => (
    <div className="space-y-0.5">
        <span className="text-xl font-bold text-foreground">${(base + extra).toFixed(2)}</span>
...
export default function Main() {
    const personalized: PersonalizedProps = {
        image: "https://images.unsplash.com/photo-1522273400909-fd1a8f77637e?w=600&h=450&fit=crop",
                ...
                <Card className="group overflow-hidden">
                    ...
                    <div className="space-y-3 p-4">
                        <div className="flex items-center justify-between">
                            ...
                            <ProductRating rating={personalized.rating} reviews={personalized.reviews} />
                        </div>
                        <ProductName text={personalized.name} />
                        <TypeSelector types={personalized.personalizationTypes} selected={personalized.selectedType} />
                        <FontSelector fonts={personalized.fonts} />
                        <TextPreview text={personalized.preview} maxChars={personalized.maxChars} />
                        <Separator />
                        <div className="flex items-center justify-between">
                            <PriceDisplay 
                                base={personalized.basePrice} 
                                extra={personalized.personalizationTypes[personalized.selectedType].price} 
                            />
