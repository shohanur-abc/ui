...
import { Heart, ShoppingCart, Star, Package2, Check, Box, Plus, Minus } from "lucide-react"
...
const PriceComparison = ({ single, pack, size }: { single: number; pack: number; size: number }) => {
    const perUnit = pack / size
    const savings = Math.round(((single - perUnit) / single) * 100)
    return (
        <div className="space-y-1">
            <div className="flex items-baseline gap-2">
                <span className="text-xl font-bold text-foreground">${pack.toFixed(2)}</span>
                <span className="text-sm text-muted-foreground">(${perUnit.toFixed(2)} each)</span>
            </div>
            <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground line-through">${single.toFixed(2)} each</span>
                ...
                <Card className="group overflow-hidden">
                    ...
                    <div className="space-y-3 p-4">
                        <div className="flex items-center justify-between">
                            ...
                            <ProductRating rating={multipack.rating} reviews={multipack.reviews} />
                        </div>
                        ...
                        <Separator />
                        <PriceComparison single={multipack.singlePrice} pack={multipack.packPrice} size={multipack.packSize} />
