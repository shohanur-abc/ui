...
import { Cpu, Heart, HardDrive, ShoppingCart, Star, Monitor, Zap, Gauge } from "lucide-react"
...
const HardwareRating = ({ rating, reviews }: { rating: number; reviews: number }) => (
...
const SpecsGrid = ({ specs }: { specs: { icon: string; label: string; value: string }[] }) => {
        ...
        <div className="grid grid-cols-2 gap-2">
            {specs.map((spec, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-2">
                    <span className="text-primary">{iconMap[spec.icon] || <Cpu className="size-4" />}</span>
                ...
                <Card className="group overflow-hidden border-primary/20">
                    ...
                    <div className="space-y-3 p-4">
                        <div className="flex items-center justify-between">
                            ...
                            <HardwareRating rating={hardware.rating} reviews={hardware.reviews} />
                        </div>
