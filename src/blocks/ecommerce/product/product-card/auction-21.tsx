...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
    <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-muted">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-4">
            <Badge variant="secondary" className="gap-1.5 bg-white/10 text-white backdrop-blur-sm">
...
const TimeRemaining = ({ hours, minutes, seconds }: { hours: number; minutes: number; seconds: number }) => (
    <div className="flex items-center gap-2">
        <Clock className="size-4 text-destructive" />
        <div className="flex gap-1 font-mono text-sm">
            <span className="rounded bg-destructive/10 px-1.5 py-0.5 text-destructive">{hours.toString().padStart(2, "0")}h</span>
            <span className="rounded bg-destructive/10 px-1.5 py-0.5 text-destructive">{minutes.toString().padStart(2, "0")}m</span>
            <span className="rounded bg-destructive/10 px-1.5 py-0.5 text-destructive">{seconds.toString().padStart(2, "0")}s</span>
        </div>
    </div>
)
