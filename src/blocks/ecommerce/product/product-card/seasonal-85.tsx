...
import { Calendar, Heart, Leaf, ShoppingCart, Snowflake, Star, Sun, Clock } from "lucide-react"
...
const SeasonBadge = ({ season, icon }: { season: string; icon: string }) => {
    ...
    return (
        <Badge className={`absolute left-3 top-3 gap-1 ${colors[icon] || "bg-primary"}`}>
    ...
    <div className="rounded-lg bg-destructive/10 p-3">
        <div className="flex items-center justify-center gap-2 text-sm">
            ...
            <div className="flex items-center gap-1">
                <span className="rounded bg-destructive px-1.5 py-0.5 font-mono font-bold text-destructive-foreground">{days}d</span>
                <span className="rounded bg-destructive px-1.5 py-0.5 font-mono font-bold text-destructive-foreground">{hours}h</span>
            </div>
        </div>
    </div>
)
...
const StockIndicator = ({ percent }: { percent: number }) => (
    <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
            ...
            <span className={percent < 30 ? "text-destructive" : "text-muted-foreground"}>
                {percent}% remaining
            </span>
        </div>
                ...
                <Card className="group overflow-hidden">
                    ...
                    <div className="space-y-3 p-4">
                        <div className="flex items-center justify-between">
                            ...
                            <ProductRating rating={seasonal.rating} reviews={seasonal.reviews} />
                        </div>
                        <ProductName text={seasonal.name} />
                        <CountdownTimer days={seasonal.endsIn.days} hours={seasonal.endsIn.hours} />
                        {seasonal.limitedStock && <StockIndicator percent={seasonal.stockPercent} />}
                        <Separator />
                        <div className="flex items-center justify-between">
                            <PriceDisplay price={seasonal.price} original={seasonal.originalPrice} />
                            <AddButton label="Add" />
                        </div>
                    </div>
                </Card>
