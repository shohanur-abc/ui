...
const FundingProgress = ({ pledged, goal }: { pledged: number; goal: number }) => {
    const percent = Math.min((pledged / goal) * 100, 100)
    return (
        <div className="space-y-2">
            <Progress value={percent} className="h-3" />
            <div className="flex justify-between text-sm">
                <span className="font-semibold text-primary">${pledged.toLocaleString()} raised</span>
                <span className="text-muted-foreground">{percent.toFixed(0)}% of ${goal.toLocaleString()}</span>
            </div>
        </div>
    )
}

const CampaignStats = ({ backers, daysLeft }: { backers: number; daysLeft: number }) => (
    <div className="flex gap-6">
        <div className="flex items-center gap-2">
            <Users className="size-5 text-primary" />
            <div>
                <p className="font-semibold text-foreground">{backers.toLocaleString()}</p>
        ...
        <section className="@container">
            <div className="mx-auto max-w-md px-4 py-8">
                <div className="space-y-5 rounded-2xl border border-border bg-card p-5">
                    ...
                    <CampaignStats backers={campaign.backers} daysLeft={campaign.daysLeft} />
