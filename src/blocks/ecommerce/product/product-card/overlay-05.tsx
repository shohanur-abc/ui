...
const ProductInfo = ({ name, designer, rating }: { name: string; designer: string; rating: number }) => (
...
const PriceTag = ({ amount, discount }: { amount: number; discount?: number }) => {
    const finalPrice = discount ? amount * (1 - discount / 100) : amount
    return (
        <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">${finalPrice.toFixed(0)}</span>
            {discount && <span className="text-sm text-white/50 line-through">${amount}</span>}
        </div>
    )
}

const QuickAddButton = ({ label }: { label: string }) => (
    <Button size="sm" className="w-full gap-2 bg-white text-black hover:bg-white/90">
            ...
            <div className="mx-auto max-w-xs px-4 py-8">
                <div className="group relative aspect-[3/4] cursor-pointer overflow-hidden rounded-2xl">
                    ...
                    <div className="absolute inset-x-0 bottom-0 space-y-4 p-5">
                        <ProductInfo name={product.name} designer={product.designer} rating={product.rating} />
