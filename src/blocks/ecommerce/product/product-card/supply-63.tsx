...
import { Box, Heart, Minus, Plus, ShoppingCart, Star, Truck, Package } from "lucide-react"
...
const SupplyRating = ({ rating, reviews }: { rating: number; reviews: number }) => (
...
const PriceDisplay = ({ total, perUnit, unit }: { total: number; perUnit: number; unit: string }) => (
    <div className="space-y-0.5">
        <span className="text-xl font-bold text-foreground">${total.toFixed(2)}</span>
        <p className="text-xs text-muted-foreground">${perUnit.toFixed(2)} per {unit}</p>
    </div>
)
        ...
        <section className="@container">
            <div className="mx-auto max-w-xs px-4 py-8">
                <Card className="group space-y-4 p-4">
                    ...
                    <Separator />
                    <div className="flex items-center justify-between">
                        <PriceDisplay total={supply.price} perUnit={supply.pricePerUnit} unit={supply.unit} />
                        <AddButton label="Add" />
                    </div>
                </Card>
            </div>
