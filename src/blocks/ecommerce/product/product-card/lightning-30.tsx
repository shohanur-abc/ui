...
const PriceDisplay = ({ original, deal }: { original: number; deal: number }) => {
    const discount = Math.round(((original - deal) / original) * 100)
    return (
        <div className="flex items-center gap-3">
            <span className="text-2xl font-bold text-primary">${deal.toFixed(2)}</span>
            <span className="text-lg text-muted-foreground line-through">${original.toFixed(2)}</span>
...
export default function Main() {
    const deal: DealProps = {
        image: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=600&h=340&fit=crop",
