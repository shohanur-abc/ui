...
import { Bell, Calendar, Clock, Heart, Package, ShoppingCart, Star, Truck } from "lucide-react"
...
const PreorderRating = ({ rating, preorders }: { rating: number; preorders: number }) => (
    <div className="flex items-center gap-3">
        <div className="flex items-center gap-1.5">
            <Star className="size-4 fill-yellow-400 text-yellow-400" />
            <span className="font-medium">{rating.toFixed(1)}</span>
        </div>
        <span className="text-sm text-muted-foreground">{preorders.toLocaleString()} pre-orders</span>
    </div>
)
...
const ReleaseCountdown = ({ date, days }: { date: string; days: number }) => (
    <div className="space-y-2 rounded-lg bg-muted/50 p-3">
        <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" className="text-xs">{days} days left</Badge>
        </div>
...
const PriceDisplay = ({ price, original, deposit }: { price: number; original: number; deposit: number }) => (
            ...
            <div className="mx-auto max-w-xs px-4 py-8">
                <Card className="group overflow-hidden">
                    ...
                    <div className="space-y-3 p-4">
                        ...
                        <PreorderRating rating={preorder.rating} preorders={preorder.preorders} />
                        <ReleaseCountdown date={preorder.releaseDate} days={preorder.daysUntil} />
