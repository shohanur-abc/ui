...
import { Heart, Music, Pause, Play, Share2, Star, Clock, Download } from "lucide-react"
...
const AlbumCover = ({ src, alt, isPlaying }: { src: string; alt: string; isPlaying: boolean }) => (
...
const AlbumRating = ({ rating, reviews }: { rating: number; reviews: number }) => (
...
const AlbumDetails = ({ tracks, duration }: { tracks: number; duration: string }) => (
        ...
        <section className="@container">
            <div className="mx-auto max-w-xs px-4 py-8">
                <Card className="group space-y-4 p-4">
                    <AlbumCover src={album.cover} alt={album.title} isPlaying={album.isPlaying} />
