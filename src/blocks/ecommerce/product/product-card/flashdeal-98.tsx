...
import { Heart, ShoppingCart, Star, Flame, Clock, Users, Zap, TrendingUp } from "lucide-react"
...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
    <div className="relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-red-100 to-orange-100 dark:from-red-950/30 dark:to-orange-950/30">
        <Image src={src} alt={alt} fill className="object-contain p-4 transition-transform duration-300 group-hover:scale-110" />
...
const CountdownTimer = ({ hours, minutes }: { hours: number; minutes: number }) => (
    ...
    return (
        <div className="space-y-1.5">
            <div className="flex items-center justify-between text-sm">
                ...
                <span className={`font-medium ${percent > 80 ? "text-destructive" : "text-foreground"}`}>
                    {total - claimed} left
                </span>
            </div>
            <Progress value={percent} className="h-2 bg-red-100 [&>div]:bg-gradient-to-r [&>div]:from-red-500 [&>div]:to-orange-500" />
        </div>
    )
}

const PriceDisplay = ({ original, sale }: { original: number; sale: number }) => (
    <div className="flex items-baseline gap-2">
        <span className="text-2xl font-bold text-destructive">${sale.toFixed(2)}</span>
                ...
                <Card className="group overflow-hidden border-red-200/50 shadow-lg shadow-red-500/10 dark:border-red-800/30">
                    ...
                    <div className="space-y-3 p-4">
                        <div className="flex items-center justify-between">
                            <DiscountBadge original={flash.originalPrice} sale={flash.salePrice} />
                            <ProductRating rating={flash.rating} reviews={flash.reviews} />
                        </div>
                        <BrandLabel text={flash.brand} />
                        <ProductName text={flash.name} />
                        <CountdownTimer hours={flash.hoursLeft} minutes={flash.minutesLeft} />
                        <ClaimProgress claimed={flash.claimed} total={flash.totalAvailable} />
                        <Separator />
                        <PriceDisplay original={flash.originalPrice} sale={flash.salePrice} />
