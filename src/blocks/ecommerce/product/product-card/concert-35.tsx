...
const EventImage = ({ src, alt }: { src: string; alt: string }) => (
	<div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-muted">
		...
		<Button
			size="icon-sm"
			variant="ghost"
			className="absolute right-3 top-3 text-white hover:bg-white/20 hover:text-white"
		>
...
export default function Main() {
	const concert: ConcertProps = {
		image:
			'https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?w=400&h=530&fit=crop',
		artist: 'The Midnight',
			...
			<div className="mx-auto max-w-xs px-4 py-8">
				<Card className="overflow-hidden">
					...
					<div className="space-y-4 p-4">
						<TicketInfo
							startingPrice={concert.startingPrice}
