...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
    <div className="relative aspect-[3/4] overflow-hidden bg-muted">
        <Image src={src} alt={alt} fill className="object-cover transition-all duration-500 group-hover:scale-105 group-hover:brightness-110" />
    </div>
)
...
const ProductName = ({ text }: { text: string }) => (
    <h3 className="font-semibold text-foreground transition-colors group-hover:text-primary">{text}</h3>
)

const ProductPrice = ({ amount }: { amount: number }) => (
    <span className="text-xl font-bold text-foreground">${amount.toFixed(0)}</span>
)

const ShopButton = ({ label }: { label: string }) => (
    <Button variant="outline" className="gap-1.5 transition-colors group-hover:bg-primary group-hover:text-primary-foreground">
        {label}
...
export default function Main() {
    const product: ProductProps = {
        image: "https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?w=400&h=530&fit=crop",
            ...
            <div className="mx-auto max-w-xs px-4 py-8">
                <Card className="group cursor-pointer overflow-hidden border-0 shadow-lg transition-shadow hover:shadow-xl">
                    ...
                    <div className="space-y-3 p-4">
                        <SocialProof views={product.viewsToday} bought={product.boughtToday} />
