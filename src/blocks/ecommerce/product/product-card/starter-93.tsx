...
import { Heart, ShoppingCart, Star, Package, Check, Truck, Shield } from "lucide-react"
...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
    <div className="relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20">
        <Image src={src} alt={alt} fill className="object-contain p-6 transition-transform duration-300 group-hover:scale-105" />
...
const ContentsList = ({ items }: { items: string[] }) => (
    <div className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Kit Contains:</p>
...
const ShippingGuarantee = ({ freeShipping, guarantee }: { freeShipping: boolean; guarantee: string }) => (
                ...
                <Card className="group space-y-4 p-4">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <StarterBadge />
                            <ProductRating rating={starter.rating} reviews={starter.reviews} />
                        </div>
                        ...
                        <ShippingGuarantee freeShipping={starter.freeShipping} guarantee={starter.guarantee} />
                    </div>
                    <Separator />
                    <div className="flex items-center justify-between">
