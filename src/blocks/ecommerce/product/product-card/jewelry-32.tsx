...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
	<div className="relative aspect-square overflow-hidden rounded-full bg-gradient-to-br from-amber-100 to-amber-50 p-6 dark:from-amber-950/30 dark:to-amber-900/20">
		...
		<Image
			...
			alt={alt}
			fill
			className="object-contain p-4 drop-shadow-lg transition-transform duration-500 group-hover:rotate-12 group-hover:scale-110"
		/>
		<Button
			size="icon-sm"
			variant="ghost"
...
const ProductRating = ({ rating }: { rating: number }) => (
	<div className="flex items-center justify-center gap-1">
		{Array.from({ length: 5 }).map((_, i) => (
			<Star
				key={i}
				className={`size-4 ${i < Math.floor(rating) ? 'fill-amber-400 text-amber-400' : 'fill-muted text-muted'}`}
			/>
		))}
	</div>
);
		...
		<section className="@container">
			<div className="mx-auto max-w-xs px-4 py-8">
				<Card className="group space-y-4 p-5">
					...
					<Separator />
					<TrustBadges
						certified={jewelry.certified}
						freeShipping={jewelry.freeShipping}
					/>
