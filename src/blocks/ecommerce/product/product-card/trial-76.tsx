...
import { Clock, Heart, Package, ShoppingCart, Star, Truck, AlertCircle } from "lucide-react"
...
const TrialRating = ({ rating, reviews }: { rating: number; reviews: number }) => (
...
const AutoRenewNotice = ({ willRenew }: { willRenew: boolean }) => (
    <div className={`flex items-start gap-2 rounded-lg p-2 text-xs ${
        willRenew ? "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-400" : "bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-400"
    }`}>
...
const PriceDisplay = ({ trial, full }: { trial: number; full: number }) => (
    <div className="space-y-0.5">
        <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-foreground">${trial.toFixed(2)}</span>
            <span className="text-sm text-muted-foreground line-through">${full.toFixed(2)}</span>
        </div>
...
export default function Main() {
    const trial: TrialProps = {
        image: "https://images.unsplash.com/photo-1556228720-195a672e8a03?w=500&h=280&fit=crop",
