...
import { Battery, Cpu, HardDrive, Heart, Monitor, ShoppingCart, Zap } from "lucide-react"
...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => (
    <div className="relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-primary/10 to-accent/10">
        <Image src={src} alt={alt} fill className="object-contain p-6 drop-shadow-xl transition-transform duration-500 group-hover:scale-110" />
...
const SpecsList = ({ specs }: { specs: { icon: string; label: string; value: string }[] }) => {
    ...
    return (
        <div className="grid grid-cols-2 gap-2">
            {specs.map((spec, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg bg-muted/50 px-2.5 py-1.5 text-xs">
