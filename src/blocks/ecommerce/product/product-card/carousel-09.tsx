...
const ImageGallery = ({ images, current, alt }: { images: string[]; current: number; alt: string }) => (
    <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        <div className="absolute inset-x-0 top-4 flex justify-between px-4">
            <Button size="icon-sm" variant="secondary" className="bg-background/80 backdrop-blur-sm">
        ...
        <Button size="icon-sm" variant="secondary" className="absolute bottom-4 right-4 bg-background/80 backdrop-blur-sm">
...
const ThumbnailStrip = ({ images, current }: { images: string[]; current: number }) => (
    <div className="flex gap-2 px-4">
        {images.map((img, i) => (
            <button
                key={i}
                className={`relative size-14 overflow-hidden rounded-md border-2 transition-all ${
                    i === current ? "border-primary ring-2 ring-primary/30" : "border-transparent opacity-60 hover:opacity-100"
                }`}
            >
        ...
        <section className="@container">
            <div className="mx-auto max-w-sm px-4 py-8">
                <Card className="overflow-hidden">
                    <ImageGallery images={product.images} current={product.currentImage} alt={product.name} />
                    <div className="space-y-4 py-4">
                        <ThumbnailStrip images={product.images} current={product.currentImage} />
