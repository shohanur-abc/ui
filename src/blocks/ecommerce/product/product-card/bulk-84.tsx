...
import { Heart, Layers, Minus, Package, Plus, ShoppingCart, Star, Truck, Percent } from "lucide-react"
...
const PriceComparison = ({ unit, bulk, count }: { unit: number; bulk: number; count: number }) => (
    <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">Regular ({count}× ${unit.toFixed(2)})</span>
            <span className="text-muted-foreground line-through">${(unit * count).toFixed(2)}</span>
        </div>
        <div className="flex items-center justify-between">
            <span className="font-medium text-foreground">Bulk Price</span>
            <span className="text-xl font-bold text-foreground">${bulk.toFixed(2)}</span>
        </div>
            ...
            <div className="mx-auto max-w-xs px-4 py-8">
                <Card className="group overflow-hidden">
                    ...
                    <div className="space-y-3 p-4">
                        ...
                        <PriceComparison unit={bulk.unitPrice} bulk={bulk.bulkPrice} count={bulk.unitCount} />
                        <div className="flex items-center justify-between">
                            <QuantitySelector />
