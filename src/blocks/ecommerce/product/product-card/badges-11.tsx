    ...
    <div className="flex items-center gap-1.5">
        <div className="flex">
            {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`size-3.5 ${i < Math.floor(rating) ? "fill-yellow-400 text-yellow-400" : "fill-muted text-muted"}`} />
            ))}
        </div>
...
const PriceDisplay = ({ current, original }: { current: number; original: number }) => (
    <div className="flex items-center gap-2">
        <span className="text-lg font-bold text-primary">${current.toFixed(2)}</span>
            ...
            <div className="mx-auto max-w-xs px-4 py-8">
                <Card className="group relative overflow-hidden">
                    ...
                    <div className="space-y-3 p-4">
                        ...
                        <PriceDisplay current={product.price} original={product.originalPrice} />
