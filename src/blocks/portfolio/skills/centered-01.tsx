				...
				<TechBadge
					items={[
						...
						},
						{
							name: 'MongoDB',
							icon: '🍃',
							color: 'bg-green-600/10 hover:bg-green-600/20',
						},
						{
							name: 'Redis',
							icon: '🔴',
	...
	return (
		<ul className="flex flex-wrap justify-center gap-3 @md:gap-4 max-w-4xl mx-auto">
			{items.map((tech, index) => (
				<li key={index}>
					<span
						className={`flex items-center gap-2 @md:gap-3 px-4 @md:px-5 @xl:px-6 py-3 @md:py-4 rounded-2xl ${tech.color} transition-all duration-300 hover:scale-105 cursor-default`}
					>
...
const Title = ({ text }: { text: string }) => (
	<div className="mb-3 @md:mb-4">
		<h2 className="text-2xl @sm:text-3xl @md:text-4xl font-bold">{text}</h2>
	</div>
);
