				...
				<SteppedSkills
					steps={[
						...
						},
						{
							title: 'Advanced',
							color: 'bg-blue-500',
							skills: ['Python', 'GraphQL', 'PostgreSQL', 'Docker', 'AWS'],
						},
						{
							title: 'Intermediate',
							color: 'bg-yellow-500',
							skills: ['Go', 'Kubernetes', 'Redis', 'MongoDB', 'Figma'],
						},
						{
							title: 'Learning',
							color: 'bg-orange-500',
							skills: ['Rust', 'AI/ML', 'WebAssembly', 'Blockchain'],
						},
					]}
				/>
			</div>
					...
					<div className="flex flex-wrap gap-2">
						{skills.map((skill, i) => (
							<div key={i} className="flex items-center gap-1.5 text-sm">
								<CheckCircle2
									className={`size-4 ${color.replace('bg-', 'text-')}`}
								/>
								<span>{skill}</span>
							</div>
						))}
