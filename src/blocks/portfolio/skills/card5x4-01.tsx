...
const MatrixCell = ({ name, category, level }: Skill) => {
    ...
    const bgOpacity = level >= 90 ? 'bg-primary/20' : level >= 80 ? 'bg-primary/15' : level >= 70 ? 'bg-primary/10' : 'bg-primary/5'

    return (
        <Card className={`group hover:border-primary/50 transition-all ${bgOpacity}`}>
