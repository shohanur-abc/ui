...
const ProfileCard = ({ name, tagline, yearsExp, topSkills, categories }: ProfileCardProps) => (
        ...
        <CardContent className="p-6 @md:p-8">
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                    {topSkills.map((skill, i) => (
                        <Badge key={i} variant="secondary" className="text-sm px-4 py-1">{skill}</Badge>
                    ))}
                </div>
            </div>
            ...
            <div>
                ...
                <div className="flex justify-between mt-2 text-xs text-muted-foreground">
                    {categories.map(({ name, percentage }, i) => (
                        <span key={i}>{name} {percentage}%</span>
                    ))}
                </div>
            </div>
        </CardContent>
