					...
					items={[
						{
							...
							description:
								'Creating fast, responsive, and SEO-friendly web applications using modern frameworks and best practices.',
							level: 'Expert',
							size: 'large',
						},
						{
						...
						},
						{
							...
							title: 'Security',
							description:
								'Implementing secure authentication flows, data encryption, and vulnerability prevention.',
							level: 'Advanced',
							size: 'medium',
						},
						{
...
const BentoCard = ({
	...
	return (
		<Card
			className={`group hover:border-primary/50 transition-all duration-300 hover:shadow-lg ${sizeClasses[size]}`}
		>
			<CardContent
				className={`h-full flex flex-col ${size === 'large' ? 'p-6 @md:p-8' : 'p-5'}`}
			>
				<div className="flex items-center justify-between mb-4">
					<div
						className={`rounded-xl bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors ${size === 'large' ? 'size-14' : 'size-11'}`}
					>
						<Icon
							className={`text-primary ${size === 'large' ? 'size-7' : 'size-5'}`}
						/>
					</div>
				...
				<h3
					className={`font-bold mb-2 ${size === 'large' ? 'text-2xl' : 'text-lg'}`}
				>
					{title}
				</h3>
				<p
					className={`text-muted-foreground flex-grow ${size === 'large' ? 'text-base' : 'text-sm'}`}
				>
					{description}
				</p>
			</CardContent>
