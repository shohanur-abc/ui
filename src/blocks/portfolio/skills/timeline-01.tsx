                    ...
                    milestones={[
                        {
                            ...
                            description: 'Leading architecture decisions and mentoring team members',
...
const TimelineItem = ({ year, icon: Icon, title, description, skills, isEven }: TimelineItemProps) => (
    <div className={`relative flex items-start gap-6 @md:gap-8 ${isEven ? '@md:flex-row' : '@md:flex-row-reverse'}`}>
        <div className={`hidden @md:block flex-1 ${isEven ? 'text-right' : 'text-left'}`}>
            <Badge variant="outline" className="text-lg font-bold">{year}</Badge>
        </div>
        ...
        <div className="flex-1 pb-8">
            <Badge variant="outline" className="@md:hidden mb-2">{year}</Badge>
