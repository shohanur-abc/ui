    ...
    return (
        <section className="@container">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <Pyramid
                    layers={[
                        { level: 'Expert', icon: Code2, skills: ['React', 'TypeScript', 'Next.js'] },
                        { level: 'Advanced', icon: Server, skills: ['Node.js', 'PostgreSQL', 'GraphQL', 'Docker'] },
                        { level: 'Intermediate', icon: Database, skills: ['Python', 'AWS', 'MongoDB', 'Redis', 'Kubernetes'] },
                        { level: 'Learning', icon: Layers, skills: ['Rust', 'Go', 'AI/ML', 'WebAssembly', 'Blockchain', 'Web3'] },
                    ]}
                />
            </div>
        </section>
...
const PyramidRow = ({ level, icon: Icon, skills, index, total }: PyramidRowProps) => {
    const widthPercent = 50 + ((index / (total - 1)) * 50)
    const colors = ['border-l-green-500', 'border-l-blue-500', 'border-l-yellow-500', 'border-l-orange-500']
    ...
    return (
        ...
        >
            <Card className={`group hover:border-primary/50 transition-all border-l-4 ${colors[index]}`}>
