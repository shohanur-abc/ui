			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
				...
				<FeaturedCenterGrid
					items={[
						...
						{
							quote:
								'Outstanding work quality and exceptional attention to detail. This team transformed our entire digital presence.',
							author: 'Maria Santos',
							role: 'CTO',
							company: 'TransformCo',
		...
		<div className="max-w-6xl mx-auto">
			<div className="grid @lg:grid-cols-3 gap-6 items-center">
				<ul className="space-y-4">
					{regular
						.slice(0, 2)
						.map(({ quote, author, role, avatar, rating }, i) => (
							<li
								key={i}
								className="bg-background border rounded-xl p-5 shadow-sm"
							>
					...
					<div className="bg-primary text-primary-foreground rounded-2xl p-8 shadow-lg">
						<div className="flex gap-0.5 mb-4">
							{Array.from({ length: 5 }).map((_, j) => (
								<Star
									key={j}
									className={`size-5 ${j < featured.rating ? 'fill-yellow-400 text-yellow-400' : 'text-primary-foreground/30'}`}
								/>
							))}
						</div>
