...
const PullQuote = ({ items }: { items: TestimonialItem[] }) => (
	<ul className="max-w-4xl mx-auto space-y-16">
		{items.map(({ quote, author, role, company, avatar }, i) => (
			<li
				key={i}
				className={`flex flex-col ${i % 2 === 0 ? '@md:items-start' : '@md:items-end'} text-${i % 2 === 0 ? 'left' : 'right'}`}
			>
				<blockquote
					className={`text-3xl @sm:text-4xl @md:text-5xl @lg:text-6xl font-bold leading-tight mb-6 ${i % 2 === 0 ? 'text-left' : '@md:text-right'}`}
				>
					&ldquo;{quote}&rdquo;
				</blockquote>
				<div
