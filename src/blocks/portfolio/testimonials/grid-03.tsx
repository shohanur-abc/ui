			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
				...
				<AsymmetricGrid
					items={[
						{
							quote:
								'The platform transformation resulted in unprecedented growth. Our team has never been more productive.',
						...
						{
							quote:
								'The UX redesign completely transformed user engagement. Metrics improved across every dimension we track.',
			...
			({ quote, author, role, company, avatar, metric, metricLabel }, i) => {
				...
				return (
					<li key={i} className={hasMetric ? '@xl:row-span-2' : ''}>
						<Card
							className={`h-full ${hasMetric ? 'bg-gradient-to-br from-primary to-primary/80 text-primary-foreground' : ''}`}
						>
							<CardContent className="p-6 flex flex-col h-full">
								...
								<blockquote
									className={`leading-relaxed mb-6 flex-1 ${hasMetric ? 'text-lg opacity-95' : 'text-base'}`}
								>
									&ldquo;{quote}&rdquo;
								</blockquote>
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<div
											className={`text-xs ${hasMetric ? 'opacity-80' : 'text-muted-foreground'}`}
										>
											{role}, {company}
										</div>
									</div>
