			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
				...
				<TestimonialGrid
					items={[
						{
							quote:
								'The project set a new standard for what we expect from digital partners. Exceptional in every way.',
							author: 'William Chen',
							role: 'CEO',
							company: 'InnovateX',
						...
						{
							quote:
								'Creative excellence combined with flawless execution. A rare combination we were fortunate to find.',
						...
						{
							quote:
								'Technical innovation that actually serves user needs. Refreshingly practical yet forward-thinking.',
							author: 'Robert Kim',
							role: 'CTO',
							company: 'TechForward',
						...
						{
							quote:
								'From strategy to execution, every step was handled with expertise and genuine care for our success.',
							author: 'Diana Moore',
							role: 'VP Marketing',
							company: 'GrowthLab',
...
const TestimonialGrid = ({ items }: { items: TestimonialItem[] }) => (
	<ul className="grid @md:grid-cols-2 gap-6">
		{items.map(({ quote, author, role, company, avatar, rating, award }, i) => (
			<li key={i}>
				<Card
					className={`h-full relative overflow-hidden ${award ? 'ring-2 ring-yellow-400/50' : ''}`}
				>
					{award && (
