			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
				...
				<MasonryGrid
					items={[
						{
							quote:
								'Remarkable work that transformed our entire digital strategy.',
						...
						{
							quote:
								"The team's dedication to excellence was evident in every deliverable. Our platform has never performed better.",
						...
						{
							quote:
								'The results speak for themselves. Our conversion rate increased by 250% within the first quarter.',
							author: 'Rachel Green',
						...
						{
							quote:
								'Professional and responsive throughout the entire project.',
							author: 'Andrew Kim',
						...
						{
							quote:
								'True partners who understand both design and business objectives.',
							author: 'Nina Patel',
							role: 'CEO, GrowthLab',
...
const MasonryGrid = ({ items }: { items: TestimonialItem[] }) => (
	<div className="columns-1 @md:columns-2 @xl:columns-3 gap-6 space-y-6">
		{items.map(({ quote, author, role, avatar, accent }, i) => (
			...
			>
				<Quote
					className={`size-8 mb-4 ${accent ? 'opacity-30' : 'text-primary/20'}`}
				/>
				<blockquote
					className={`text-base leading-relaxed mb-6 ${quote.length < 50 ? 'text-lg font-medium' : ''}`}
				>
					&ldquo;{quote}&rdquo;
				</blockquote>
				<div className="flex items-center gap-3">
					<Avatar
						className={`size-11 ${accent ? 'ring-2 ring-primary-foreground/20' : ''}`}
					>
					...
					<div>
						...
						<div
							className={`text-sm ${accent ? 'opacity-80' : 'text-muted-foreground'}`}
						>
							{role}
						</div>
					</div>
