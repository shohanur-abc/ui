		...
		<div>
			<Card className="py-0 mb-4 @md:mb-6 border-primary/30 bg-primary/5">
				<CardContent className="p-4 @md:p-6 flex items-center gap-4">
					<div
						className={`size-4 rounded-full ${status.available ? 'bg-green-500' : 'bg-red-500'} animate-pulse`}
					/>
					<div>
					...
					<div className="space-y-3">
						{projectTypes.map(({ name, available }, i) => (
							<div key={i} className="flex items-center gap-3">
								...
								/>
								<span
									className={`text-sm ${available ? '' : 'text-muted-foreground line-through'}`}
								>
									{name}
								</span>
							</div>
