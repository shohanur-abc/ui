            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @md:px-8 @xl:px-12 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                ...
                <PhaseWizard
                    phases={[
                        {
                            phase: 'Discovery',
                            tasks: ['Requirements gathering', 'User research', 'Technical analysis', 'Project scoping'],
                            status: 'completed',
                        },
                        {
                            phase: 'Design',
                            tasks: ['Wireframing', 'UI design', 'Prototype creation', 'Design review'],
                            status: 'completed',
                        },
                        {
                            phase: 'Development',
                            tasks: ['Frontend build', 'Backend API', 'Database setup', 'Integration'],
                            status: 'current',
                        },
                        {
                            phase: 'Launch',
                            tasks: ['Testing & QA', 'Performance tuning', 'Deployment', 'Monitoring'],
                            status: 'upcoming',
                        },
                    ]}
                />
                            ...
                            {tasks.map((task, j) => (
                                <li key={j} className="flex items-center gap-2 text-sm">
                                    <div className={`size-1.5 rounded-full ${
                                        status === 'completed' ? 'bg-primary' : status === 'current' ? 'bg-primary/50' : 'bg-muted-foreground/30'
                                    }`} />
                                    <span className={status === 'upcoming' ? 'text-muted-foreground' : ''}>{task}</span>
                                </li>
                            ))}
                        </ul>
                    </CardContent>
