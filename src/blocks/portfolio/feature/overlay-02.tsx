...
const Eyebrow = ({ text }: { text: string }) => (
    <Badge variant="outline" className="mb-3 @md:mb-4 bg-background/50 backdrop-blur-sm">
        {text}
    </Badge>
)
...
const ServiceCards = ({ items }: { items: ServiceItem[] }) => (
    <div className="grid @md:grid-cols-3 gap-4 @md:gap-6">
        {items.map(({ icon: Icon, title, description, link }, i) => (
            <Card key={i} className="py-0 group hover:shadow-xl transition-all bg-background/80 backdrop-blur-sm">
