					...
					items={[
						{
							...
							title: 'Design First',
							description:
								'Starting with user-centered design ensures the final product meets real needs.',
						},
						{
							icon: Layers,
							title: 'Component Architecture',
							description:
								'Building reusable components for consistency and faster development.',
						},
						{
							icon: Cpu,
							title: 'Performance Focus',
							description:
								'Optimizing every aspect for speed and smooth user experience.',
						},
						{
							icon: Globe2,
							title: 'Accessibility',
							description:
								'Making sure everyone can use the product regardless of ability.',
						},
						{
							icon: GitBranch,
							title: 'Version Control',
							description:
								'Maintaining clean git history for easy collaboration and rollbacks.',
						},
						{
							icon: Zap,
							title: 'Continuous Delivery',
							description:
								'Automated pipelines for fast, reliable deployments.',
						},
					]}
				/>
			</div>
...
const NumberedGrid = ({ items }: { items: GridItem[] }) => (
	<div className="grid @sm:grid-cols-2 @xl:grid-cols-3 gap-6 @md:gap-8">
		{items.map(({ icon: Icon, title, description }, i) => (
			<Card
				key={i}
				className="py-0 group hover:shadow-lg transition-all relative overflow-hidden"
			>
