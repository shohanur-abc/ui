					...
					items={[
						{
							...
							description:
								'Custom web apps built with modern frameworks and best practices.',
							color: 'bg-blue-500',
						},
						{
							icon: Smartphone,
							...
							description:
								'Cross-platform apps for iOS and Android with native performance.',
							color: 'bg-purple-500',
						},
						{
							icon: Palette,
		...
		{items.map(({ icon: Icon, title, description, color }, i) => (
			...
			>
				<CardContent className="p-4 @md:p-5 flex items-center gap-4 @md:gap-5">
					<div
						className={`size-12 @md:size-14 rounded-xl ${color} flex items-center justify-center shrink-0 text-white shadow-lg`}
					>
						<Icon className="size-6 @md:size-7" />
					</div>
					<div className="flex-1 min-w-0">
						<h3 className="font-semibold text-base @md:text-lg mb-0.5">
							{title}
						</h3>
