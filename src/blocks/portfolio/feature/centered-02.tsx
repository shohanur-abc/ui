...
const Description = ({ text }: { text: string }) => (
    <p className="text-base @md:text-lg @xl:text-xl text-muted-foreground leading-relaxed mb-8 @md:mb-10">{text}</p>
)

const FeatureList = ({ items }: { items: string[] }) => (
    <div className="flex flex-wrap justify-center gap-3 @md:gap-4 mb-8 @md:mb-10">
        {items.map((item, i) => (
            <div key={i} className="flex items-center gap-2 text-sm @md:text-base px-3 py-1.5 rounded-full bg-muted/50">
