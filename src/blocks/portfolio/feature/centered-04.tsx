...
const Title = ({ text }: { text: string }) => (
    <h2 className="text-3xl @sm:text-4xl @md:text-5xl @xl:text-6xl font-bold tracking-tight mb-5 @md:mb-6">{text}</h2>
)

const Description = ({ text }: { text: string }) => (
    <p className="text-base @md:text-lg @xl:text-xl text-muted-foreground leading-relaxed mb-10 @md:mb-14">{text}</p>
)
        ...
        {items.map(({ value, label }, i) => (
            <div key={i} className="flex items-center">
                <div className="text-center px-4 @md:px-8 @xl:px-12">
                    <div className="text-3xl @sm:text-4xl @md:text-5xl font-bold text-primary mb-1">{value}</div>
                    <div className="text-sm @md:text-base text-muted-foreground">{label}</div>
                </div>
                {i < items.length - 1 && (
                    <Separator orientation="vertical" className="h-12 @md:h-16 hidden @md:block" />
                )}
            </div>
        ))}
    </div>
