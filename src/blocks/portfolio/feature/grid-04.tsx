...
import { BookOpen, GraduationCap, Lightbulb, MessageCircle, Users, Video } from 'lucide-react'
    ...
    return (
        <section className="@container">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @md:px-8 @xl:px-12 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                ...
                <IconGrid
                    items={[
                        { icon: Video, title: 'Tutorials', description: 'Video content explaining complex concepts.' },
                        { icon: BookOpen, title: 'Articles', description: 'In-depth technical blog posts.' },
                        { icon: MessageCircle, title: 'Mentoring', description: 'One-on-one guidance sessions.' },
                        { icon: Users, title: 'Workshops', description: 'Interactive group learning.' },
                        { icon: GraduationCap, title: 'Courses', description: 'Structured learning paths.' },
                        { icon: Lightbulb, title: 'Tips', description: 'Quick productivity hacks.' },
                    ]}
                />
            </div>
        </section>
...
const IconGrid = ({ items }: { items: IconItem[] }) => (
    <div className="grid @xs:grid-cols-2 @lg:grid-cols-3 gap-4 @md:gap-6">
        {items.map(({ icon: Icon, title, description }, i) => (
            <Card key={i} className="py-0 group hover:shadow-lg transition-all border-transparent hover:border-primary/20 bg-muted/30 hover:bg-card">
