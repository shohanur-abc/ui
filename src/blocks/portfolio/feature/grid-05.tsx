...
import { Activity, FileSearch, PieChart, Settings, TrendingUp, Wallet } from 'lucide-react'
    ...
    return (
        <section className="@container">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @md:px-8 @xl:px-12 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                ...
                <CompactGrid
                    items={[
                        { icon: TrendingUp, title: 'Growth Metrics', value: '+127%', description: 'Year over year growth' },
                        { icon: Activity, title: 'Performance', value: '99.9%', description: 'System uptime' },
                        { icon: Wallet, title: 'ROI', value: '3.2x', description: 'Return on investment' },
                        { icon: PieChart, title: 'Conversion', value: '24%', description: 'Visitor to customer' },
                        { icon: FileSearch, title: 'SEO Score', value: '95', description: 'Search optimization' },
                        { icon: Settings, title: 'Automation', value: '80%', description: 'Tasks automated' },
                    ]}
                />
            </div>
        </section>
...
const CompactGrid = ({ items }: { items: CompactItem[] }) => (
    <div className="grid @xs:grid-cols-2 @lg:grid-cols-3 gap-6 @md:gap-8">
        {items.map(({ icon: Icon, title, value, description }, i) => (
            <div key={i} className="group flex gap-4 items-start p-4 @md:p-5 rounded-xl hover:bg-muted/50 transition-colors">
