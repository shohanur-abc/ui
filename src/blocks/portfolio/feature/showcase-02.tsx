		...
		{items.map(({ image, title, description, featured, links, stats }, i) => (
			...
			>
				<div className={`grid ${featured ? '@lg:grid-cols-2' : ''}`}>
					<div
						className={`relative ${featured ? 'aspect-video @lg:aspect-auto min-h-[250px]' : 'aspect-video'}`}
					>
						<Image
							src={image}
							alt={title}
					...
					<CardContent
						className={`p-5 @md:p-6 ${featured ? '@lg:flex @lg:flex-col @lg:justify-center' : ''}`}
					>
						<h3
							className={`font-bold mb-2 ${featured ? 'text-xl @md:text-2xl' : 'text-lg'}`}
						>
							{title}
						</h3>
