                    ...
                    items={[
                        {
                            ...
                            year: '2023',
                            description: 'Recognized for exceptional open source contributions.',
                        },
                        {
                            icon: Medal,
...
const AwardsGrid = ({ items }: { items: AwardItem[] }) => (
    <div className="grid @sm:grid-cols-2 @xl:grid-cols-4 gap-4 @md:gap-5">
        {items.map(({ icon: Icon, title, organization, year, description }, i) => (
            <Card key={i} className="py-0 text-center group hover:shadow-lg transition-all hover:border-primary/30">
                <CardContent className="p-5 @md:p-6">
                    ...
                    <p className="text-xs @md:text-sm text-primary font-medium mb-2">{organization}</p>
