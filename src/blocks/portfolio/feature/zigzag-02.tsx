					...
					items={[
						{
							...
							description:
								'Building fast, accessible, and beautiful websites using the latest technologies and best practices.',
							points: [
						...
						},
						{
							...
							description:
								'Cross-platform applications that deliver exceptional user experiences on any device.',
							points: [
						...
						},
						{
							...
							description:
								'Designing and implementing cloud solutions that grow with your business needs.',
							points: [
								'AWS & GCP certified',
...
const ZigzagBlocks = ({ items }: { items: ZigzagItem[] }) => (
	<div className="space-y-16 @md:space-y-24 @xl:space-y-32">
		{items.map(({ image, eyebrow, title, description, points, reverse }, i) => (
			<div
				key={i}
				className={`grid @3xl:grid-cols-2 gap-8 @lg:gap-12 @3xl:gap-16 items-center`}
			>
				<div
					className={`relative aspect-4/3 rounded-2xl overflow-hidden shadow-lg ${reverse ? '@3xl:order-2' : ''}`}
				>
