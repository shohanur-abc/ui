        ...
        <section className="@container">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @md:px-8 @xl:px-12 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                <div className="grid @3xl:grid-cols-3 gap-4 @md:gap-6">
                    ...
                    <BentoGrid
                        items={[
                            { icon: Globe, title: 'Cloud Native', description: 'Built for scale with modern cloud infrastructure.', accent: true },
                            { icon: Database, title: 'Data Layer', description: 'Optimized storage solutions.' },
                            { icon: Shield, title: 'Security First', description: 'Enterprise-grade protection.' },
                            { icon: Cpu, title: 'Edge Computing', description: 'Low latency worldwide.' },
                            { icon: Lock, title: 'Auth Systems', description: 'Secure identity management.' },
                            { icon: Workflow, title: 'CI/CD Pipelines', description: 'Automated deployments.', accent: true },
                        ]}
                    />
                </div>
            </div>
        ...
        {items.map(({ icon: Icon, title, description, accent }, i) => (
            ...
            >
                <CardContent className="p-5 @md:p-6">
                    <div className={`size-10 @md:size-11 rounded-lg flex items-center justify-center mb-3 transition-colors ${accent ? 'bg-primary-foreground/20' : 'bg-primary/10 group-hover:bg-primary group-hover:text-primary-foreground'}`}>
                        <Icon className="size-5" />
                    </div>
                    <h3 className="font-semibold mb-1">{title}</h3>
                    <p className={`text-sm leading-relaxed ${accent ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>{description}</p>
                </CardContent>
            </Card>
        ))}
    </div>
