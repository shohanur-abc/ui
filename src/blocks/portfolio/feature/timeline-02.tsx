                    ...
                    items={[
                        {
                            ...
                            description: 'Began learning web development and built my first projects.',
                        ...
                        },
                        {
                            ...
                            description: 'Promoted to senior role, leading technical decisions.',
...
const VerticalTimeline = ({ items }: { items: TimelineItem[] }) => (
    <div className="max-w-3xl mx-auto">
        {items.map(({ icon: Icon, year, title, description, tags }, i) => (
            <div key={i} className="relative pl-12 @md:pl-16 pb-12 @md:pb-16 last:pb-0">
                {i < items.length - 1 && (
                ...
                <div className="flex flex-wrap gap-2">
                    {tags.map((tag, j) => (
                        <span key={j} className="px-2.5 py-1 text-xs rounded-full bg-muted text-muted-foreground">
                            {tag}
                        </span>
                    ))}
                </div>
