    ...
    return (
        <section className="@container">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @md:px-8 @xl:px-12 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                ...
                <HighlightGrid
                    items={[
                        { icon: Zap, title: 'Fast Turnaround', description: 'Quick delivery without compromising quality.', highlight: true },
                        { icon: Shield, title: 'Reliable', description: 'Consistent communication and updates.' },
                        { icon: Heart, title: 'Passionate', description: 'Genuine care for your success.' },
                        { icon: Award, title: 'Quality', description: 'Production-ready code every time.' },
                        { icon: Clock, title: 'On-Time', description: 'Meeting deadlines is non-negotiable.' },
                        { icon: Star, title: 'Experienced', description: '8+ years in the industry.', highlight: true },
                    ]}
                />
            </div>
        </section>
        ...
        {items.map(({ icon: Icon, title, description, highlight }, i) => (
            ...
            >
                <CardContent className="p-5 @md:p-6 flex gap-4 items-start">
                    <div className={`size-11 @md:size-12 rounded-xl flex items-center justify-center shrink-0 transition-all ${
                        highlight
