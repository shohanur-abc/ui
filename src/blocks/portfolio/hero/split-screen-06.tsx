            ...
            <div className="grid @3xl:grid-cols-2 min-h-screen">
                <div className="flex items-center justify-center py-12 @md:py-16 @xl:py-20 @3xl:py-24 px-4 @sm:px-6 @md:px-8 @xl:px-12 bg-gradient-to-br from-background to-muted/20">
                    <div className="max-w-lg w-full">
                        ...
                        <CTA items={[
                            { label: 'View Projects', href: '#projects', icon: ArrowRight, variant: 'default' },
...
const Stats = ({ items }: { items: StatItem[] }) => (
    <div className="grid grid-cols-3 gap-3 @md:gap-4">
        {items.map(({ label, value, icon: Icon }) => (
            <div key={label} className="text-center p-3 @md:p-4 rounded-lg bg-card border hover:border-primary/50 transition-colors">
...
const HeroImage: FC<HeroImageProps> = ({ src, alt = 'Profile Image', initials = 'DP' }) => (
