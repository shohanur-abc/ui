                ...
                <div className="max-w-4xl mx-auto">
                    <div className="flex flex-col @md:flex-row gap-6 @lg:gap-8 items-center @md:items-start">
                        ...
                        <div className="flex-1 text-center @md:text-left">
                            ...
                            <CTA items={[
                                { label: 'View Portfolio', href: '#portfolio', icon: ArrowRight },
...
const AvatarWithStatus = ({ initials, src }: AvatarWithStatusProps) => (
    <div className="relative shrink-0">
        <Avatar className="size-28 @sm:size-32 @md:size-36 @lg:size-40 ring-4 ring-background shadow-xl">
            <AvatarImage src={src} />
            <AvatarFallback className="text-3xl @md:text-4xl bg-primary text-primary-foreground">{initials}</AvatarFallback>
        </Avatar>
...
const Eyebrow = ({ icon: Icon, text }: { icon: ComponentType<{ className?: string }>, text: string }) => (
    <Badge variant="secondary" className="inline-flex items-center gap-1.5 bg-background/50 backdrop-blur-sm px-4 py-2 mb-6">
