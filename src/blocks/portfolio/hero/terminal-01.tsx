...
const Eyebrow = ({ icon: Icon, text }: { icon: ComponentType<{ className?: string }>; text: string }) => (
    <Badge variant="outline" className="inline-flex items-center gap-2 px-3 py-1.5 mb-6 @md:mb-8 font-mono text-xs">
...
const Title = ({ greeting, name }: { greeting: string; name: string }) => (
    <div className="mb-3 @md:mb-4">
        <p className="text-lg @md:text-xl text-muted-foreground font-mono">{greeting}</p>
...
const TechStack = ({ items }: { items: { icon: ComponentType<{ className?: string }>; label: string }[] }) => (
    <div className="flex flex-wrap gap-3 mb-8 @md:mb-10">
        {items.map(({ icon: Icon, label }) => (
            <div key={label} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/50 border font-mono text-sm">
    ...
    <div className="relative">
        ...
        <div className="relative bg-card border rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-3 bg-muted/50 border-b">
                ...
                <span className="text-xs text-muted-foreground font-mono ml-2">~/.profile</span>
            </div>
            {/* Terminal Content */}
