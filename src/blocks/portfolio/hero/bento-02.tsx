        ...
        <section className="@container relative overflow-hidden">
            ...
            <div className="relative mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-12 @md:py-16 @xl:py-20 @3xl:py-24">
                <div className="grid @lg:grid-cols-3 @2xl:grid-cols-4 gap-4 @md:gap-5">
                    ...
                    />
                    <ProfileCard src="https://i.pravatar.cc/400?img=33" fallback="AK" name="Alex Kim" location="San Francisco, CA" />
                    <CTACard
...
const HeroTextCard = ({ badgeIcon: BadgeIcon, badgeText, title, description }: {
...
const ProfileCard = ({ src, fallback, name, location }: { src?: string; fallback: string; name: string; location: string }) => (
    <Card className="@2xl:row-span-2 overflow-hidden py-0">
        <CardContent className="p-0 h-full">
            <ProfileImage src={src} fallback={fallback} name={name} location={location} />
        </CardContent>
    </Card>
)

const CTACard = ({ heading, items }: {
    heading: string
    items: { label: string; href: string; icon?: ComponentType<{ className?: string }> }[]
}) => (
    <Card className="py-0">
...
const ProfileImage = ({ src, fallback, name, location }: { src?: string; fallback: string; name: string; location: string }) => (
