...
const CTA = ({ items }: { items: { label: string; href: string; icon?: React.ComponentType<{ className?: string }>; variant?: React.ComponentProps<typeof Button>['variant'] }[] }) => (
    <div className="flex flex-wrap gap-3 @md:gap-4 mb-8 @md:mb-10 @xl:mb-12">
        {items.map(({ label, href, icon: Icon }, i) => (
            <Button key={i} size="lg" className="gap-2" variant={i === 0 ? 'default' : 'outline'} asChild>
                <Link href={href}>
                    {label}
...
const HeroImage: FC<HeroImageProps> = ({ src = 'https://avatars.githubusercontent.com/u/252440198?v=4', alt = 'Profile', fallback = 'ML' }) => (
    <div className="relative w-full max-w-md aspect-square">
        ...
        <div className="absolute inset-0 bg-card border-2 rounded-3xl shadow-2xl flex items-center justify-center">
            <Avatar className="size-56">
                ...
                <AvatarFallback className="rounded-2xl bg-linear-to-br from-primary to-primary/60 text-4xl @sm:text-5xl font-bold text-primary-foreground">{fallback}</AvatarFallback>
            </Avatar>
        </div>
    </div>
)
