...
const Eyebrow = ({
	...
	<Badge
		variant="secondary"
		className="inline-flex items-center gap-2 mb-4 @md:mb-5 @xl:mb-6"
	>
...
const CTA = ({
	...
	<div className="flex flex-wrap gap-3 @md:gap-4 mb-8 @md:mb-10 @xl:mb-12">
		{items.map(
			({ label, href, iconLeft: IconLeft, iconRight: IconRight }, i) => (
				<Button
					key={i}
					size="lg"
					className="gap-2"
					variant={i === 0 ? 'default' : i === 1 ? 'outline' : 'ghost'}
					asChild
				>
					<Link href={href}>
	...
	<div className="relative aspect-square max-w-md mx-auto">
		...
		<div className="relative rounded-4xl bg-card border-2 shadow-2xl flex items-center justify-center h-full">
			<Avatar className="size-60">
				...
				<AvatarFallback className="rounded-3xl bg-linear-to-br from-primary to-primary/60 text-5xl font-bold">
					{fallback}
				</AvatarFallback>
			</Avatar>
		</div>
