						...
						items={[
							{
								Icon: Code2,
								title: 'Clean Code',
								description: 'Writing scalable, maintainable solutions',
							},
							{
								Icon: Layers,
								title: 'Modern Stack',
								description: 'Latest tech for best results',
							},
							{
								Icon: Zap,
								title: 'Fast Delivery',
								description: 'Efficient workflows, timely results',
							},
						]}
					/>
				</div>
...
const FeaturesGrid = ({ items }: { items: FeatureCardProps[] }) => (
	<ul className="grid @md:grid-cols-3 gap-4 @md:gap-5 @xl:gap-6 max-w-4xl mx-auto">
		{items.map(({ Icon, title, description }, i) => (
			<li
				key={i}
				className="group p-5 @md:p-6 @xl:p-8 rounded-2xl border bg-card/50 backdrop-blur-sm hover:bg-card transition-all hover:shadow-lg hover:-translate-y-1"
			>
