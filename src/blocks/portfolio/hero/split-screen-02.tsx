...
import { ArrowRight, Calendar, Code2, Github, Linkedin, Mail, MapPin, Twitter } from 'lucide-react'
...
const HeroImage: FC<HeroImageProps> = ({ initials, src = 'https://avatars.githubusercontent.com/u/252440198?v=4' }) => (
    <div className="relative">
        ...
        <div className="absolute top-4 @md:top-6 -left-3 @md:-left-4 px-3 @md:px-4 py-1 @md:py-1.5 bg-background/50 rounded-full border-2 border-primary/20 shadow-xl backdrop-blur-sm -rotate-35 animate-bounce">
            <div className="flex items-center gap-2 @md:gap-2.5">
                ...
                <span className="text-xs @md:text-sm font-semibold whitespace-nowrap">Available for work</span>
            </div>
        </div>
    </div>
)
...
const Eyebrow = ({ icon: Icon, text }: { icon: ComponentType<{ className?: string }>, text: string }) => (
    <Badge variant="outline" className="inline-flex items-center gap-1.5 px-3 @md:px-4 py-1 @md:py-1.5 text-sm font-medium border-primary/30 bg-primary/5">
...
const CTA = ({ items }: { items: { label: string; href: string; variant?: React.ComponentProps<typeof Button>['variant']; icon?: React.ComponentType<{ className?: string }> }[] }) => (
    <div className="flex flex-col @sm:flex-row items-center justify-center @3xl:justify-start gap-3 @md:gap-4 pt-2 @md:pt-4">
        {items.map(({ label, href, variant, icon: Icon }, i) => (
            <Button key={i} size="lg" variant={variant || "default"} className="gap-2 px-6 @md:px-8 h-11  text-base font-semibold shadow-lg hover:shadow-xl transition-all" asChild>
                <Link href={href}>
                    {label}
...
const SocialLinks = ({ items }: { items: SocialLink[] }) => (
    <div className="flex items-center gap-2 @md:gap-3">
        {items.map(({ icon: Icon, href, label }) => (
            <Button key={label} variant="outline" size="icon" className="rounded-full h-10 w-10 @md:h-12 @md:w-12 border-2 hover:border-primary hover:bg-primary/5 transition-all" asChild>
