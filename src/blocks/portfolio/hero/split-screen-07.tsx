            ...
            <ul className="grid @3xl:grid-cols-2 min-h-screen overflow-hidden items-center gap-8 @md:gap-12 @2xl:gap-16 @4xl:gap-20 py-16 @md:py-20 @3xl:py-24 @5xl:py-32 px-4 @sm:px-6 @2xl:px-8">
                ...
                <li className="max-w-xl -order-1 @3xl:order-1">
                    <div className="space-y-5 @md:space-y-6 @xl:space-y-8">
                        ...
                        <CTA items={[
                            { label: 'Explore Portfolio', icon: ArrowRight, href: '#portfolio', variant: 'default' },
                            { label: "Let's Talk", icon: Mail, href: '#contact', variant: 'outline' },
                        ]} />

                        <SocialLinks items={[
                            { label: 'Twitter', icon: Twitter, href: 'https://twitter.com/emmajohnson' },
                            { label: 'LinkedIn', icon: Linkedin, href: 'https://linkedin.com/in/emmajohnson' },
                            { label: 'GitHub', icon: Github, href: 'https://github.com/emmajohnson' },
                        ]} />
...
const HeroImage: FC<HeroImageProps> = ({
    ...
    <div className={`relative aspect-square rounded-3xl  bg-linear-to-br from-secondary via-secondary/85 to-secondary/70 p-6 @md:p-8 shadow-2xl  transition-transform duration-500 hover:scale-[1.02] ${className}`} >
...
const CTA = ({ items }: { items: CTALink[] }) => (
    <div className="flex flex-wrap gap-3 pt-2">
        {items.map(({ href, label, icon: Icon, variant }) => (
            <Button asChild key={label} size="lg" variant={variant} className="gap-2 px-5 @md:px-6 shadow-lg h-11 @md:h-12">
                <Link href={href}>
                    {label}
...
const SocialLinks = ({ items }: { items: SocialLink[] }) => (
    <div className="border-t flex items-center gap-1 pt-3 @md:pt-4">
        ...
        {items.map(({ href, label, icon: Icon }) => (
            <Button key={label} asChild size="icon" variant="ghost" className="hover:text-primary transition-colors">
