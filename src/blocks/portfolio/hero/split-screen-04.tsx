                ...
                <div className="flex items-center justify-center py-12 @md:py-16 @xl:py-20 @3xl:py-24 px-4 @sm:px-6 @md:px-8 @xl:px-12">
                    <div className="max-w-xl">
                        ...
                        <ServiceList services={[
                            { title: 'User-Centered Design', description: 'Creating intuitive experiences that users love' },
                            { title: 'Brand Strategy', description: 'Building strong brand identities' },
                            { title: 'Product Development', description: 'From concept to launch and beyond' },
                        ]} />

                        <CTA items={[
                            { label: 'View Case Studies', href: '#cases', icon: ArrowRight },
...
const HeroImage: FC<HeroImageProps> = ({ initials = 'RK', src = 'https://avatars.githubusercontent.com/u/252440198?v=4', achievements }) => (
    <div className="text-center text-primary-foreground">
        <Avatar className="size-56 mx-auto mb-6 @md:mb-8">
            <AvatarImage src={src} alt="Profile" />
            <AvatarFallback className="bg-white/10 text-6xl @md:text-7xl font-bold">{initials}</AvatarFallback>
        </Avatar>
        <ul className="space-y-3 @md:space-y-4 max-w-md mx-auto">
            {achievements.map(({ text }) => (
                <li key={text} className="flex items-center justify-center gap-2 @md:gap-3">
...
const CTA = ({ items }: { items: { label: string; href: string; icon?: React.ComponentType<{ className?: string }>; variant?: React.ComponentProps<typeof Button>['variant'] }[] }) => (
