...
const ProfileColumn = ({ src, fallback, name, role, company, achievements, cta }: ProfileColumnProps) => (
    <div className="@lg:col-span-2 text-center @lg:text-left @lg:sticky @lg:top-8">
        ...
        <h1 className="text-3xl @md:text-4xl font-bold tracking-tight mb-1">{name}</h1>
        ...
        <div className="space-y-2 mb-8">
            {achievements.map(({ icon: Icon, text }, i) => (
                <div key={i} className="flex items-center justify-center @lg:justify-start gap-2 text-sm text-muted-foreground">
    ...
    <div className="@lg:col-span-3">
        ...
        <div className="space-y-4 text-muted-foreground leading-relaxed mb-10">
            {paragraphs.map((text, i) => (
                <p key={i} className="text-base @md:text-lg">{text}</p>
            ))}
        </div>
        <div>
