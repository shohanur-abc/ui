				...
				<TimelineGrid
					items={[
						...
						},
						{
							year: '2015',
							title: 'First Job',
							description: 'Junior Developer at Agency',
							category: 'work',
						},
						{
							year: '2017',
						...
						},
						{
							year: '2021',
							title: 'Director',
							description: 'Engineering Director role',
							category: 'work',
						},
						{
							year: '2023',
							title: 'CTO',
							description: 'Joined TechCorp as CTO',
							category: 'work',
						},
						{
							year: 'Now',
							title: 'Building',
							description: 'Scaling to 100+ engineers',
							category: 'milestone',
						},
					]}
				/>
...
const TimelineGrid = ({ items }: TimelineGridProps) => (
	<div className="grid grid-cols-2 @md:grid-cols-4 gap-4 @lg:gap-6">
		{items.map((item, i) => (
			<div
				key={i}
				className="relative p-4 @lg:p-6 rounded-xl border bg-card hover:shadow-md transition-shadow"
			>
				<Circle
					className={`size-3 absolute top-4 right-4 ${item.category === 'milestone' ? 'fill-primary text-primary' : item.category === 'education' ? 'fill-blue-500 text-blue-500' : 'fill-muted text-muted'}`}
				/>
