            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="max-w-sm mx-auto">
                    <ProfileCard
                        ...
                        skills={['User Interviews', 'Usability Testing', 'Surveys', 'Analytics']}
                        actions={[
                            { label: 'View Portfolio', href: '/portfolio', icon: ArrowUpRight, variant: 'default' },
                            { label: 'Download CV', href: '/cv.pdf', icon: Download, variant: 'outline' },
                        ]}
                    />
                </div>
            </div>
...
const ProfileCard = ({ src, fallback, name, role, tagline, socials, skills, actions }: ProfileCardProps) => (
    <Card>
        ...
        <CardContent>
            ...
            <div className="space-y-2">
                {actions.map(({ label, href, icon: Icon, variant }) => (
                    <Button key={label} variant={variant} className="gap-2 w-full" asChild>
                        <Link href={href}>
                            {label}
                            <Icon className="size-4" />
                        </Link>
