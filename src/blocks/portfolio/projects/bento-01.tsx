            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <BentoGrid
                    featured={{
                        ...
                        description: 'A comprehensive business management solution with advanced analytics, team collaboration, and automated workflows.',
...
const BentoGrid = ({ featured, secondary, tertiary }: BentoGridProps) => (
    <div className="grid @lg:grid-cols-2 @xl:grid-cols-3 gap-4 @md:gap-6">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 @md:p-8">
                    <div className="flex flex-wrap gap-2 mb-3">
                        {featured.tags.map((tag, i) => (
                            <Badge key={i} variant="secondary" className="bg-white/10 backdrop-blur-sm text-white border-white/20">{tag}</Badge>
                        ))}
                    </div>
                    <h3 className="text-white text-2xl @md:text-3xl font-bold mb-2">{featured.title}</h3>
        ...
        {secondary.map(({ image, title, description, tags, href }, i) => (
            <div key={i} className="group relative rounded-2xl overflow-hidden bg-card border transition-all hover:shadow-lg hover:shadow-primary/10 hover:border-primary/30">
        ...
        {tertiary.map(({ image, title, tags, href }, i) => (
            <div key={i} className="group relative rounded-xl overflow-hidden bg-card border transition-all hover:shadow-md hover:shadow-primary/5 hover:border-primary/20">
                <Link href={href} className="block">
