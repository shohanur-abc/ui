                    ...
                    items={[
                        {
                            ...
                            description: 'Revolutionary content generation tool powered by GPT.',
                            views: 12500,
                            trend: '+240%',
                            rank: 1,
                            tags: ['AI', 'SaaS'],
...
const rankStyles: Record<number, { bg: string; text: string; icon: ComponentType<{ className?: string }> }> = {
    1: { bg: 'bg-gradient-to-br from-yellow-400 to-amber-500', text: 'text-yellow-900', icon: Rocket },
    2: { bg: 'bg-gradient-to-br from-gray-300 to-gray-400', text: 'text-gray-700', icon: Star },
    3: { bg: 'bg-gradient-to-br from-orange-400 to-orange-500', text: 'text-orange-900', icon: Zap },
...
const TrendingGrid = ({ items }: { items: TrendingItem[] }) => (
    <div className="space-y-4">
        {items.map(({ image, title, description, views, trend, rank, tags, href }, i) => {
            ...
            return (
                <Card key={i} className="group overflow-hidden border transition-all hover:shadow-xl hover:shadow-primary/10 hover:border-primary/20 p-0">
                    <Link href={href} className="flex flex-col @md:flex-row">
                        {/* Rank */}
                        <div className={`@md:w-20 flex @md:flex-col items-center justify-center gap-2 p-3 @md:p-4 ${style.bg}`}>
                            <span className={`text-2xl @md:text-3xl font-bold ${style.text}`}>#{rank}</span>
