					...
					items={[
						{
							...
							quote:
								'The platform transformed how we serve our customers. Transaction speed improved by 300%.',
							href: '#',
						},
						{
							...
							quote:
								'Patient satisfaction scores increased 45% after launching this intuitive system.',
							href: '#',
						},
						{
							...
							quote:
								'Our online sales doubled within the first quarter. The UX is absolutely phenomenal.',
							href: '#',
						},
					]}
				/>
	...
	<div className="space-y-8">
		{items.map(
			(
				...
				>
					<div
						className={`grid @lg:grid-cols-2 ${i % 2 === 1 ? '@lg:grid-flow-col-dense' : ''}`}
					>
						{/* Project image */}
						<Link
							href={href}
						...
						>
							...
							<div className="absolute bottom-4 left-4 right-4 @lg:right-auto">
								<h3 className="text-white font-bold text-xl @md:text-2xl group-hover:text-primary transition-colors">
									{title}
								</h3>
							</div>
						</Link>
						...
						{/* Quote section */}
						<div
							className={`p-6 @md:p-8 flex flex-col justify-center ${i % 2 === 1 ? '@lg:col-start-1' : ''}`}
						>
