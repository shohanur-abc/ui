                ...
                <RowList
                    items={[
                        ...
                        },
                        {
                            ...
                            description: 'Comprehensive patient management system with telehealth integration.',
                            category: 'Health',
                            year: '2024',
                        ...
                        },
                        {
                            ...
                            description: 'GPT-powered content creation and optimization tool for marketing teams.',
                            category: 'AI/ML',
                            year: '2025',
                        ...
                        },
                        {
                            ...
                            description: 'Complete overhaul of online shopping experience with improved conversion.',
                            category: 'Retail',
                            year: '2024',
...
const RowList = ({ items }: { items: RowItem[] }) => (
    <div className="space-y-4">
        {items.map(({ image, title, description, category, year, technologies, href }, i) => (
                    ...
                    <div className="flex flex-col @md:flex-row">
                        ...
                        <CardContent className="flex-1 p-4 @md:p-6 flex flex-col @lg:flex-row @lg:items-center gap-4">
                            <div className="flex-1 min-w-0">
                                ...
                                <h3 className="font-bold text-lg @md:text-xl mb-2 group-hover:text-primary transition-colors">{title}</h3>
                                <p className="text-sm text-muted-foreground line-clamp-2">{description}</p>
                            </div>
                            
                            {/* Technologies */}
                            <div className="@lg:w-48 shrink-0">
                                <div className="text-xs text-muted-foreground mb-2 uppercase tracking-wider">Tech Stack</div>
                                <div className="flex flex-wrap gap-1">
                                    {technologies.slice(0, 4).map((tech, j) => (
                                        <Badge key={j} variant="outline" className="text-xs">{tech}</Badge>
                                    ))}
                                    {technologies.length > 4 && (
                                        <Badge variant="outline" className="text-xs">+{technologies.length - 4}</Badge>
                                    )}
                                </div>
                            </div>
