					...
					items={[
						{
							...
							description:
								'Led a complete redesign of the analytics dashboard, improving user engagement by 45% and reducing support tickets by 30%.',
						...
						},
						{
							...
							description:
								'Architected and built a headless e-commerce solution handling 10,000+ orders daily with 99.9% uptime.',
						...
						},
						{
							...
							description:
								'Created and maintained a React component library used by 5,000+ developers with 2,000+ GitHub stars.',
...
const ProjectList = ({ items }: { items: ProjectItem[] }) => (
	<div className="space-y-12 @md:space-y-16">
		{items.map(
			({ image, title, description, tags, year, liveUrl, githubUrl }, i) => (
				<article
					key={i}
					className={`grid @3xl:grid-cols-2 gap-8 @lg:gap-12 items-center ${i % 2 === 1 ? '' : ''}`}
				>
					<div
						className={`relative aspect-video rounded-2xl overflow-hidden group ${i % 2 === 1 ? '@3xl:order-2' : ''}`}
					>
						<Image
							src={image}
							alt={title}
					...
					<div className={i % 2 === 1 ? '@3xl:order-1' : ''}>
						...
						<h3 className="text-2xl @md:text-3xl @xl:text-4xl font-bold mb-4 leading-tight">
							{title}
						</h3>
