...
const ExperimentGrid = ({ items }: { items: ExperimentItem[] }) => (
    <div className="grid @sm:grid-cols-2 @xl:grid-cols-3 gap-6">
        {items.map(({ image, title, description, status, tech, href }, i) => {
            ...
            return (
                <Card key={i} className="group overflow-hidden border border-dashed transition-all hover:shadow-lg hover:shadow-primary/10 hover:border-primary/30 hover:border-solid p-0">
                    <Link href={href} className="block">
                        <div className="relative aspect-video overflow-hidden bg-muted">
                            ...
                            {/* Status badge */}
                            <Badge className={`absolute top-3 left-3 gap-1.5 ${config.color}`}>
