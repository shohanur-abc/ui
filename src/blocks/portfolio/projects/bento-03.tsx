            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <AsymmetricBento
                    primary={{
                        ...
                        description: 'Comprehensive design system powering multiple product lines with consistent UX.',
...
const AsymmetricBento = ({ primary, items }: { primary: PrimaryItem; items: GridItem[] }) => (
    <div className="grid @lg:grid-cols-2 gap-4 @md:gap-6">
        ...
        <Card className="group relative overflow-hidden border-0 bg-transparent @lg:row-span-2 p-0">
            <Link href={primary.href} className="block h-full">
                <div className="relative aspect-square @lg:h-full min-h-[400px] rounded-2xl overflow-hidden">
                    <Image src={primary.image} alt={primary.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
                    ...
                    <div className="absolute top-4 left-4">
                        <Badge className="bg-primary/90 backdrop-blur-sm">{primary.category}</Badge>
                    </div>
                    
                    <div className="absolute bottom-0 left-0 right-0 p-6 @md:p-8">
                        <h3 className="text-white text-2xl @md:text-3xl @xl:text-4xl font-bold mb-2">{primary.title}</h3>
        ...
        <div className="grid @sm:grid-cols-2 gap-4 @md:gap-6">
            {items.map(({ image, title, category, href }, i) => (
                <Link key={i} href={href} className="group relative rounded-xl overflow-hidden bg-card border transition-all hover:shadow-lg hover:shadow-primary/10 hover:border-primary/30">
                    <div className="relative aspect-[4/3]">
                        ...
                        <div className="absolute top-3 left-3">
                            <Badge variant="outline" className="bg-white/10 backdrop-blur-sm text-white border-white/20">{category}</Badge>
                        </div>
