					...
					items={[
						{
							...
							description:
								'Enterprise analytics platform with machine learning predictions and automated insights generation.',
						...
						},
						{
							...
							description:
								'Complete patient management system with telemedicine, scheduling, and EHR integration.',
						...
						},
						{
							...
							description:
								'Multi-vendor marketplace with advanced inventory management and analytics.',
			...
			{items.map(({ year, title, description, image, tags, href }, i) => (
				...
				>
					...
					>
						...
						<div
							className={`flex flex-wrap gap-2 mb-4 ${i % 2 === 0 ? '@md:justify-end' : ''}`}
						>
							{tags.map((tag, j) => (
					...
					{/* Image */}
					<div
						className={`ml-10 @md:ml-0 @md:w-1/2 ${i % 2 === 0 ? '@md:pl-12' : '@md:pr-12'}`}
					>
						<Link
							href={href}
							className="group block rounded-xl overflow-hidden bg-card border transition-all hover:shadow-xl hover:shadow-primary/10 hover:border-primary/30"
						>
