				...
				<HoverGrid
					items={[
						...
						},
						{
							...
							title: 'Food Delivery',
							description:
								'Real-time order tracking and restaurant management.',
						...
						},
						{
							...
							description:
								'High-fidelity audio streaming with social features.',
...
const HoverGrid = ({ items }: { items: HoverItem[] }) => (
	<div className="grid @sm:grid-cols-2 @xl:grid-cols-3 gap-6">
		{items.map(({ image, title, description, tags, color, href }, i) => (
			<Card
				key={i}
				className="group relative overflow-hidden border transition-all duration-500 hover:shadow-2xl hover:shadow-primary/15 hover:border-primary/30 hover:-translate-y-1 p-0"
			>
				<Link href={href} className="block">
					{/* Background glow */}
					<div
						className={`absolute inset-0 bg-gradient-to-br ${color} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
					/>
