        ...
        <section className="@container" data-theme="neon">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="flex flex-col @lg:flex-row @lg:items-end @lg:justify-between gap-6 mb-12 @md:mb-16">
                    ...
                    <FilterTabs items={['All', 'Web', 'Mobile', 'Design']} activeIndex={0} />
                </div>
...
const FilterTabs = ({ items, activeIndex }: { items: string[]; activeIndex: number }) => (
...
const ProjectGrid = ({ items }: { items: ProjectItem[] }) => (
    <ul className="grid @sm:grid-cols-2 @xl:grid-cols-3 gap-6">
        {items.map(({ image, title, category, year, liveUrl, githubUrl }, i) => (
            <li key={i} className="group relative rounded-xl overflow-hidden bg-card border">
                <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 flex gap-2">
                        ...
                        <Badge variant="outline" className="bg-background/50 backdrop-blur-sm">{year}</Badge>
                    </div>
                    <div className="absolute bottom-0 left-0 right-0 p-4 @md:p-6">
                        <h3 className="text-white text-lg @md:text-xl font-semibold mb-3">{title}</h3>
                        <div className="flex gap-2 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all">
                            ...
                            {githubUrl && (
                                <Button size="sm" variant="outline" className="gap-1.5 bg-background/50 backdrop-blur-sm" asChild>
                                    <Link href={githubUrl}>
