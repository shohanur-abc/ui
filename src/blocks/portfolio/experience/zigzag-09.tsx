...
const TalkZigzag = ({ title, event, type, date, location, attendees, videoUrl, align }: TalkZigzagProps) => (
    <div className={`grid @lg:grid-cols-2 gap-8 @lg:gap-16 items-center`}>
        <div className={`${align === 'right' ? '@lg:order-2' : ''}`}>
            <Badge variant={type === 'Keynote' ? 'default' : 'secondary'} className="mb-4">{type}</Badge>
        ...
        <Card className={`${align === 'right' ? '@lg:order-1' : ''}`}>
            <CardContent className="p-6">
                ...
                {videoUrl && (
                    <Link href={videoUrl} target="_blank" className="flex items-center justify-center gap-1.5 text-sm text-primary hover:underline">
