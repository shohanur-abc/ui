...
const TimelineItem = ({
	...
	<Link
		href={`/experience/${company.toLowerCase().replace(' ', '-')}`}
		className="block group"
	>
		<div className="relative pl-12 @md:pl-16">
			<div
				className={`absolute left-0 size-8 @md:size-12 rounded-full border-4 border-background flex items-center justify-center ${current ? 'bg-primary' : 'bg-muted'}`}
			>
				<Avatar className="size-full">
					<AvatarImage src={logo} alt={company} />
					<AvatarFallback
						className={current ? 'bg-primary text-primary-foreground' : ''}
					>
						{initials}
					</AvatarFallback>
				</Avatar>
			...
			<Card
				className={`hover:shadow-md transition-all ${current ? 'ring-2 ring-primary' : ''}`}
			>
