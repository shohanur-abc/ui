...
import { Briefcase, Code, Users, Award, TrendingUp, GraduationCap } from 'lucide-react'
...
const BentoCard = ({ icon: Icon, title, subtitle, description, className = '', featured }: BentoCardProps) => (
    <Card className={`group hover:shadow-lg transition-all ${className}`}>
        <CardContent className={`p-6 h-full flex flex-col ${featured ? 'justify-center items-center text-center' : ''}`}>
            <div className={`size-12 rounded-xl bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/20 transition-colors ${featured ? 'size-16' : ''}`}>
                <Icon className={`text-primary ${featured ? 'size-8' : 'size-6'}`} />
            </div>
            <p className={`font-bold mb-1 ${featured ? 'text-5xl @md:text-6xl' : 'text-3xl'}`}>{title}</p>
            <p className={`text-primary mb-2 ${featured ? 'text-lg' : 'text-sm'}`}>{subtitle}</p>
            <p className={`text-muted-foreground ${featured ? 'text-base' : 'text-xs'}`}>{description}</p>
        </CardContent>
    </Card>
)
