...
import { Briefcase, Download, Mail, Calendar, MapPin, Linkedin, Github, Globe } from 'lucide-react'
...
const ResumeHeader = ({ name, title, email, location, linkedin, github, website, resumeUrl }: ResumeHeaderProps) => (
    <Card className="mb-8">
            ...
            <div className="flex flex-col @md:flex-row @md:items-start justify-between gap-6">
                <div>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-muted-foreground mb-4">
                        <span className="flex items-center gap-1"><Mail className="size-4" />{email}</span>
                        <span className="flex items-center gap-1"><MapPin className="size-4" />{location}</span>
                    </div>
                    <div className="flex gap-3">
                        <Link href={`https://${linkedin}`} target="_blank">
                            <Badge variant="outline" className="gap-1"><Linkedin className="size-3" />LinkedIn</Badge>
                        </Link>
                        <Link href={`https://${github}`} target="_blank">
                            <Badge variant="outline" className="gap-1"><Github className="size-3" />GitHub</Badge>
                        </Link>
                        <Link href={`https://${website}`} target="_blank">
                            <Badge variant="outline" className="gap-1"><Globe className="size-3" />Website</Badge>
                        </Link>
                    </div>
                </div>
                <Button asChild>
