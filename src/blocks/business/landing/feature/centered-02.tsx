    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <FeatureCards items={[
                    { icon: Zap, title: 'Lightning Fast', description: 'Optimized performance with sub-100ms response times globally.' },
                    { icon: Shield, title: 'Bank-Level Security', description: 'SOC 2 Type II certified with end-to-end encryption.' },
                    { icon: Gauge, title: 'Real-time Updates', description: 'Live synchronization across all devices and team members.' },
                    { icon: Clock, title: '24/7 Availability', description: 'Multi-region infrastructure ensures zero downtime.' },
                    { icon: Headphones, title: 'Expert Support', description: 'Dedicated success team available around the clock.' },
                    { icon: Sparkles, title: 'AI-Powered', description: 'Smart automation that learns and improves over time.' },
                ]} />
            </div>
        </section>
    )
...
const FeatureCards = ({ items }: { items: FeatureItem[] }) => (
    <div className="grid gap-4 @md:gap-6 @sm:grid-cols-2 @xl:grid-cols-3">
        {items.map((item) => (
            <Card key={item.title} className="group text-center border-border/50 transition-all hover:border-primary/30 hover:shadow-lg">
