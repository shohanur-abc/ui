		...
		<section className="@container relative overflow-hidden" data-theme="neon">
			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32 relative">
				<div className="text-center max-w-3xl mx-auto mb-10 @md:mb-12 @xl:mb-16">
					...
					<Title
						text="Supercharge Your Workflow with"
					...
					items={[
						{
							...
							description:
								'ML models that predict trends and anomalies before they happen.',
							tag: 'Machine Learning',
						},
						{
							icon: MessageSquare,
							title: 'Natural Language',
							description: 'Query your data using plain English questions.',
							tag: 'NLP',
						},
						{
							icon: Wand2,
							...
							description:
								'Generate reports, summaries, and content automatically.',
							tag: 'Generative AI',
						},
						{
							icon: Cpu,
							title: 'Process Automation',
							description: 'AI identifies and automates repetitive workflows.',
							tag: 'RPA',
						},
					]}
				/>

				<CTASection label="Explore AI Features" href="/ai" />
			</div>
		</section>
	);
}
...
const Eyebrow = ({
	...
	<div className="mb-4">
		<Badge
			variant="secondary"
			className="gap-2 px-3 py-1.5 bg-gradient-to-r from-primary/20 to-accent/20 border-primary/30"
		>
...
const AIFeatureGrid = ({ items }: { items: AIFeature[] }) => (
	<div className="grid gap-5 @md:gap-6 @sm:grid-cols-2">
		{items.map((item) => (
			<Card
				key={item.title}
				className="group border-border/40 bg-card/40 backdrop-blur-xl transition-all hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10"
			>
