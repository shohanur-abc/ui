					...
					items={[
						{
							...
							description:
								'Optimized infrastructure delivering sub-millisecond response times for critical operations.',
						},
						{
							icon: Shield,
						...
						},
						{
							...
							description:
								'Connect with 200+ tools and platforms through our robust API ecosystem.',
						},
					]}
				/>
		...
		{items.map((item, index) => (
			...
			>
				<CardContent className={`p-5 @md:p-6 ${index === 0 ? '@lg:p-8' : ''}`}>
					<div
						className={`mb-4 inline-flex items-center justify-center rounded-xl bg-primary/10 ${index === 0 ? 'size-14 @lg:size-16' : 'size-12'}`}
					>
						<item.icon
							className={`text-primary ${index === 0 ? 'size-7 @lg:size-8' : 'size-6'}`}
						/>
					</div>
					<h3
						className={`mb-2 font-semibold ${index === 0 ? 'text-xl @lg:text-2xl' : 'text-lg'}`}
					>
						{item.title}
					</h3>
					<p
						className={`text-muted-foreground leading-relaxed ${index === 0 ? 'text-base @lg:text-lg' : 'text-sm'}`}
					>
						{item.description}
					</p>
				</CardContent>
