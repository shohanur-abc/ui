    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <CertificationGrid items={[
                    { icon: Medal, level: 'Foundation', title: 'Platform Essentials', description: 'Master the fundamentals and core features of our platform.', duration: '4 hours', badge: 'Foundation Badge' },
                    { icon: Star, level: 'Professional', title: 'Advanced Features', description: 'Deep dive into automation, integrations, and analytics.', duration: '8 hours', badge: 'Professional Badge' },
                    { icon: Trophy, level: 'Expert', title: 'Architecture & Scale', description: 'Enterprise deployment, security, and optimization strategies.', duration: '16 hours', badge: 'Expert Badge' },
                ]} />
...
const CertificationGrid = ({ items }: { items: CertificationItem[] }) => (
    <div className="grid gap-6 @lg:grid-cols-3 mb-12">
        {items.map((cert) => (
            <Link key={cert.title} href={`/certifications/${cert.level.toLowerCase()}`}>
                <Card className="group h-full border-border/50 bg-card/50 backdrop-blur-sm transition-all hover:border-primary/30 hover:shadow-lg">
                    <CardContent className="p-6">
                        <Badge variant="outline" className="mb-4">{cert.level}</Badge>
                        ...
                        <h3 className="text-lg font-semibold mb-2 group-hover:text-primary transition-colors">{cert.title}</h3>
