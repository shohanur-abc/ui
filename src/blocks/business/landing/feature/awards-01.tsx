...
import { Award, Clock, Globe, Rocket, Shield, Star, Trophy, Users, Zap } from 'lucide-react'
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <AwardsRow items={[
                    { icon: Trophy, title: 'Best SaaS Product', source: 'TechCrunch', year: '2025' },
                    { icon: Star, title: 'Top Rated', source: 'G2 Crowd', year: '2025' },
                    { icon: Award, title: 'Editor\'s Choice', source: 'PCMag', year: '2025' },
                    { icon: Shield, title: 'Most Secure', source: 'Security Weekly', year: '2024' },
                ]} />
...
const AwardsRow = ({ items }: { items: AwardItem[] }) => (
    <div className="grid grid-cols-2 @lg:grid-cols-4 gap-4 @md:gap-6">
        {items.map((item) => (
            <Card key={item.title} className="text-center border-border/50 transition-all hover:border-primary/30">
                <CardContent className="p-5 @md:p-6">
                    <div className="mb-3 mx-auto flex size-12 items-center justify-center rounded-full bg-primary/10">
                        <item.icon className="size-6 text-primary" />
                    </div>
                    <h3 className="text-sm font-semibold mb-1">{item.title}</h3>
                    <p className="text-xs text-muted-foreground">{item.source} • {item.year}</p>
                </CardContent>
            </Card>
        ))}
    </div>
