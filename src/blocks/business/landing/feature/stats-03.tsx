...
import { BarChart2, CircleDollarSign, Sparkles, Target, TrendingUp, Users } from 'lucide-react'
        ...
        <section className="@container relative overflow-hidden" data-theme="neon">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="text-center max-w-3xl mx-auto mb-10 @md:mb-12 @xl:mb-16">
                    ...
                    <Title text="Measurable Results That" highlight="Speak for Themselves" />
                    <Description text="Our platform delivers real, quantifiable improvements for businesses of all sizes." />
                </div>

                <StatsCardGrid items={[
                    { icon: Users, value: '2.5M+', label: 'Active Users', trend: '+28% MoM' },
                    { icon: CircleDollarSign, value: '$4.2B', label: 'Transactions Processed', trend: '+42% YoY' },
                    { icon: Target, value: '99.99%', label: 'Uptime SLA' },
                    { icon: BarChart2, value: '73%', label: 'Efficiency Improvement', trend: 'Average' },
                ]} />
            </div>
        </section>
    )
...
const StatsCardGrid = ({ items }: { items: StatItem[] }) => (
    <div className="grid gap-4 @md:gap-6 @sm:grid-cols-2 @xl:grid-cols-4">
        {items.map((item) => (
            <Card key={item.label} className="border-border/50 bg-card/50 backdrop-blur-sm text-center overflow-hidden">
