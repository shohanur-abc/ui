...
const ComparisonTable = ({ ourName, othersName, items }: { ourName: string; othersName: string; items: ComparisonItem[] }) => (
    <Card className="max-w-3xl mx-auto border-border/50 overflow-hidden">
        <CardContent className="p-0">
            ...
            <div className="grid grid-cols-3 p-4 @md:p-5 bg-muted/50">
                ...
                <div className="text-center text-sm font-medium text-muted-foreground">{othersName}</div>
            </div>
            <Separator />
            {/* Rows */}
...
const ComparisonValue = ({ value, positive }: { value: boolean | string; positive?: boolean }) => {
    if (typeof value === 'boolean') {
        return value ? (
            <CheckCircle2 className={`size-5 ${positive ? 'text-emerald-500' : 'text-muted-foreground'}`} />
        ) : (
            <X className="size-5 text-muted-foreground/50" />
        )
    }
    return (
        <span className={`text-sm font-medium ${positive ? 'text-primary' : 'text-muted-foreground'}`}>
            {value}
        </span>
    )
}
