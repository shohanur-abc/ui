					...
					items={[
						{
							...
							description:
								'Optimized for speed with edge caching and lazy loading.',
						},
						{
							icon: Shield,
							title: 'Enterprise Security',
							description:
								'Bank-grade encryption with SOC 2 Type II certification and regular penetration testing.',
						},
						{
							icon: Users,
						...
						},
						{
							...
							title: 'Global Scale',
							description:
								'Deploy to 200+ edge locations worldwide for consistent performance.',
						},
						{
							icon: Rocket,
							title: 'Quick Setup',
						...
						},
						{
							...
							description: 'Pick only the features you need.',
						},
					]}
				/>
			</div>
...
const MasonryGrid = ({ items }: { items: FeatureItem[] }) => (
	<div className="columns-1 @sm:columns-2 @xl:columns-3 gap-4 @md:gap-5 space-y-4 @md:space-y-5">
		{items.map((item, index) => (
			...
			>
				<CardContent
					className={`p-5 @md:p-6 ${index % 3 === 1 ? 'py-8 @md:py-10' : ''}`}
				>
