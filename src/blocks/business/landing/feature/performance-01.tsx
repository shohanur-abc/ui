...
import { Gauge, HardDrive, Network, RefreshCw, Server, Timer, Zap } from 'lucide-react'
        ...
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <PerformanceMetrics items={[
                    { icon: Timer, metric: 'API Response', value: '<50ms', description: 'Average response time' },
                    { icon: Gauge, metric: 'Page Load', value: '<1s', description: 'Time to interactive' },
                    { icon: Server, metric: 'Uptime', value: '99.99%', description: 'SLA guaranteed' },
                    { icon: Network, metric: 'Edge Nodes', value: '200+', description: 'Global locations' },
                    { icon: HardDrive, metric: 'Data Sync', value: 'Real-time', description: 'Instant updates' },
                    { icon: RefreshCw, metric: 'Backup', value: 'Every 5min', description: 'Automatic snapshots' },
                ]} />
            </div>
        </section>
    )
...
const PerformanceMetrics = ({ items }: { items: PerformanceMetric[] }) => (
    <div className="grid gap-4 @md:gap-6 @sm:grid-cols-2 @xl:grid-cols-3 max-w-5xl mx-auto">
        {items.map((metric) => (
            <Card key={metric.metric} className="group border-border/50 transition-all hover:border-primary/30 text-center">
