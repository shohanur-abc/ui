            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="grid gap-10 @xl:gap-16 @xl:grid-cols-2 items-center">
                    <div>
                        ...
                        <Title text="Replace Multiple Tools with" highlight="One Platform" />
                    ...
                    <FeatureColumns items={[
                        { title: 'Core Features', items: ['Project management', 'Team collaboration', 'Time tracking', 'Resource planning'] },
                        { title: 'Advanced Tools', items: ['Custom workflows', 'API integrations', 'Reporting suite', 'White labeling'] },
                        { title: 'Enterprise', items: ['SSO / SAML', 'Audit logging', 'Custom SLA', 'Dedicated support'] },
                    ]} />
                </div>
            </div>
        </section>
...
const FeatureColumns = ({ items }: { items: FeatureColumn[] }) => (
    <div className="grid gap-4 @sm:grid-cols-3">
        {items.map((column) => (
            <Card key={column.title} className="border-border/50 bg-card/50 backdrop-blur-sm">
                <CardContent className="p-5">
                    <h3 className="mb-4 text-sm font-semibold text-primary">{column.title}</h3>
