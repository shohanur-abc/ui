        ...
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="grid gap-10 @xl:gap-16 @xl:grid-cols-2 items-center">
                    <MetricsDisplay items={[
                        { icon: TrendingUp, label: 'Revenue Growth', value: '$2.4M', change: '+24%', positive: true },
                        { icon: BarChart3, label: 'Active Users', value: '48.2K', change: '+18%', positive: true },
                        { icon: LineChart, label: 'Conversion Rate', value: '3.8%', change: '+0.6%', positive: true },
                        { icon: PieChart, label: 'Market Share', value: '12.4%', change: '+2.1%', positive: true },
                    ]} />
                    <div>
                        <Eyebrow icon={BarChart3} text="Advanced Analytics" />
                        <Title text="Data-Driven Insights for" highlight="Smarter Decisions" />
...
const MetricsDisplay = ({ items }: { items: MetricItem[] }) => (
    <div className="grid gap-4 @sm:grid-cols-2">
        {items.map((item) => (
            <Card key={item.label} className="border-border/50 transition-all hover:border-primary/30 hover:shadow-md">
