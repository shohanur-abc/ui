...
import { ArrowRight, BookMarked, GraduationCap, Lightbulb, Play, Sparkles, Video } from 'lucide-react'
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <LearningPathGrid items={[
                    { icon: Lightbulb, level: 'Beginner', title: 'Getting Started', lessons: 8, duration: '2 hours', progress: 100 },
                    { icon: BookMarked, level: 'Intermediate', title: 'Core Features', lessons: 12, duration: '4 hours', progress: 60 },
                    { icon: Video, level: 'Advanced', title: 'Power User', lessons: 10, duration: '3 hours' },
                    { icon: GraduationCap, level: 'Expert', title: 'Mastery', lessons: 15, duration: '6 hours' },
                ]} />
            ...
            <Link key={path.title} href={`/learn/${path.level.toLowerCase()}`}>
                <Card className="group h-full border-border/50 transition-all hover:border-primary/30 hover:shadow-lg">
                    <CardContent className="p-5 @md:p-6">
                        <Badge variant="secondary" className="mb-4">{path.level}</Badge>
                        ...
                        <h3 className="font-semibold mb-2 group-hover:text-primary transition-colors">{path.title}</h3>
                        <p className="text-sm text-muted-foreground mb-4">{path.lessons} lessons • {path.duration}</p>
