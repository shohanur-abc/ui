...
import { CheckCircle2, Globe, HardDrive, Shield, Users, Zap } from 'lucide-react'
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <DetailedSpecs items={[
                    { category: 'Performance', specs: [{ name: 'Average Response Time', value: '45ms' }, { name: 'Throughput', value: '100K req/s' }, { name: 'Cold Start', value: '<100ms' }] },
                    { category: 'Scalability', specs: [{ name: 'Auto-scaling', value: 'Yes' }, { name: 'Max Instances', value: 'Unlimited' }, { name: 'Load Balancing', value: 'Global' }] },
                    { category: 'Security', specs: [{ name: 'Encryption', value: 'AES-256' }, { name: 'TLS Version', value: '1.3' }, { name: 'Certifications', value: 'SOC 2, GDPR' }] },
                    { category: 'Storage', specs: [{ name: 'Object Storage', value: 'Unlimited' }, { name: 'Database', value: 'Managed PostgreSQL' }, { name: 'Backup Retention', value: '30 days' }] },
                ]} />
            </div>
        </section>
    )
...
const QuickSpecs = ({ items }: { items: SpecItem[] }) => (
    <div className="grid grid-cols-2 @sm:grid-cols-3 @lg:grid-cols-5 gap-4">
        {items.map((item) => (
            <Card key={item.label} className="text-center border-border/50 transition-all hover:border-primary/30">
                ...
                <CardContent className="p-5">
                    ...
                    <ul className="space-y-3">
                        {category.specs.map((spec) => (
                            <li key={spec.name} className="flex items-center justify-between text-sm">
