...
import { Activity, ArrowUp, BarChart3, DollarSign, ShoppingCart, Users, Zap } from 'lucide-react'
    ...
    return (
        <section className="@container relative overflow-hidden" data-theme="corporate">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <DashboardPreview stats={[
                    { icon: DollarSign, label: 'Revenue', value: '$124.5K', change: '+12.5%', changeType: 'positive' },
                    { icon: Users, label: 'Active Users', value: '8,429', change: '+8.2%', changeType: 'positive' },
                    { icon: ShoppingCart, label: 'Orders', value: '1,847', change: '+23.1%', changeType: 'positive' },
                    { icon: Activity, label: 'Conversion', value: '3.24%', change: '-0.4%', changeType: 'negative' },
                ]} />
            </div>
        </section>
    )
                ...
                <div className="grid @sm:grid-cols-2 @xl:grid-cols-4 divide-y @sm:divide-y-0 @sm:divide-x divide-border/50">
                    {stats.map((stat) => (
                        <div key={stat.label} className="p-4 @md:p-6">
                            ...
                            <div className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${changeColors[stat.changeType]}`}>
                                <ArrowUp className={`size-3 ${stat.changeType === 'negative' ? 'rotate-180' : ''}`} />
                                {stat.change}
                            </div>
                        </div>
                    ))}
