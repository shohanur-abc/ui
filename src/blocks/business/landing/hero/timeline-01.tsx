            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32 w-full">
                <div className="grid @3xl:grid-cols-2 gap-12 @xl:gap-20 items-center">
                    <div>
                        ...
                        <Title text="Reclaim Your Most Precious Resource" highlight="Time" />
                    ...
                    <Timeline items={[
                        { time: 'Week 1', title: 'Onboarding', description: 'Quick setup and team training', status: 'complete' },
                        { time: 'Week 2', title: 'Integration', description: 'Connect your existing tools', status: 'complete' },
                        { time: 'Week 3', title: 'Automation', description: 'Configure workflows', status: 'current' },
                        { time: 'Week 4', title: 'Optimization', description: 'Fine-tune and scale', status: 'upcoming' },
                    ]} />
                </div>
            </div>
        </section>
...
const Timeline = ({ items }: { items: { time: string; title: string; description: string; status: 'complete' | 'current' | 'upcoming' }[] }) => (
    <div className="relative">
        <div className="absolute left-4 top-0 bottom-0 w-0.5 bg-border" />
        <div className="space-y-6">
            {items.map(({ time, title, description, status }, i) => (
                <div key={i} className="relative flex gap-4 pl-10">
                    <div className={`absolute left-0 size-8 rounded-full flex items-center justify-center ${
                    ...
                    <div className={`flex-1 p-4 rounded-lg border ${
                        status === 'current' ? 'bg-primary/5 border-primary/30' : 'bg-card border-border'
                    }`}>
