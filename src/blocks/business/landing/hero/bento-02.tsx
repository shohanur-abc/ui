				...
				<BentoGrid
					items={[
						...
						},
						{
							icon: MessageSquare,
							title: 'Communication',
							description: 'Built-in messaging and video',
							span: '',
						},
						{
							icon: FileText,
							title: 'Documents',
							description: 'Create, share, and manage files',
							span: '',
						},
						{
							icon: Settings,
...
const BentoGrid = ({
	...
	<div className="grid @sm:grid-cols-2 @xl:grid-cols-3 gap-4 @md:gap-6">
		{items.map(({ icon: Icon, title, description, span }, i) => (
			<Card
				key={i}
				className={`group hover:shadow-lg hover:border-primary/50 transition-all bg-card/50 backdrop-blur ${span}`}
			>
