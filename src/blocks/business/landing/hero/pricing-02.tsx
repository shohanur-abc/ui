            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <PricingGrid items={[
                    ...
                    },
                    {
                        ...
                        features: ['Up to 25 users', '100GB storage', 'Advanced analytics', 'Priority support', 'Custom integrations', 'SSO authentication'],
                        cta: 'Start Free Trial',
                        popular: true,
                    },
                    {
                        ...
                        features: ['Unlimited users', 'Unlimited storage', 'White-label option', 'Dedicated support', 'Custom development', 'SLA guarantee'],
                        cta: 'Contact Sales',
                        popular: false,
                    },
                ]} />
...
const PricingGrid = ({ items }: { items: { name: string; icon: ComponentType<{ className?: string }>; price: string; period: string; description: string; features: string[]; cta: string; popular: boolean }[] }) => (
    <div className="grid @md:grid-cols-3 gap-6 @xl:gap-8">
        {items.map(({ name, icon: Icon, price, period, description, features, cta, popular }, i) => (
            <Card key={i} className={`relative ${popular ? 'border-primary shadow-lg shadow-primary/10' : ''}`}>
                {popular && (
                ...
                <CardFooter>
                    <Button className="w-full gap-2" variant={popular ? 'default' : 'outline'} asChild>
                        <Link href="#start">
                            {cta}
