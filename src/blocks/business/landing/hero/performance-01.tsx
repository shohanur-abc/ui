                ...
                <div className="grid @3xl:grid-cols-2 gap-8 @xl:gap-16 items-center">
                    <div>
                        ...
                        <PerformanceMetrics items={[
                            { icon: Zap, label: 'API Latency', value: '<50ms', description: 'Average response time' },
                            { icon: TrendingUp, label: 'Throughput', value: '100K+', description: 'Requests per second' },
                            { icon: Activity, label: 'Uptime', value: '99.99%', description: 'Over the past year' },
                        ]} />
                        <CTA items={[
                            { label: 'Run Speed Test', href: '#test', icon: ArrowRight },
                            { label: 'View Benchmarks', href: '#benchmarks', variant: 'outline' },
                        ]} />
                    </div>
                    <SpeedVisualization />
                </div>
...
const PerformanceMetrics = ({ items }: { items: { icon: ComponentType<{ className?: string }>; label: string; value: string; description: string }[] }) => (
    ...
    <div className="relative aspect-square rounded-2xl bg-card/50 border border-border overflow-hidden">
        ...
        <div className="absolute top-4 right-4 space-y-2">
            <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">First Contentful Paint: 0.8s</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
                <span className="size-2 rounded-full bg-green-500" />
            ...
            <div className="flex items-center gap-2 text-xs">
                <span className="size-2 rounded-full bg-green-500" />
                <span className="text-muted-foreground">Cumulative Layout Shift: 0.01</span>
            </div>
        </div>
    </div>
)
