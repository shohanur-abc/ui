...
const SocialProof = ({ avatars, count, text }: { avatars: string[]; count: number; text: string }) => (
    <div className="bg-card border border-border rounded-2xl p-8 @md:p-10 text-center">
        <div className="flex justify-center mb-6">
            <div className="flex -space-x-3">
                {avatars.map((avatar, i) => (
                    <div key={i} className="relative size-12 rounded-full border-2 border-background overflow-hidden">
        ...
        <p className="text-lg font-semibold mb-1">
            <span className="text-primary">{count.toLocaleString()}+</span> people {text}
        </p>
        <p className="text-sm text-muted-foreground">Join the fastest-growing platform</p>
