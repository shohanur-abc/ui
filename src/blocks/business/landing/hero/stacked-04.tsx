    ...
    return (
        <section className="@container min-h-screen" data-theme="business-emerald">
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <LogoCloud items={[
                    { name: 'Company 1', logo: 'https://via.placeholder.com/150x50/f1f5f9/64748b?text=Company' },
                    { name: 'Company 2', logo: 'https://via.placeholder.com/150x50/f1f5f9/64748b?text=Company' },
                    { name: 'Company 3', logo: 'https://via.placeholder.com/150x50/f1f5f9/64748b?text=Company' },
                    { name: 'Company 4', logo: 'https://via.placeholder.com/150x50/f1f5f9/64748b?text=Company' },
                    { name: 'Company 5', logo: 'https://via.placeholder.com/150x50/f1f5f9/64748b?text=Company' },
                    { name: 'Company 6', logo: 'https://via.placeholder.com/150x50/f1f5f9/64748b?text=Company' },
                ]} />
    ...
    <div className="grid grid-cols-2 @sm:grid-cols-3 @xl:grid-cols-6 gap-6 @md:gap-8 items-center mb-10 @md:mb-14">
        {items.map(({ name, logo }, i) => (
            <div key={i} className="flex justify-center opacity-60 hover:opacity-100 transition-opacity">
                <Image src={logo} alt={name} width={150} height={50} className="h-8 @md:h-10 w-auto object-contain" />
            </div>
        ))}
    </div>
)
    ...
    <div className="flex flex-col items-center gap-2">
        <div className="flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className="size-5 text-primary" fill={i < Math.floor(value) ? 'currentColor' : 'none'} />
            ))}
        </div>
        <p className="text-sm text-muted-foreground">
            <span className="font-semibold text-foreground">{value}</span> out of 5 based on <span className="font-semibold text-foreground">{reviews.toLocaleString()}</span> reviews
        </p>
    </div>
)
