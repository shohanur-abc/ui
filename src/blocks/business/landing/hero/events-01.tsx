...
const EventGrid = ({
	...
	<div className="grid @xl:grid-cols-2 gap-6">
		{items.map(
			(
				{ type, icon: Icon, title, date, time, attendees, image, featured },
				i,
			) => (
				<div
					key={i}
				...
				>
					<div className={`grid ${featured ? '@xl:grid-cols-2' : ''}`}>
						<div
							className={`relative ${featured ? 'aspect-[16/9] @xl:aspect-auto' : 'aspect-video'}`}
						>
							<Image
								src={image}
								alt={title}
						...
						<div className="p-6 @md:p-8 flex flex-col justify-center">
							...
							<h3
								className={`font-bold mb-3 group-hover:text-primary transition-colors ${featured ? 'text-2xl @md:text-3xl' : 'text-xl'}`}
							>
								{title}
							</h3>
