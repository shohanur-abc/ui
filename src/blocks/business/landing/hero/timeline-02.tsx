            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                <div className="text-center mb-12 @md:mb-16">
                    ...
                    <CTA items={[
                        ...
                        { label: 'See Success Stories', href: '#stories', variant: 'outline' },
                    ]} />
                </div>
                <HorizontalTimeline items={[
                    { step: 1, title: 'Discovery', description: 'Deep dive into your business needs and goals', status: 'complete' },
                    { step: 2, title: 'Strategy', description: 'Craft a tailored roadmap for success', status: 'complete' },
                    { step: 3, title: 'Execution', description: 'Implement with precision and agility', status: 'current' },
                    { step: 4, title: 'Scale', description: 'Grow and optimize for maximum impact', status: 'upcoming' },
                ]} />
            </div>
        </section>
    )
...
const HorizontalTimeline = ({ items }: { items: { step: number; title: string; description: string; status: 'complete' | 'current' | 'upcoming' }[] }) => (
                ...
                <div key={i} className="flex gap-4">
                    <div className="flex flex-col items-center">
                        <div className={`size-10 rounded-full flex items-center justify-center font-bold ${
                            ...
                            {status === 'complete' ? <CheckCircle className="size-5" /> : step}
                        </div>
                        {i < items.length - 1 && <div className="w-0.5 h-full min-h-12 bg-border mt-2" />}
                    </div>
                    <div className={`flex-1 p-4 rounded-lg border ${
                        status === 'current' ? 'bg-primary/5 border-primary/30' : 'bg-card'
                    }`}>
                ...
                {items.map(({ step, title, status }, i) => (
                    <div key={i} className="flex items-center flex-1">
                        <div className={`size-12 rounded-full flex items-center justify-center font-bold text-lg ${
                            ...
                            {status === 'complete' ? <CheckCircle className="size-6" /> : step}
                        </div>
                        {i < items.length - 1 && (
