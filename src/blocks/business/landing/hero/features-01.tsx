					...
					items={[
						{
							...
							description:
								'Data centers in 25+ regions for ultra-low latency worldwide',
							stat: '25+',
							statLabel: 'Regions',
						},
						{
						...
						},
						{
							...
							description: 'Sub-50ms response times even under heavy load',
							stat: '<50ms',
						...
						},
						{
							...
							description: 'Industry-leading SLA with 99.99% uptime guarantee',
							stat: '99.99%',
...
const FeatureGrid = ({
	...
	<div className="grid @sm:grid-cols-2 @xl:grid-cols-3 gap-4 @md:gap-6">
		{items.map(({ icon: Icon, title, description, stat, statLabel }, i) => (
			<Card
				key={i}
				className="group hover:shadow-lg hover:border-primary/30 transition-all overflow-hidden"
			>
