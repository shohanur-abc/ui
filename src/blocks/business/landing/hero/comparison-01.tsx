            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32">
                ...
                <ComparisonTable 
                    ...
                    rows={[
                        ...
                        { feature: 'Advanced Analytics', us: true, compA: true, compB: false },
                        { feature: '24/7 Support', us: true, compA: false, compB: false },
                        { feature: 'API Access', us: true, compA: true, compB: true },
                        { feature: 'Custom Integrations', us: true, compA: false, compB: false },
                        { feature: 'White-label Option', us: true, compA: true, compB: false },
                        { feature: 'Starting Price', us: '$49/mo', compA: '$99/mo', compB: '$79/mo' },
                    ]}
                />
                <div className="text-center mt-10">
                    <CTA items={[
                        { label: 'Start Free Trial', href: '#trial', icon: ArrowRight },
                        { label: 'Full Comparison', href: '#compare', variant: 'outline' },
                    ]} />
                </div>
            </div>
        </section>
...
const ComparisonTable = ({ headers, rows }: { headers: string[]; rows: { feature: string; us: boolean | string; compA: boolean | string; compB: boolean | string }[] }) => (
                    ...
                    <thead>
                        <tr className="border-b border-border bg-muted/50">
                            {headers.map((header, i) => (
                                <th key={i} className={`px-4 @md:px-6 py-4 text-left font-semibold ${i === 1 ? 'bg-primary/10' : ''}`}>
                                    {header}
                                    {i === 1 && <Badge className="ml-2 text-xs">Recommended</Badge>}
                                </th>
                            ))}
                        </tr>
                    </thead>
...
const CellValue = ({ value, highlighted = false }: { value: boolean | string; highlighted?: boolean }) => {
    if (typeof value === 'boolean') {
        return value ? (
            <Check className={`size-5 ${highlighted ? 'text-primary' : 'text-green-500'}`} />
        ) : (
            <X className="size-5 text-muted-foreground/50" />
        )
    }
    return <span className={highlighted ? 'font-semibold text-primary' : ''}>{value}</span>
}
