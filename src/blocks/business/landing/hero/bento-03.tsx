...
const BentoLayout = ({
	...
	<div className="grid grid-cols-2 gap-4">
		{items.map(({ icon: Icon, title, value, label, variant }, i) => (
			...
			>
				<CardContent
					className={`pt-6 ${variant === 'highlight' ? 'pb-4' : ''}`}
				>
					<div
					...
					>
						<Icon
							className={`size-5 ${variant === 'highlight' ? 'text-primary-foreground' : 'text-primary'}`}
						/>
					</div>
					<p
						className={`text-sm mb-2 ${variant === 'highlight' ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}
					>
						{title}
					</p>
					<p
						className={`text-3xl @md:text-4xl font-bold mb-1 ${variant === 'highlight' ? '' : 'text-foreground'}`}
					>
						{value}
					</p>
					<p
