...
export default function Main() {
    return (
        <section className="@container min-h-screen flex items-center relative overflow-hidden" data-theme="business-amber">
            <GradientDecorative />
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32 w-full relative z-10">
                <div className="grid @3xl:grid-cols-2 gap-8 @xl:gap-16 items-center">
                    ...
                    <FeatureCards items={[
                        { icon: Zap, title: 'Lightning Speed', description: 'Sub-100ms response times globally', color: 'from-yellow-500/20 to-orange-500/20' },
                        { icon: Shield, title: 'Bank-Level Security', description: 'SOC 2 Type II certified', color: 'from-blue-500/20 to-cyan-500/20' },
                        { icon: Rocket, title: 'Infinite Scale', description: 'From 10 to 10 million users', color: 'from-purple-500/20 to-pink-500/20' },
                    ]} />
                </div>
            </div>
        </section>
...
const FeatureCards = ({ items }: { items: { icon: ComponentType<{ className?: string }>; title: string; description: string; color: string }[] }) => (
    <div className="space-y-4">
        {items.map(({ icon: Icon, title, description, color }, i) => (
            <Card key={i} className="group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 backdrop-blur-sm bg-card/80 border-border/50">
                <CardContent className="flex items-center gap-4 py-5">
                    <div className={`size-14 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center group-hover:scale-110 transition-transform`}>
