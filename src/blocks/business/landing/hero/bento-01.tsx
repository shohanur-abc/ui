...
const BentoGrid = ({
	...
	<div className="grid grid-cols-2 gap-4">
		{items.map(({ icon: Icon, title, description, size }, i) => (
			...
			>
				<CardContent
					className={`flex flex-col justify-center ${size === 'large' ? 'pt-8 pb-2' : 'pt-6'}`}
				>
					<div
						className={`rounded-xl bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/20 transition-colors ${
							size === 'large' ? 'size-16' : 'size-12'
						}`}
					>
						<Icon
							className={`text-primary ${size === 'large' ? 'size-8' : 'size-6'}`}
						/>
					</div>
					<h3
						className={`font-semibold mb-1 ${size === 'large' ? 'text-xl' : 'text-base'}`}
					>
						{title}
					</h3>
					<p
						className={`text-muted-foreground ${size === 'large' ? 'text-base' : 'text-sm'}`}
					>
						{description}
					</p>
				</CardContent>
