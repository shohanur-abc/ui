                ...
                <div className="grid @3xl:grid-cols-2 gap-8 @xl:gap-16 items-center">
                    <div>
                        ...
                        <CTA items={[
                            ...
                            { label: 'Watch Demo', href: '#demo', icon: Play, variant: 'outline' },
                        ]} />
                    </div>
                    <ProductShowcase 
...
const StatsList = ({ items }: { items: { icon: ComponentType<{ className?: string }>; value: string; label: string }[] }) => (
...
const ProductShowcase = ({ image, caption }: { image: string; caption: string }) => (
    <div className="relative">
        <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl border border-border">
            <Image src={image} alt="Product Screenshot" fill className="object-cover" />
        </div>
