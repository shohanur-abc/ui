			...
			<div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32 w-full">
				<div className="grid @3xl:grid-cols-2 gap-8 @xl:gap-16 items-center">
					...
					<div>
						...
						<Title
							text="Recognized by Industry Leaders"
		...
		<div className="flex items-center gap-2">
			<div className="flex">
				{Array.from({ length: 5 }).map((_, i) => (
					<Star
						key={i}
						className={`size-5 ${i < Math.floor(averageRating) ? 'text-primary fill-primary' : 'text-muted-foreground'}`}
					/>
				))}
			</div>
...
const AwardsShowcase = ({
	...
	<div className="grid grid-cols-2 gap-4">
		{items.map(({ icon: Icon, title, subtitle, rating }, i) => (
			<div
				key={i}
				className="group bg-card border border-border rounded-2xl p-6 text-center hover:shadow-lg hover:border-primary/30 transition-all"
			>
