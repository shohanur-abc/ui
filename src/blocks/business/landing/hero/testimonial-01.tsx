            ...
            <div className="mx-auto max-w-7xl px-4 @sm:px-6 @2xl:px-8 py-16 @md:py-20 @xl:py-24 @3xl:py-32 w-full">
                <div className="grid @3xl:grid-cols-2 gap-8 @xl:gap-16 items-center">
                    ...
                    <div>
                        ...
                        <CTA items={[
                            { label: 'Read More Stories', href: '#stories', icon: ArrowRight },
...
const CustomerLogos = ({ items }: { items: string[] }) => (
    <div className="flex flex-wrap gap-6 items-center mb-8">
        {items.map((logo, i) => (
            <Image key={i} src={logo} alt="Customer logo" width={80} height={32} className="h-8 w-auto opacity-60 hover:opacity-100 transition-opacity" />
        ))}
    </div>
)
...
const TestimonialCard = ({ quote, author, role, avatar, rating }: { quote: string; author: string; role: string; avatar: string; rating: number }) => (
    <div className="relative bg-card border border-border rounded-2xl p-6 @md:p-8 shadow-xl">
        ...
        <div className="flex gap-1 mb-4">
            {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`size-5 ${i < rating ? 'text-primary fill-primary' : 'text-muted-foreground'}`} />
            ))}
        </div>
        <p className="text-lg @md:text-xl leading-relaxed mb-6">&ldquo;{quote}&rdquo;</p>
