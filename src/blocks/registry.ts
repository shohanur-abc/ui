...
export class Blocks extends Array<IMetadataFlat> {
	...
	elements(items: string[]) {
		return this.filter((b) =>
			items.every((item) => b.elements.includes(item)),
		) as this;
	}
	...
	dateRange(from: string, to: string = new Date().toISOString().slice(0, 10)) {
	...
	lazy(params: Partial<IMetadataFlat>) {
		const paramsEntries = Object.entries(params).filter(
		...
		return this.filter((block) => {
			for (const [key, value] of paramsEntries) {
				...
				if (
					Array.isArray(value) &&
					!value.every((v) => (blockValue as string[]).includes(v))
				)
					return false;
			}
			return true;
