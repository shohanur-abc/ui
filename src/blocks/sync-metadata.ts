...
class metadata {
		...
		await Promise.all(
			(await this.tsxFolderPaths()).map(async (folder) => {
				await writeFile(
					...
					JSON.stringify(await this.buildMetadata(folder), null, 2),
				);
			}),
		).catch((err) => {
			console.error(err);
	...
	static tsxFolderPaths = (loc = 'src/blocks') =>
		this.tsxPaths(loc).then((paths) => [...new Set(paths.map(path.dirname))]);
			...
			data: await Promise.all(
				(await this.tsxPaths(folder)).map(async (tsx) => {
					try {
						...
						const variant = meta?.data?.find(
							(d: IBlock) => d.inode === stats.ino,
						);
						return {
						...
						console.error('Error processing at meta.data:', e);
					}
				}),
			),
		} as IMetadata;
