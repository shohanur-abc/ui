...
class registry {
    static async build() {
        ...
        const results = (await Promise.all(metaPaths.map(registry.buildBlock))).flat().sort((a, b) => {
            return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
        });
        await writeFile('src/blocks/registry.json', JSON.stringify(results, null, 2));
    }

    static store: IMetadataFlat[] = []

    static metadataPaths = (loc = "src/blocks") => Array.fromAsync(glob(`${loc}/**/metadata.json`));
