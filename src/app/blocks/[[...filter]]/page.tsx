...
export default async function AllBlocksPage({
    ...
    searchParams: Promise<{ dateFrom?: string; dateTo?: string; sort?: 'asc' | 'desc' }>
}) {
    ...
    if (filter?.at(-1) === "preview") {
        ...
        return <Playground href={"blocks/" + filter.slice(0, -1).join("/")} website={website || "websites"} />
    }

    // Get filtered blocks from server
    const { filteredBlocks, websitesKeys: websites, blocks: blockTypes, variants } = await getFilteredBlocks(
        website,
        block,
        variant,
        dateFrom,
