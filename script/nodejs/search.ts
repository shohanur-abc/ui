...
export class Search {
    ...
    static getFilesByExtension(dirPath: string, ext: string, recursive = true): string[] {
        const pattern = recursive ? `${dirPath}/**/*.${ext}` : `${dirPath}/*.${ext}`;
    ...
    static async findFiles(dirPath: string, predicate: (name: string) => boolean): Promise<string[]> {
    ...
    static async searchInFiles(
        ...
        const files = ext ? this.getFilesByExtension(dirPath, ext) : this.glob(`${dirPath}/**/*`);
    ...
    static async getModifiedSince(dirPath: string, date: Date): Promise<string[]> {
        const files = this.glob(`${dirPath}/**/*`);
        const result: string[] = [];
    ...
    static async getModifiedBefore(dirPath: string, date: Date): Promise<string[]> {
        const files = this.glob(`${dirPath}/**/*`);
        const result: string[] = [];
