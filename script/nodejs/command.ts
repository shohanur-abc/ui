...
export class Command {
    ...
    static execWithOutput(command: string): { stdout: string; stderr: string; code: number } {
        try {
            const stdout = execSync(command, { encoding: 'utf-8' });
            return { stdout: stdout.trim(), stderr: '', code: 0 };
        } catch (error: unknown) {
            const err = error as { stdout?: Buffer; stderr?: Buffer; status?: number; message?: string };
            return {
